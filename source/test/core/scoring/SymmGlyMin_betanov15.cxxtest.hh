...
static THREAD_LOCAL basic::Tracer TR("core.scoring.SymmGlyMin_betanov15_Tests.cxxtest");
	...
	void repeat_structure_test( core::scoring::ScoreFunctionOP sfxn, bool const cartesian ) {
		...
		for ( int iphi=-180; iphi<180; iphi+=60 ) {
			for ( int ipsi=-180; ipsi<=180; ipsi+=60 ) {
				...
				if( !skip_pose3_tests ) TS_ASSERT_DELTA(pose->energies().total_energy(), pose3->energies().total_energy(), std::max( std::abs( std::max(pose->energies().total_energy(), pose3->energies().total_energy())/1000.0 ), 1e-12 ) ); //Skip this check for phi=0, psi=0 and the rama score term.  There's a known silly discontinuity there.
