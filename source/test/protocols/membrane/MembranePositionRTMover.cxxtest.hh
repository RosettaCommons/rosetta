...
/// @file		protocols/membrane/MembranePositionRTMover.cxxtest.hh
///
...
public:
    ...
    void test_membrane_position_translation() {
		...
		MembranePositionTranslationMoverOP xyz_move = new MembranePositionTranslationMover( xyz_trans );
		xyz_move->apply( *pose_ );
		
		// Check result
		TS_ASSERT( xyz_trans == pose_->conformation().membrane_center() );
    ...
    void test_membrane_position_rotation() {
		...
		// Grab current normal
		Vector current_normal( pose_->conformation().membrane_normal() );
		
		// Simple rotation 1
		Vector rot_1( 0, 1, 0 );
		MembranePositionRotationMoverOP first_move = new MembranePositionRotationMover( rot_1 );
		first_move->apply( *pose_ );
		TS_ASSERT_DELTA( angle_of( current_normal, pose_->conformation().membrane_normal() ), 1.57, 0.001);
		TS_ASSERT( position_equal_within_delta( rot_1, pose_->conformation().membrane_normal(), 0.0001 ) );
	...
	void test_uniform_rotation_translation() {
		...
		MembranePositionRTMoverOP rt = new MembranePositionRTMover( new_center, new_normal );
		rt->apply( *pose_ );
		
		// Check the structure was moved to the correct position
		TS_ASSERT( position_equal_within_delta( new_center, pose_->conformation().membrane_center(), 0.0001 ) );
		TS_ASSERT( position_equal_within_delta( new_normal, pose_->conformation().membrane_normal(), 0.0001 ) );
	}
