...
/// @file  protocols/farna//RNA_DeNovoProtocolTest.cxxtest.hh
...
public:
	...
	void test_sarcin_ricin_loop_fixed(){
		...
		options->set_rna_params_file("protocols/farna/farna_rebuild.params");
		options->set_chunk_pdb_files( make_vector1( "protocols/farna/srl_fixed_START1_1q9a_RNA.pdb") );
		...
		std::string const sequence = core::sequence::read_fasta_file_return_str( "protocols/farna/farna_rebuild.fasta" );
		Pose pose;
		ResidueTypeSetCOP rsd_set = core::chemical::ChemicalManager::get_instance()->residue_type_set( FA_STANDARD );
	...
	void test_sarcin_ricin_loop_free_with_base_pair_steps(){
		...
		options->initialize_from_command_line();
		// Sarcin-ricin loop modeling run
		options->set_rna_params_file("protocols/farna/bps_test.params");
