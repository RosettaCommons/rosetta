...
public:
	...
	void test_correctly_create_abbreviated_assembly(){
		TS_TRACE( "Begin correctly_create_abbreviated_assembly" );
		assembly_ = sewing_testing::create_simple_non_vital_assembly( assembly_ );
		utility::vector1< protocols::sewing::data_storage::BasisPair > bp_list;
		core::Size first_res_id = 5;
		core::Size first_seg_id = 4;
		core::Size second_seg_id = 21;
		core::Size second_res_id = 3;
		protocols::sewing::data_storage::Basis b1 = protocols::sewing::data_storage::Basis( first_seg_id, first_res_id );
		protocols::sewing::data_storage::Basis b2 = protocols::sewing::data_storage::Basis( second_seg_id, second_res_id );
		bp_list.push_back( std::make_pair( b1, b2 ) );
		first_res_id = 6;
		first_seg_id = 6;
		second_res_id = 7;
		second_seg_id = 13;
		b1 = protocols::sewing::data_storage::Basis( first_seg_id, first_res_id );
		b2 = protocols::sewing::data_storage::Basis( second_seg_id, second_res_id );
		bp_list.push_back( std::make_pair( b1, b2 ) );

		utility::vector1< protocols::sewing::data_storage::BasisPair > bp_test = assembly_->get_abbreviated_assembly();
		TS_TRACE( "Checking the same number of basis pairs are recovered" );
		TS_ASSERT_EQUALS( bp_list.size(), bp_test.size() );
		TS_TRACE( "Checking that all basis pairs are equal" );
		for ( core::Size i = 1; i < bp_test.size(); ++i ) {
			TS_ASSERT_EQUALS( bp_list[ i ].first.segment_id() , bp_test[ i ].first.segment_id() );
			TS_ASSERT_EQUALS( bp_list[ i ].first.resnum() , bp_test[ i ].first.resnum() );
			TS_ASSERT_EQUALS( bp_list[ i ].second.segment_id() , bp_test[ i ].second.segment_id() );
			TS_ASSERT_EQUALS( bp_list[ i ].second.resnum() , bp_test[ i ].second.resnum() );
		}
	}

	void test_correctly_recover_basic_assembly_from_abbreviated_assembly(){
		TS_TRACE( "Begin correctly_recover_basic_assembly_from_abbreviated_assembly" );
		assembly_ = sewing_testing::create_simple_non_vital_assembly( assembly_ );
		utility::vector1< protocols::sewing::data_storage::BasisPair > bp_test = assembly_->get_abbreviated_assembly();
		std::string org_fwd_assembly = assembly_->get_comprehensive_forward_assembly();
		std::string org_rvs_assembly = assembly_->get_comprehensive_reverse_assembly();
		TS_TRACE( "Forward Assembly: " + org_fwd_assembly );
		TS_TRACE( "Reverse Assembly: " + org_rvs_assembly );
		//first test adding to n_term
		bool n_term = true;
		bool add = assembly_->add_segment( n_term );
		TS_TRACE( "Forward Assembly: " + assembly_->get_comprehensive_forward_assembly() );
		TS_TRACE( "Reverse Assembly: " + assembly_->get_comprehensive_reverse_assembly() );
		TS_ASSERT( add );
		TS_ASSERT( assembly_->get_comprehensive_forward_assembly() != org_fwd_assembly );
		TS_ASSERT( assembly_->get_comprehensive_reverse_assembly() != org_rvs_assembly );
		assembly_->revert_to_abbreviated_assembly( bp_test );
		TS_TRACE( "Forward Assembly: " + assembly_->get_comprehensive_forward_assembly() );
		TS_TRACE( "Reverse Assembly: " + assembly_->get_comprehensive_reverse_assembly() );
		TS_ASSERT_EQUALS( assembly_->get_comprehensive_forward_assembly(), org_fwd_assembly );
		TS_ASSERT_EQUALS( assembly_->get_comprehensive_reverse_assembly(), org_rvs_assembly );
	}

	void test_correctly_create_cterm_double_chimaera_abbreviated_assembly(){
		TS_TRACE( "Begin correctly_create_cterm_double_chimaera_abbreviated_assembly" );
		assembly_ = sewing_testing::make_double_chimaera_cterm_first( assembly_ );
		utility::vector1< protocols::sewing::data_storage::BasisPair > bp_test = assembly_->get_abbreviated_assembly();
		utility::vector1< protocols::sewing::data_storage::BasisPair > bp_list;
		core::Size seg1 = 31;
		...
		core::Size seg2 = 21;
		core::Size res2 = 4;
		protocols::sewing::data_storage::Basis b1 = protocols::sewing::data_storage::Basis( seg1, res1 );
		protocols::sewing::data_storage::Basis b2 = protocols::sewing::data_storage::Basis( seg2, res2 );
		bp_list.push_back( std::make_pair( b1, b2 ) );
		seg1 = 31;
		res1 = 8;
		seg2 = 1;
		res2 = 3;
		b1 = protocols::sewing::data_storage::Basis( seg1, res1 );
		b2 = protocols::sewing::data_storage::Basis( seg2, res2 );
		bp_list.push_back( std::make_pair( b1, b2 ) );
		TS_TRACE( "Checking the same number of basis pairs are recovered" );
		TS_ASSERT_EQUALS( bp_list.size(), bp_test.size() );
		TS_TRACE( "Checking that all basis pairs are equal" );
	...
	void test_correctly_recover_double_cterm_chimaera_assembly_from_abbreviated_assembly(){
		TS_TRACE( "Begin correctly_recover_double_cterm_chimaera_assembly_from_abbreviated_assembly" );
		assembly_ = sewing_testing::make_double_chimaera_cterm_first( assembly_ );
		utility::vector1< protocols::sewing::data_storage::BasisPair > org_bp_test = assembly_->get_abbreviated_assembly();
		std::string org_fwd_assembly = assembly_->get_comprehensive_forward_assembly();
		std::string org_rvs_assembly = assembly_->get_comprehensive_reverse_assembly();
		TS_TRACE( "Forward Assembly: " + org_fwd_assembly );
		TS_TRACE( "Reverse Assembly: " + org_rvs_assembly );
		...
		bool add = assembly_->add_segment( n_term );
		TS_ASSERT( add );
		TS_ASSERT( org_fwd_assembly != assembly_->get_comprehensive_forward_assembly() );
		TS_ASSERT( org_rvs_assembly != assembly_->get_comprehensive_reverse_assembly() );
		TS_TRACE( "Forward Assembly: " + assembly_->get_comprehensive_forward_assembly() );
		TS_TRACE( "Reverse Assembly: " + assembly_->get_comprehensive_reverse_assembly() );
		assembly_->revert_to_abbreviated_assembly( org_bp_test );
		utility::vector1< protocols::sewing::data_storage::BasisPair > final_bp_test = assembly_->get_abbreviated_assembly();
		TS_TRACE( "Checking the same number of basis pairs are recovered" );
		TS_ASSERT_EQUALS( org_bp_test.size(), final_bp_test.size() );
		TS_TRACE( "Checking that all basis pairs are equal" );
		for ( core::Size i = 1; i < org_bp_test.size(); ++i ) {
			TS_ASSERT_EQUALS( org_bp_test[ i ].first.segment_id() , final_bp_test[ i ].first.segment_id() );
			TS_ASSERT_EQUALS( org_bp_test[ i ].first.resnum() , final_bp_test[ i ].first.resnum() );
			TS_ASSERT_EQUALS( org_bp_test[ i ].second.segment_id() , final_bp_test[ i ].second.segment_id() );
			TS_ASSERT_EQUALS( org_bp_test[ i ].second.resnum() , final_bp_test[ i ].second.resnum() );
		}
	}

	void test_correctly_create_nterm_double_chimaera_abbreviated_assembly(){
		TS_TRACE( "Begin correctly_create_nterm_double_chimaera_abbreviated_assembly" );
		assembly_ = sewing_testing::make_double_chimaera_nterm_first( assembly_ );
		utility::vector1< protocols::sewing::data_storage::BasisPair > bp_test = assembly_->get_abbreviated_assembly();
		utility::vector1< protocols::sewing::data_storage::BasisPair > bp_list;
		core::Size seg1 = 31;
	...
	void test_correctly_recover_double_nterm_chimaera_assembly_from_abbreviated_assembly(){
		TS_TRACE( "Begin correctly_recover_double_nterm_chimaera_assembly_from_abbreviated_assembly" );
		assembly_ = sewing_testing::make_double_chimaera_nterm_first( assembly_ );
		utility::vector1< protocols::sewing::data_storage::BasisPair > org_bp_test = assembly_->get_abbreviated_assembly();
		std::string org_fwd_assembly = assembly_->get_comprehensive_forward_assembly();
		std::string org_rvs_assembly = assembly_->get_comprehensive_reverse_assembly();
	...
	void test_correctly_recover_complex_double_chimaera_assembly_from_abbreviated_assembly(){
		TS_TRACE( "Begin correctly_recover_complex_double_chimaera_assembly_from_abbreviated_assembly" );
		assembly_ = sewing_testing::make_double_chimaera_nterm_first( assembly_ );
		bool n_term = true;
		bool add = assembly_->add_segment( n_term );
		TS_ASSERT( add );
		add = assembly_->add_segment( n_term );
		TS_ASSERT( add );
		add = assembly_->add_segment( n_term );
		TS_ASSERT( add );
		n_term = false;
		...
		TS_TRACE( "Reverse Assembly: " + org_rvs_assembly );
		n_term = true;
		bool del = assembly_->delete_segment( n_term );
		TS_ASSERT( del );
		add = assembly_->add_segment( n_term );
		TS_ASSERT( add );
		add = assembly_->add_segment( n_term );
		TS_ASSERT( add );
		n_term = false;
		del = assembly_->delete_segment( n_term );
		TS_ASSERT( del );
		TS_ASSERT( org_fwd_assembly != assembly_->get_comprehensive_forward_assembly() );
		TS_ASSERT( org_rvs_assembly != assembly_->get_comprehensive_reverse_assembly() );
		TS_TRACE( "Forward Assembly: " + assembly_->get_comprehensive_forward_assembly() );
	...
	void test_auto_detect_contacts(){
		...
		TS_ASSERT( assembly_->get_local_ligands().at( 1 )->get_owner_segment()->get_segment_id() == 32 );
		...
		for ( data_storage::LigandContactOP contact: contacts ) {
			...
			TS_ASSERT( contact->segment_id == 32 );
	...
	void test_maintain_contact_cterm_chimerization(){
		...
		//The basis residue on segment 32 (the assembly) cannot be <= 14
		...
		TS_ASSERT( std::dynamic_pointer_cast< data_storage::LigandSegment const >( assembly_->local_segments().at( 32 ) )->get_const_owned_ligand_residues().size() > 0 );
	}
