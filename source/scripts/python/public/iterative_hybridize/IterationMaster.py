...
#ENVIRONMENT SETUP
ROSETTABIN = '~/Rosetta/main/source/bin' # Please modify this line
DB = '~/Rosetta/main/database' # Please modify this line
...
######## Preparation
	...
	if self.it > 0:
        ...
        cmd = '%s/extract_pdbs.linuxgccrelease -database %s '%(ROSETTABIN,DB)+\
...
######## Structure Generation
    ...
    def make_joblist(self):
        ...
        cmds = []
        form = '%s/runhyb.sh %s %s %s %s %d %s %s %s %s'
        for icomb,comb in enumerate(self.combs):
            ...
            cmd = form%(SCRIPTDIR,self.pwd,ROSETTABIN,DB,SCRIPTDIR,
                        self.it,templatesstr,mode,iseed,extra_cmd)
        ...
        while True:
            ...
            if os.path.exists(GENSILENT):
                ...
                if dmin > self.opt.max_min_perjob and n == 0:
                    sys.exit('Iter %d not producing anything in %f minutes! Terminate!'%(dmin,self.it))
                if dmin > self.opt.max_min_perjob:
                    sys.exit('Iter %d taking too long: %f minutes! Terminate!'%(dmin,self.it))
            else:
                if self.opt.debug:
                    os.system('echo "parallel -j 12 --workdir . :::: alljobs" > run.cmd')
                    os.system('%s -j 12 --workdir . :::: alljobs &> run.log'%(PARALLELBIN))
                else:
...
######## Selection
    ...
    def run_selection(self):
        ...
        cmdline = '%s/iterhybrid_selector.linuxgccrelease -database %s'%(ROSETTABIN,DB)+\
            ' -in:file:silent %s -template_silent %s -out:file:silent %s'%(GENSILENT,PARENTSILENT,OFFSPRINGSILENT)+\
            ' -score:weights ref2015_cart'+\
            ' -cst_fa_file ../fa.cst -set_weights atom_pair_constraint 1.0'+\
