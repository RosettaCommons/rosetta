...
/// @file apps/pilot/kalngyk/rmsd.hh
/// @author YK Ng & SC Li (kalngyk@gmail.com)

#ifndef apps_pilot_kalngyk_rmsd_HH
#define apps_pilot_kalngyk_rmsd_HH
...
#include <stdio.h>
#include <math.h>
#include <calibur/jacobi.hh>
#include <calibur/cubic.hh>
...
double RMSD(double * coords1, double * coords2, int n, double R[3][3])
{
    /*************************************************************************
    ...
    if (!jacobi3(CCt, eigenval, eigenvec))
        return -1.;
    ...
    switch (eigenval[0] >= eigenval[1])
    {
    case 1:
        switch (eigenval[1] >= eigenval[2])
        {
        case 1: // eigenval[0] >= eigenval[1] >= eigenval[2]
            ...
            break;
        case 0: // eigenval[1] <= min{ eigenval[0], eigenval[2] }
            eigenval2 = eigenval[1];
            for (int i=0; i < 3; i++) eigenvec2[i] = eigenvec[i][1];
            switch (eigenval[0] >= eigenval[2])
            {
            case 1:
                eigenval0 = eigenval[0];
                for (int i=0; i < 3; i++) eigenvec0[i] = eigenvec[i][0];
        ...
        break;
    case 0: // eigenval[1] > eigenval[0]
        switch (eigenval[2] >= eigenval[1])
        {
        case 1: // eigenval[2] >= eigenval[1] > eigenval[0]
            eigenval0 = eigenval[2];
            for (int i=0; i < 3; i++) eigenvec0[i] = eigenvec[i][2];
            eigenval1 = eigenval[1];
            for (int i=0; i < 3; i++) eigenvec1[i] = eigenvec[i][1];
            ...
            break;
        case 0: // eigenval[1] >= max{ eigenval[0], eigenval[2] }
...
#ifdef __DEBUG_RMSD__
    cout << "    [" << U0[0] << " " << U1[0] << " " << U2[0] << "]" << endl;
    cout << "U = [" << U0[1] << " " << U1[1] << " " << U2[1] << "]" << endl;
    cout << "    [" << U0[2] << " " << U1[2] << " " << U2[2] << "]" << endl;
    cout << endl;
    cout << "    [" << V0[0] << " " << V1[0] << " " << V2[0] << "]" << endl;
    cout << "V = [" << V0[1] << " " << V1[1] << " " << V2[1] << "]" << endl;
    cout << "    [" << V0[2] << " " << V1[2] << " " << V2[2] << "]" << endl;
    cout << endl;
#endif
...
double RMSD(double * coords1, double * coords2, int n)
{
    /*************************************************************************
     * Compute the centroids of coords1 and coords2 respectively             *
     *************************************************************************/
...
void rotate(double * coords, int n, double R[3][3], double * result)
{
    for (int m=0; m < n; m++)
    {
        int m3 = 3*m;
...
double fast_rmsd(double * coords1, double * coords2, int n)
{
    /*************************************************************************
     * Compute the centroids of coords1 and coords2 respectively             *
     *************************************************************************/
