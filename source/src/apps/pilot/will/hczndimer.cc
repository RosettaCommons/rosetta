...
void dock(Pose init, std::string const & fn) {
  ...
  for(Size ih = 230; ih <= hits.size(); ++ih) {
    ...
    for(Size jh = ih+1; jh <= hits.size(); ++jh) {
      ...
      Size nh=0,nc=0,nd=0;
      if(hi.itype==CYS ) nc++; if(hi.jtype==CYS ) nc++; if(hj.itype==CYS ) nc++; if(hj.jtype==CYS ) nc++;
      if(hi.itype==HIS1) nh++; if(hi.jtype==HIS1) nh++; if(hj.itype==HIS1) nh++; if(hj.jtype==HIS1) nh++;
      if(hi.itype==HIS2) nh++; if(hi.jtype==HIS2) nh++; if(hj.itype==HIS2) nh++; if(hj.jtype==HIS2) nh++;
      if(hi.itype==ASP1) nd++; if(hi.jtype==ASP1) nd++; if(hj.itype==ASP1) nd++; if(hj.jtype==ASP1) nd++;
      if(hi.itype==ASP2) nd++; if(hi.jtype==ASP2) nd++; if(hj.itype==ASP2) nd++; if(hj.jtype==ASP2) nd++;
      if(hi.itype==ASP3) nd++; if(hi.jtype==ASP3) nd++; if(hj.itype==ASP3) nd++; if(hj.jtype==ASP3) nd++;
      // if(nc+nd!=2) continue;
      // if(nd > 2) continue;
