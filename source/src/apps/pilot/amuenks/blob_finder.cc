...
OPT_1GRP_KEY(String, denstools, search_metal)
OPT_1GRP_KEY(Boolean, denstools, score_ligand)
...
densityTools()
{
	...
	std::string search_metal = option[ denstools::search_metal ]();
	if (search_metal != "" and !userpose) {
		utility_exit_with_message("if search_metal is enabled, an input pose must be provided with -s");
	}
	...
	if ( userpose ) {
		...
		if (ligand_finder) {
			maskcut = option[ denstools::mask_cutoff ]();//0.5;//4e-2;
		}

		core::scoring::electron_density::getDensityMap().calcRhoC( pose, mask_resolution, rhoC, rhoMask, -1, 600, maskcut );
	}
	...
	TR.Trace << "* load input model" << std::endl;
	if ( userpose ) {
	...
	//score ligand_residue
	if ( option[ denstools::score_ligand ]() ) {
		...
		lig_resno = fullpose.total_residue();
		TR << "Ligand is " << fullpose.residue(lig_resno).name() << std::endl;
		core::conformation::Residue resi = fullpose.residue(lig_resno);
		//}
		/*else {
			lig_resno = fullpose.total_residue();
			TR << "Ligand is " << fullpose.residue(lig_resno).name() << std::endl;
		...
		core::Real dens_cc = core::scoring::electron_density::getDensityMap().matchRes(lig_resno, resi, fullpose, nullptr, false);
		TR << "Lig dens cc: " << dens_cc << std::endl;
	}
	...
	if (ligand_finder) {
		...
		find_unexplained_density( rhoO, rhoMask, points, scores, zscore_cutoff );


		std::ofstream ofs ("ligands.pdb", std::ofstream::out);
	...
	/*if (search_metal != "" ) {
		//core::import_pose::pose_from_file( fullpose, pdbfile , core::import_pose::PDB_file);
		//core::pose::PoseOP pose_tmp( &fullpose );
		core::pose::PoseOP pose_tmp = core::import_pose::pose_from_file( pdbfile, false, core::import_pose::PDB_file );
		core::Real dens_min = 100.0;
		core::Real dens_max = -100.0;
		for ( int z=1; z<=(int)rhoO.u3(); ++z ) {
			for ( int y=1; y<=(int)rhoO.u2(); ++y ) {
				for ( int x=1; x<=(int)rhoO.u1(); ++x ) {

					if ( rhoO(x,y,z) < dens_min ) { dens_min = rhoO(x,y,z); }
                                        if ( rhoO(x,y,z) > dens_max ) { dens_max = rhoO(x,y,z); }
					
				}
			}
		}

		std::ofstream ofs ("metals_" + search_metal + ".pdb", std::ofstream::out);
		int i = 1;
		protocols::ligand_docking::ga_ligand_dock::GALigandDock Dock;
		for ( int z=1; z<=(int)rhoO.u3(); ++z ) {
			for ( int y=1; y<=(int)rhoO.u2(); ++y ) {
				for ( int x=1; x<=(int)rhoO.u1(); ++x ) {
					core::Real metal_score = 1.0;
					...
					metal_score = Dock.calculate_metal_score( pose_tmp, x_cart, rhoO(x,y,x), dens_min, dens_max, search_metal);
					if ( metal_score != 1.0 && metal_score < -1.0) {
						//TR << "metal score: " << metal_score << std::endl;
						using ObjexxFCL::format::F;
						using ObjexxFCL::format::I;

						ofs << "ATOM  "<<I(5,i%100000)<<" CA   CA  A"<<I(4,i%10000)<<"    "
							<<F(8,3,x_cart.x())<<F(8,3,x_cart.y())<<F(8,3,x_cart.z())
							<<F(6,2,1.0)<<F(6,2,metal_score)
							<<std::endl;
						i += 1;
	...
	if ( userpose ) {
		if ( perres ) {
			TR.Trace << "* get per-res stats" << std::endl;
...
main( int argc, char * argv [] )
{
	try {
		...
		NEW_OPT(denstools::search_metal, "score sites for a metal", "");
		NEW_OPT(denstools::score_ligand, "get a density score for a ligand", false);
