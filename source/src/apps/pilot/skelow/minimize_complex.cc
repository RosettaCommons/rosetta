...
void get_scores( core::pose::Pose & bps, scoring::ScoreFunctionOP & locsf, std::string const & tag1, std::string const & tag2, std::string const & input_pdb_name){

			//setup the unbound pose
			core::pose::Pose unbound_pose(bps, 1, bps.total_residue() -1);
			(*locsf)(unbound_pose);
			...
			bps.metric(hbond_calc_name,"all_Hbonds", tot_hb_mval);
			...
			bps.metric(packstat_calc_name,"total_packstat", tot_packstat_mval);
			...
			bps.metric(burunsat_calc_name,"all_bur_unsat_polars", tot_unsat_mval);
			...
			// print scores
			std::cout << "Interface_Scores:"<< tag1 << tag2 <<"	"<< input_pdb_name <<"	" << bound_energy <<"	" << Interface_Energy <<"	"<< Total_BSA <<"	"<< Interface_HB <<"	"<< Total_packstats <<"	"<< Interface_unsat << std::endl;
...
int main( int argc, char * argv [] ){
	...
	//	scoring::ScoreFunctionOP repack_scorefxn = getScoreFunction();
		...
		for (core::Size f=1; f <= basic::options::start_files().size(); f++) {
			...
			get_scores(bound_pose, scorefxn, tag, "pre_min", input_pdb_name);
			...
			if (!option[ score_only ]){
				...
				get_scores(bound_pose, scorefxn, tag, "post_min", input_pdb_name);
