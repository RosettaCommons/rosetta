...
static thread_local basic::Tracer tr( "abego_predict" );
...
char get_label(vector1 <Real> ss_pred_pos){
	...
	if((ss_pred_pos[1] >= ss_pred_pos[2]) && (ss_pred_pos[1] >= ss_pred_pos[3]) &&(ss_pred_pos[1] >= ss_pred_pos[4]) && (ss_pred_pos[1] >= ss_pred_pos[5]))
		...
		if((ss_pred_pos[2] >= ss_pred_pos[1]) && (ss_pred_pos[2] >= ss_pred_pos[3]) &&(ss_pred_pos[2] >= ss_pred_pos[4]) && (ss_pred_pos[2] >= ss_pred_pos[5]))
			label = 'B';
		else
			if((ss_pred_pos[3] >= ss_pred_pos[1]) && (ss_pred_pos[3] >= ss_pred_pos[2]) &&(ss_pred_pos[3] >= ss_pred_pos[4]) && (ss_pred_pos[3] >= ss_pred_pos[5]))
				label = 'E';
			else
				if((ss_pred_pos[4] >= ss_pred_pos[1]) && (ss_pred_pos[4] >= ss_pred_pos[2]) &&(ss_pred_pos[4] >= ss_pred_pos[3]) && (ss_pred_pos[4] >= ss_pred_pos[5]))
					label = 'G';
				else
					if((ss_pred_pos[5] >= ss_pred_pos[1]) && (ss_pred_pos[5] >= ss_pred_pos[2]) &&(ss_pred_pos[5] >= ss_pred_pos[3]) && (ss_pred_pos[5] >= ss_pred_pos[4]))
						label = 'O';
	return(label);
}
