...
mutate_residue_to_ala( Pose & pose, Size i ) {
	...
	core::chemical::ResidueTypeCOP ala_type = chemical::ResidueTypeFinder( *residue_type_set ).name3( "ALA" ).variants( variant_types ).get_representative_type();

	ResidueOP new_res = ResidueFactory::create_residue( *ala_type, res, pose.conformation() );
