...
typedef utility::pointer::owning_ptr< A3BHbsCreatorMover > A3BHbsCreatorMoverOP;
typedef utility::pointer::owning_ptr< A3BHbsCreatorMover const > A3BHbsCreatorMoverCOP;
...
)
{
	...
	*/
	...
	for ( Size i=1; i<=pose.total_residue(); ++i) {
		...
		if (chn == hbs_chain) { // correct chain to be truncated and prepped
			...
			if (pdb_res_num < final_res) {
				...
				A3BHbsPatcherOP hbs_patcher (new A3BHbsPatcher( i ) );
    ...
    if( option[ a3b_hbs_creator::final_mc ].value() )
	{
		...
		for( Size i = 1; i <= pose.total_residue(); ++i ) {
            if (i != ((unsigned) (option[a3b_hbs_creator::hbs_final_res].value()))) {
