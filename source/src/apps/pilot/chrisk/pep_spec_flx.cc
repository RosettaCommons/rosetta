...
)
{
	...
	while( n_prepended < n_prepend || n_appended < n_append ){
		Size this_prepend( static_cast< int >( RG.uniform() * ( n_prepend - n_prepended + 1 ) ) );
		Size this_append( static_cast< int >( RG.uniform() * ( n_append - n_appended + 1 ) ) );
		for( Size ii = 1; ii <= this_prepend; ++ii ){
			...
			if( option[ pep_spec::no_cen ] ){
				if( RG.uniform() < 0.5 ) chemical::make_sequence_change( pep_begin, chemical::AA( aa_from_oneletter_code( 'A' ) ), pose );
		...
		for( Size ii = 1; ii <= this_append; ++ii ){
			...
			pose.conformation().update_polymeric_connection( pep_end - 1 );
			if( option[ pep_spec::no_cen ] ){
				if( RG.uniform() < 0.5 ) chemical::make_sequence_change( pep_end, chemical::AA( aa_from_oneletter_code( 'A' ) ), pose );
