...
void do_tmalign( core::pose::Pose const &pose1, core::pose::Pose const &pose2, core::Real &tmscore) {
...
//		TR << pdbnamei << endl;
	...
	for ( Size i = 1; i <= PoseVec.size(); ++i ) {
			...
			TR << "i " << i << " j " << j << " "; 
			//boost::thread*  threadij = new boost::thread( &do_tmalign, boost::cref(*PoseVec[i]), boost::cref(*PoseVec[j]), boost::ref(sc_matrix(i,j)));
			//threads.add_thread(threadij);
			threads.create_thread( boost::bind( &do_tmalign, boost::cref(*PoseVec[i]), boost::cref(*PoseVec[j]), boost::ref(sc_matrix(i,j)) )) ; 
	...
	//TR << " use extras=boost_thread to compile boost version, seg fault, why? " << endl;
        for ( Size i = 1; i <= PoseVec.size(); ++i ) {
                for ( Size j = i+1 ; j <= PoseVec.size(); ++j ) {
                        do_tmalign(*PoseVec[i], *PoseVec[j], sc_matrix(i,j));
