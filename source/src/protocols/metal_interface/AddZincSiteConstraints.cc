...
AddZincSiteConstraints::add_constraints( pose::Pose & pose ) {
	...
	for( core::Size i(2); i <= msr_.size(); ++i ) { // zinc is 1
		...
		if ( resn == "CYS" ) { // CYS
			...
			distance_constraints_.push_back( new AtomPairConstraint( atom_ids[i], zinc_id, S_dist_func ) );
			...
			angle_constraints_.push_back( new AngleConstraint( pre_atom_ids[i], atom_ids[i], zinc_id, S_angle_func ) );
			...
			distance_constraints_.push_back( new AtomPairConstraint( atom_ids[i], zinc_id, N_dist_func ) );
			pose.add_constraint( distance_constraints_[ distance_constraints_.size() ] );

			TR << "Adding His angle_constraint " << pre_atom_ids[i] << " " << atom_ids[i] << " " << zinc_id << std::endl;
			angle_constraints_.push_back( new AngleConstraint( pre_atom_ids[i], atom_ids[i], zinc_id, N_angle_func ) );
			pose.add_constraint( angle_constraints_[ angle_constraints_.size() ] );
		}
		else if ( resn == "ASP" || resn == "GLU" ) { // ASP/GLU
			TR << "Adding Asp/Glu atom_pair_constraint " << atom_ids[i] << " " << zinc_id << std::endl;
			distance_constraints_.push_back( new AtomPairConstraint( atom_ids[i], zinc_id, O_dist_func ) );
			pose.add_constraint( distance_constraints_[ distance_constraints_.size() ] );

			TR << "Adding Asp/Glu angle_constraint " << pre_atom_ids[i] << " " << atom_ids[i] << " " << zinc_id << std::endl;
			angle_constraints_.push_back( new AngleConstraint( pre_atom_ids[i], atom_ids[i], zinc_id, O_angle_func ) );
			pose.add_constraint( angle_constraints_[ angle_constraints_.size() ] );
		}
	}
