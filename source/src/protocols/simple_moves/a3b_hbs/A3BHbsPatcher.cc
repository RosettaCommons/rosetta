...
/// @file protocols/simple_moves/hbs/A3BHbsPatcher.cc
...
void A3BHbsPatcher::apply( core::pose::Pose & pose )
{
	...
	if ( pose.residue(hbs_pre_pos_).has_variant_type(chemical::A3B_HBS_PRE) != 1)
	{
		...
		conformation::Residue replace_res_pre( restype_set->get_residue_type_with_variant_added(pre_base_type, chemical::A3B_HBS_PRE), true );
	...
	if ( pose.residue(hbs_post_pos_).has_variant_type(chemical::A3B_HBS_POST) != 1 ) {
		...
		//awatkins: add variant
		conformation::Residue replace_res_post( restype_set->get_residue_type_with_variant_added(post_base_type, chemical::A3B_HBS_POST), true );
