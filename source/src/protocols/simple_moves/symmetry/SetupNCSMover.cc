		...
		for (Size j=1; j<=src_i.size(); ++j) {
			...
			if (bb_) {
				...
				for ( Size k=1, k_end = pose.residue(resnum_src).mainchain_torsions().size(); k<= k_end; ++k ) {
					...
					// make the cst
					pose.add_constraint( scoring::constraints::ConstraintCOP( new DihedralPairConstraint( id_a1, id_a2, id_a3, id_a4, id_b1, id_b2, id_b3, id_b4, core::scoring::func::FuncOP( new core::scoring::func::TopOutFunc( wt_, 0.0, limit_ ) ) ) ) );
				}
			}
	  ...
	  // map residue ranges -> resid pairs (using PDBinfo if necessary)
	    ...
	    runtime_assert( srcD_i.size() % 2 == 0 );
			...
			id::AtomID id_ad1,id_ad2, id_bd1,id_bd2;
				...
				// make the cst
				pose.add_constraint( scoring::constraints::ConstraintCOP( new DistancePairConstraint( id_ad1, id_ad2, id_bd1, id_bd2, core::scoring::func::FuncOP( new core::scoring::func::HarmonicFunc( 0.0, sd_ ) ) ) ) ); // for Harmonic
