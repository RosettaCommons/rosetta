...
/// @file protocols/swa/rna/StepWiseRNA_PoseSetupFromCommandLine.cc
/// @brief
/// @detailed
...
static basic::Tracer TR( "protocols.swa.rna.StepWiseRNA_PoseSetupFromCommandLine" );

///////////////////////////////////////////////////////////////////////////
//
// All utility functions for setting up job parameters, poses, etc. for SWA
// and ERRASER. This used to all be in swa_rna_main, the main application!
//
///////////////////////////////////////////////////////////////////////////
namespace protocols {
namespace swa {
...
get_fixed_res( core::Size const nres ){
	...
	utility::vector1< core::Size > const fixed_res_list    = option[ OptionKeys::swa::rna::fixed_res ]();
	utility::vector1< core::Size > const minimize_res_list = option[ OptionKeys::swa::rna::minimize_res ]();
...
get_input_res( core::Size const nres, std::string const pose_num ){
	...
	if ( pose_num == "1" ){
		...
		missing_res_list = option[ OptionKeys::swa::rna::missing_res ]();
	} else if ( pose_num == "2" ){
		input_res_list = option[ OptionKeys::swa::input_res2 ]();
		missing_res_list = option[ OptionKeys::swa::rna::missing_res2 ]();
	} else{
...
get_silent_file_tags(){
	...
	if ( option[ OptionKeys::swa::rna::job_queue_ID ].user() && option[ OptionKeys::swa::rna::filter_output_filename ].user() ){

		output_title_text( "importing tag from filter_outfile", TR );
		bool combine_long_loop = option[ OptionKeys::swa::rna::combine_long_loop_mode ]();
		bool combine_helical = option[ OptionKeys::swa::rna::combine_helical_silent_file ]();
		runtime_assert( combine_long_loop || combine_helical );

		tags_from_filterer_outfile = true;

		std::string const filtered_tag_file = option[ OptionKeys::swa::rna::filter_output_filename ]();
...
setup_simple_full_length_rna_job_parameters(){
  ...
  using namespace core::chemical;
	...
	if ( !option[ OptionKeys::swa::rna::rmsd_res ].user() ) utility_exit_with_message( "Must supply rmsd_res!" );
	if ( !option[ OptionKeys::swa::rna::alignment_res ].user() ) utility_exit_with_message( "Must supply alignment_res!" );
	if ( !option[ OptionKeys::swa::rna::global_sample_res_list ].user() ) utility_exit_with_message( "Must supply global_sample_res_list!" );
	...
	job_parameters->set_output_extra_RMSDs( option[ OptionKeys::swa::rna::output_extra_RMSDs ]() );
	...
	if ( option[ OptionKeys::swa::rna::rmsd_res ].user() ){
		job_parameters->set_rmsd_res_list( option[ OptionKeys::swa::rna::rmsd_res ]() );
	} else {
		TR << "Warning! rmsd_res not specified. Assuming sample_res is rmsd_res" << std::endl;
		job_parameters->set_rmsd_res_list( option[ OptionKeys::swa::rna::sample_res ]() );
	}
 	job_parameters->set_gap_size( 0 );

	if ( option[ OptionKeys::swa::rna::native_alignment_res ].user() ){
		...
		job_parameters->set_native_alignment( option[ OptionKeys::swa::rna::native_alignment_res ]() );
		job_parameters->set_working_native_alignment( option[ OptionKeys::swa::rna::native_alignment_res ]() );
	}
	/////////////////////////////////////////////////////
	...
	utility::vector1< std::string > alignment_res_string_list = option[ OptionKeys::swa::rna::alignment_res ]();
	...
	job_parameters->set_global_sample_res_list(  option[ OptionKeys::swa::rna::global_sample_res_list ]() );
	job_parameters->set_force_syn_chi_res_list(  option[ OptionKeys::swa::rna::force_syn_chi_res_list]() );
	job_parameters->set_force_north_sugar_list(  option[ OptionKeys::swa::rna::force_north_sugar_list ]() );
	job_parameters->set_force_south_sugar_list(  option[ OptionKeys::swa::rna::force_south_sugar_list ]() );
	job_parameters->set_protonated_H1_adenosine_list( option[ OptionKeys::swa::rna::protonated_H1_adenosine_list ]() );
	...
	if ( option[ in::file::native ].user() ){
		...
		utility::vector1< core::Size > const native_virtual_res_list = option[ OptionKeys::swa::rna::native_virtual_res]();
...
setup_rna_job_parameters( bool check_for_previously_closed_cutpoint_with_input_pose /* = false */ ){
	...
	if ( !option[ OptionKeys::swa::rna::sample_res ].user() ) utility_exit_with_message( "Must supply sample_res!" );
	...
	StepWiseRNA_JobParametersSetup stepwise_rna_job_parameters_setup( option[ OptionKeys::swa::rna::sample_res ](), /*the first element of moving_res_list is the sampling_res*/
																																		full_sequence,
																																		get_input_res( nres, "1" ),
																																		get_input_res( nres, "2" ),
																																		option[ OptionKeys::full_model::cutpoint_open ](),
																																		cutpoint_closed_ );
	stepwise_rna_job_parameters_setup.set_simple_append_map( option[ OptionKeys::swa::rna::simple_append_map]() );
	stepwise_rna_job_parameters_setup.set_allow_fixed_res_at_moving_res( option[ OptionKeys::swa::rna::allow_fixed_res_at_moving_res ]() ); //Hacky just to get Hermann Duplex working. Need to called before set_fixed_res
	stepwise_rna_job_parameters_setup.set_fixed_res( get_fixed_res( nres ) );
	stepwise_rna_job_parameters_setup.set_terminal_res( option[ OptionKeys::swa::rna::terminal_res ]() );

	if ( option[ OptionKeys::swa::rna::rmsd_res ].user() ){
		stepwise_rna_job_parameters_setup.set_rmsd_res_list( option[ OptionKeys::swa::rna::rmsd_res ]() );
	} else {
		TR << "Warning! rmsd_res not specified. Assuming sample_res is rmsd_res" << std::endl;
		stepwise_rna_job_parameters_setup.set_rmsd_res_list( option[ OptionKeys::swa::rna::sample_res ]() );
	}
	...
	stepwise_rna_job_parameters_setup.set_jump_point_pair_list( option[ OptionKeys::swa::rna::jump_point_pairs ]() ); //Important!: Need to be called after set_fixed_res
	stepwise_rna_job_parameters_setup.set_force_user_defined_jumps( option[ OptionKeys::swa::rna::force_user_defined_jumps ]() );
	...
	stepwise_rna_job_parameters_setup.set_alignment_res( option[ OptionKeys::swa::rna::alignment_res ]() );
	stepwise_rna_job_parameters_setup.set_filter_user_alignment_res( option[ OptionKeys::swa::rna::filter_user_alignment_res ]() );
	stepwise_rna_job_parameters_setup.set_native_alignment_res( option[ OptionKeys::swa::rna::native_alignment_res ]() );

	stepwise_rna_job_parameters_setup.set_global_sample_res_list( option[ OptionKeys::swa::rna::global_sample_res_list ]() ); //March 20, 2011

	stepwise_rna_job_parameters_setup.set_force_syn_chi_res_list( option[ OptionKeys::swa::rna::force_syn_chi_res_list]() ); //April 29, 2011
	stepwise_rna_job_parameters_setup.set_force_north_sugar_list( option[ OptionKeys::swa::rna::force_north_sugar_list ]() ); //April 29, 2011
	stepwise_rna_job_parameters_setup.set_force_south_sugar_list( option[ OptionKeys::swa::rna::force_south_sugar_list ]() ); //April 29, 2011
	stepwise_rna_job_parameters_setup.set_protonated_H1_adenosine_list( option[ OptionKeys::swa::rna::protonated_H1_adenosine_list ]() ); //May 02, 2011

	stepwise_rna_job_parameters_setup.set_allow_chain_boundary_jump_partner_right_at_fixed_BP( option[ OptionKeys::swa::rna::allow_chain_boundary_jump_partner_right_at_fixed_BP ]() ); //Hacky just to get Square RNA working.
...
//	StepWiseRNA_PoseSetup stepwise_rna_pose_setup( pdb_tags, silent_files_in, job_parameters);
	...
	stepwise_rna_pose_setup->set_virtual_res( option[ basic::options::OptionKeys::swa::rna::virtual_res ]() );
...
get_tag_and_silent_file_for_struct( std::string & swa_silent_file,
	...
	if ( multiple_shots ){
		runtime_assert( option[ OptionKeys::swa::rna::choose_random ]() );
