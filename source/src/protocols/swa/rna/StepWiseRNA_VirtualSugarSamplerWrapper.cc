...
/// @file protocols/swa/rna/StepWiseRNA_VirtualSugarSamplerWrapper.cc
/// @brief
/// @detailed
...
static basic::Tracer TR( "protocols.swa.rna.StepWiseRNA_VirtualSugarSamplerWrapper" );
...
//Constructor
StepWiseRNA_VirtualSugarSamplerWrapper::StepWiseRNA_VirtualSugarSamplerWrapper( StepWiseRNA_JobParametersCOP & job_parameters ):
...
//Destructor
StepWiseRNA_VirtualSugarSamplerWrapper::~StepWiseRNA_VirtualSugarSamplerWrapper()
{}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool
StepWiseRNA_VirtualSugarSamplerWrapper::do_sugar_sampling( pose::Pose & viewer_pose, SugarModeling & sugar_modeling, std::string const name ){
...
////New June 12, 2011/////
bool
StepWiseRNA_VirtualSugarSamplerWrapper::is_current_sugar_virtual( core::pose::Pose const & pose ) {
...
bool
StepWiseRNA_VirtualSugarSamplerWrapper::is_five_prime_chain_break_sugar_virtual( core::pose::Pose const & pose ) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool
StepWiseRNA_VirtualSugarSamplerWrapper::is_three_prime_chain_break_sugar_virtual( core::pose::Pose const & pose ) {
	if ( gap_size_ != 0 ) return false;
	// Make sure to not over count number of virtual_sugar-virtual_bulge
	// pairs to be build!
...
////////////////////////////////////////////////////////////////////////////////////
bool
StepWiseRNA_VirtualSugarSamplerWrapper::sampling_sugar() const{
...
////////////////////////////////////////////////////////////////////////////////////
void
StepWiseRNA_VirtualSugarSamplerWrapper::prepare_from_prior_sampled_sugar_jobs( pose::Pose const & pose,
																																							 utility::vector1< PoseOP > & pose_data_list ) {

	if ( anchor_sugar_modeling_.pose_list.size()         == 0 &&
			 current_sugar_modeling_.pose_list.size()           == 0 &&
			 five_prime_chain_break_sugar_modeling_.pose_list.size()  == 0 &&
			 three_prime_chain_break_sugar_modeling_.pose_list.size() == 0 ){
		utility_exit_with_message( "pose_data_list is empty for all 4 possible virtual sugar!" );
	}

	Size count = 0;

	for ( Size anchor_sugar_ID = 1; anchor_sugar_ID <= anchor_sugar_modeling_.pose_list.size() || anchor_sugar_ID == 1; anchor_sugar_ID++ ){
		for ( Size current_sugar_ID = 1; current_sugar_ID <= current_sugar_modeling_.pose_list.size() || current_sugar_ID == 1; current_sugar_ID++ ){
			for ( Size five_prime_chain_break_sugar_ID = 1; five_prime_chain_break_sugar_ID <= five_prime_chain_break_sugar_modeling_.pose_list.size() || five_prime_chain_break_sugar_ID == 1; five_prime_chain_break_sugar_ID++ ){
				for ( Size three_prime_chain_break_sugar_ID = 1; three_prime_chain_break_sugar_ID <= three_prime_chain_break_sugar_modeling_.pose_list.size() || three_prime_chain_break_sugar_ID == 1; three_prime_chain_break_sugar_ID++ ){
					...
					if ( anchor_sugar_modeling_.pose_list.size() > 0 ) {
						tag_into_pose( start_pose,   tag_from_pose(start_pose) + tag_from_pose( *anchor_sugar_modeling_.pose_list[anchor_sugar_ID] ) );
						copy_bulge_res_and_sugar_torsion( anchor_sugar_modeling_, start_pose, ( *anchor_sugar_modeling_.pose_list[anchor_sugar_ID] ), true /*instantiate_sugar*/ );
					} else{
						tag_into_pose( start_pose, tag_from_pose( start_pose ) + "_null" );
					}

					if ( current_sugar_modeling_.pose_list.size() > 0 ) {
						tag_into_pose( start_pose,   tag_from_pose(start_pose) + tag_from_pose( *current_sugar_modeling_.pose_list[current_sugar_ID] ) );
						copy_bulge_res_and_sugar_torsion( current_sugar_modeling_, start_pose, ( *current_sugar_modeling_.pose_list[current_sugar_ID] ), true /*instantiate_sugar*/ );
					} else{
						tag_into_pose( start_pose, tag_from_pose( start_pose ) + "_null" );
					}

					if ( five_prime_chain_break_sugar_modeling_.pose_list.size() > 0 ){
						tag_into_pose( start_pose,   tag_from_pose(start_pose) + tag_from_pose( *five_prime_chain_break_sugar_modeling_.pose_list[five_prime_chain_break_sugar_ID] ) );
						copy_bulge_res_and_sugar_torsion( five_prime_chain_break_sugar_modeling_, start_pose, ( *five_prime_chain_break_sugar_modeling_.pose_list[five_prime_chain_break_sugar_ID] ),  true /*instantiate_sugar*/ );
					} else{
						tag_into_pose( start_pose, tag_from_pose( start_pose ) + "_null" );
					}

					if ( three_prime_chain_break_sugar_modeling_.pose_list.size() > 0 ){
						tag_into_pose( start_pose,   tag_from_pose(start_pose) + tag_from_pose( *three_prime_chain_break_sugar_modeling_.pose_list[three_prime_chain_break_sugar_ID] ) );
						copy_bulge_res_and_sugar_torsion( three_prime_chain_break_sugar_modeling_, start_pose, ( *three_prime_chain_break_sugar_modeling_.pose_list[three_prime_chain_break_sugar_ID] ),  true /*instantiate_sugar*/ );
					} else{
						tag_into_pose( start_pose, tag_from_pose( start_pose ) + "_null" );
					}

					pose_data_list.push_back( start_pose_data );
				}
			}
		}
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool
StepWiseRNA_VirtualSugarSamplerWrapper::initialize_parameters( pose::Pose const & pose ){
...
////////////////////////////////////////////////////////////////////////////////////////////////////
bool
StepWiseRNA_VirtualSugarSamplerWrapper::do_consistency_checks(){
	...
	runtime_assert( !do_not_sample_multiple_virtual_sugar_ ||  !sample_ONLY_multiple_virtual_sugar_ );

	// this was just for testing.
	if ( do_not_sample_multiple_virtual_sugar_ &&  num_virtual_sugar_ > 1 ) return false;
...
/////////////////////
SugarModeling const &
StepWiseRNA_VirtualSugarSamplerWrapper::anchor_sugar_modeling(){ return anchor_sugar_modeling_; }

/////////////////////
void
StepWiseRNA_VirtualSugarSamplerWrapper::set_scorefxn( core::scoring::ScoreFunctionOP const & scorefxn ){ scorefxn_ = scorefxn; }

/////////////////////
std::string
StepWiseRNA_VirtualSugarSamplerWrapper::get_name() const {
	return "StepWiseRNA_VirtualSugarSamplerWrapper";
}
