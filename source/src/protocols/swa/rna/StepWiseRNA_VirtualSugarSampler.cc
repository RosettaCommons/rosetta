...
/// @file StepWiseRNA_VitualSugarSampler
...
floating_base_chain_closure_setup( utility::vector1< PoseOP > const & input_pose_data_list,
	...
	RNA_NucleosideRotamer sampler( sugar_modeling.moving_res,
			sugar_modeling.moving_res_pucker_state,
			sugar_modeling.moving_res_base_state );
	...
	for ( Size n = 1; n <= input_pose_data_list.size(); n++ ) {
		...
		for ( sampler.reset(); sampler.not_end(); ++sampler ) {
			...
			pose::Pose pose_with_sugar = ( *input_pose_data );//Hard copy
			pose_data.starting_fold_tree = pose_with_sugar.fold_tree();
			pose_data.starting_cst_set_OP =
					pose_with_sugar.constraint_set()->clone();
			assert( pose_data.starting_cst_set_OP );

			( *atr_rep_screening_scorefxn )( pose_with_sugar );
			Real const without_sugar_rep_score = atr_rep_screening_scorefxn->
					get_weight( fa_rep ) *
					pose_with_sugar.energies().total_energies()[ scoring::fa_rep ];

			pose::remove_variant_type_from_pose_residue( pose_with_sugar,
					"VIRTUAL_RIBOSE", sugar_modeling.moving_res );

			add_harmonic_chain_break_constraint( pose_with_sugar,
																					sugar_modeling.five_prime_chain_break );

			setup_chain_break_jump_point( pose_with_sugar, sugar_modeling.moving_res,
					sugar_modeling.reference_res, sugar_modeling.five_prime_chain_break,
					verbose );

			sampler.apply( pose_with_sugar );
			...
			if ( do_minimize == true ){
				...
				TR << "--------------START Creating rescaled one_tenth_sugar_score_fxn_without_ch_bond--------------" << std::endl;
				core::scoring::ScoreFunctionOP rescaled_sugar_score_fxn_without_ch_bond = rescale_scorefxn( sugar_scorefxn_without_ch_bond, 0.1 );
				TR << "--------------FINISH Creating rescaled one_tenth_sugar_score_fxn_without_ch_bond--------------" << std::endl;
				/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				if ( verbose ) TR << "pose.fold_tree().num_jump() = " << pose_with_sugar.fold_tree().num_jump() << std::endl;

				Size bulge_jump_1, bulge_jump_2, bulge_cutpoint;
				if ( sugar_modeling.moving_res > sugar_modeling.reference_res ){ //consistency check (from setup_chain_break_jump_point function)
					bulge_jump_1 = sugar_modeling.reference_res;
					bulge_jump_2 = sugar_modeling.moving_res;
					bulge_cutpoint = sugar_modeling.moving_res - 1;
				} else{
					bulge_jump_1 = sugar_modeling.moving_res;
					bulge_jump_2 = sugar_modeling.reference_res;
					bulge_cutpoint = sugar_modeling.moving_res;
				}
				...
				//	core::scoring::constraints::add_coordinate_constraints( pose_with_sugar );//crap I left this on for May_28_1CSL_SYN_CHI_floating_base_use_May_18_data run!

				viewer_pose = pose_with_sugar;
				TR << "removing sugar clashes pose # " << n << " sugar_rotamer # " << count << " " << std::endl;
				...
				// MINIMIZER OFF -- DO NOT CHECK IN!
				//if ( false ) {
				minimize_with_constraints( viewer_pose, mm, rescaled_sugar_score_fxn_without_ch_bond, options_armijo ); //Add this round on Sept 20, 2011, Switch to armijo on Sept 21, 2011
				minimizer.run( viewer_pose, mm, *( rescaled_sugar_score_fxn_without_ch_bond ), options_armijo );         //Add this round on Sept 20, 2011, Switch to armijo on Sept 21, 2011
				////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				minimize_with_constraints( viewer_pose, mm, sugar_scorefxn_without_ch_bond, options_standard );
				minimizer.run( viewer_pose, mm, *( sugar_scorefxn_without_ch_bond ), options_standard );

				minimize_with_constraints( viewer_pose, mm, sugar_scorefxn, options_standard );
				minimizer.run( viewer_pose, mm, *( sugar_scorefxn ), options_standard );
				//}

				pose_with_sugar = viewer_pose;
			}

			//testing
			//pose_with_sugar.dump_pdb( "pose_with_sugar_after_minimize_"+ string_of(count) + ".pdb"  );


			////////////////////////////////////////////Screens/////////////////////////////////////////////
			//OK check that with this sugar, the chain can be theoretically closed..
			std::string const moving_atom_name = ( sugar_modeling.is_prepend ) ? "O3'" : " C5'";
			std::string const reference_atom_name = ( sugar_modeling.is_prepend ) ? " C5'" : "O3'";
			Distance O3i_C5iplus2_distance = ( pose_with_sugar.residue( sugar_modeling.moving_res ).xyz( moving_atom_name ) - pose_with_sugar.residue( sugar_modeling.reference_res ).xyz( reference_atom_name ) ).length();

			( *atr_rep_screening_scorefxn )( pose_with_sugar );
			pose_data.base_rep_score = atr_rep_screening_scorefxn->get_weight( fa_rep ) * pose_with_sugar.energies().total_energies()[ scoring::fa_rep ];

			TR << "tag = " << pose_data.tag << " with_sugar_rep = " << pose_data.base_rep_score << " without_sugar_rep_score = " << without_sugar_rep_score << " O3i_C5iplus2_dist = " << O3i_C5iplus2_distance;
			...
			if ( ( pose_data.base_rep_score - without_sugar_rep_score ) > 10 ){
			...
			remove_virtual_rna_residue_variant_type( pose_with_sugar, sugar_modeling.bulge_res );
			pose::add_variant_type_to_pose_residue( pose_with_sugar, "VIRTUAL_PHOSPHATE", sugar_modeling.five_prime_chain_break + 1 );

			pose_data.pose_OP = new pose::Pose;
			( *pose_data.pose_OP ) = pose_with_sugar;
...
floating_base_chain_closure_sampling( utility::vector1< FB_Pose_Data > & pose_data_list,
	...
	Size const bulge_suite = sugar_modeling.bulge_suite;
	Size const bulge_rsd = sugar_modeling.bulge_res;
	...
	for ( sampler.reset(); sampler.not_end(); ++sampler ) {
		...
		if ( !VDW_bin_screener->VDW_rep_screen( screening_pose, sugar_modeling.bulge_res ) ) continue;
		...
		for ( Size n = 1; n <= pose_data_list.size(); n++ ){
			...
			if ( sugar_modeling.is_prepend ){
				if ( !check_chain_closable_floating_base( current_pose,
						screening_pose, sugar_modeling.five_prime_chain_break, 0 ) )
						continue;
			} else {
				if ( check_chain_closable_floating_base( screening_pose,
						current_pose, sugar_modeling.five_prime_chain_break, 0 ) )
						continue;
			}
			...
			if ( !fast_full_atom_VDW_repulsion_screen( current_pose,
					sugar_modeling.bulge_res, sugar_modeling.moving_res,
					sugar_modeling.is_prepend ) ) continue;
...
floating_base_chain_closure_post_process( utility::vector1< FB_Pose_Data > & pose_data_list,
		...
		mm.set( TorsionID( sugar_modeling.bulge_res, id::CHI, 2 ), true ); //nu2
		mm.set( TorsionID( sugar_modeling.bulge_res, id::CHI, 3 ), true ); //nu1
		...
		for ( Size n = 1; n <= pose_data_list.size(); n++ ){
			...
			if ( rm_chain_break_jump_point ) remove_chain_break_jump_point( viewer_pose, sugar_modeling.five_prime_chain_break, pose_data_list[n].starting_fold_tree );

			pose::remove_variant_type_from_pose_residue( viewer_pose, "VIRTUAL_PHOSPHATE",  sugar_modeling.five_prime_chain_break + 1 );
			apply_virtual_rna_residue_variant_type( viewer_pose, sugar_modeling.bulge_res );
...
minimize_all_sampled_floating_bases( core::pose::Pose & viewer_pose,
																		...
																		bool const virtual_sugar_is_from_prior_step ){
	...
	for ( Size n = 1; n <= modeling_list.size(); n++ ){
		...
		SugarModeling const & sugar_modeling = modeling_list[n];
	...
	for ( Size n = 1; n <= pose_data_list.size(); n++ ){
		...
		if ( virtual_sugar_is_from_prior_step ){ //Virtualize the other partition since it doesn't exist in prior step!
...
is_sugar_virtual( core::pose::Pose const & pose, core::Size const sugar_res, core::Size const bulge_res ){
	...
	if ( ( sugar_res + 1 ) != bulge_res && ( sugar_res - 1 ) != bulge_res ) {
		TR << "sugar_res = " << sugar_res << " bulge_res = " << bulge_res << std::endl;
	...
	if ( pose.residue( sugar_res ).has_variant_type( "VIRTUAL_RIBOSE" ) ) {
		...
		runtime_assert ( pose.residue( bulge_res ).has_variant_type( "VIRTUAL_RNA_RESIDUE" ) );
...
sample_virtual_sugar_and_bulge_and_close_chain(
	...
	output_title_text( "Enter sample_virtual_sugar_and_bulge_and_close_chain()", TR );
	clock_t const time_start( clock() );

	pose::Pose const viewer_pose_copy = viewer_pose; //BACKUP

	sugar_modeling.check_compatibility( viewer_pose.total_residue() );
	...
	if ( virtual_sugar_is_from_prior_step ){ //Virtualize the other partition since it doesn't exist in prior step!
		...
		bool const bulge_res_partition_value = partition_definition( sugar_modeling.bulge_res );

		if ( bulge_res_partition_value != partition_definition( sugar_modeling.moving_res ) ){ //Check that these three nts are in the same paritition!
			utility_exit_with_message( "bulge_res_partition_value != partition_definition( sugar_modeling.moving_res )" );
		}

		if ( bulge_res_partition_value != partition_definition( sugar_modeling.reference_res ) ){ //Check that these three nts are in the same paritition!
			utility_exit_with_message( "bulge_res_partition_value != partition_definition( sugar_modeling.reference_res )" );
		}
	...
	reference_stub.v = core::chemical::rna::get_rna_base_centroid( input_pose.residue( sugar_modeling.reference_res ), true );
	reference_stub.M = core::chemical::rna::get_rna_base_coordinate_system( input_pose.residue( sugar_modeling.reference_res ), reference_stub.v );

	utility::vector1 < core::Size > ignore_res_list;
	ignore_res_list.push_back( sugar_modeling.moving_res );
	ignore_res_list.push_back( sugar_modeling.bulge_res );
	ignore_res_list.push_back( sugar_modeling.reference_res );

	screener::StepWiseRNA_VDW_BinScreenerOP anchor_floating_base_VDW_bin_screener = new screener::StepWiseRNA_VDW_BinScreener();
	...
	anchor_floating_base_VDW_bin_screener->create_VDW_screen_bin( input_pose, ignore_res_list, sugar_modeling.is_prepend, reference_stub.v, true /*verbose*/ );
	...
	floating_base_chain_closure_setup( input_pose_data_list,
			sugar_modeling, atr_rep_screening_scorefxn, scorefxn, viewer_pose,
			do_minimize, use_phenix_geo );

	bool const chain_closure_sampling = true;

	if ( chain_closure_sampling ){
		floating_base_chain_closure_sampling( pose_data_list, sugar_modeling,
				chainbreak_scorefxn, atr_rep_screening_scorefxn,
				anchor_floating_base_VDW_bin_screener, integration_test_mode,
				use_phenix_geo );
	} else{
	...
	utility::vector1< PoseOP > final_pose_data_list = floating_base_chain_closure_post_process( pose_data_list, viewer_pose, sampling_scorefxn, sugar_modeling, rm_CB_JP_during_post_process );
	...
	TR << "Time in sample_virtual_sugar_and_bulge_and_close_chain(): " << static_cast< Real > ( clock() - time_start ) / CLOCKS_PER_SEC << std::endl;

	output_title_text( "Exit sample_virtual_sugar_and_bulge_and_close_chain()", TR );
...
enumerate_starting_pose_data_list( utility::vector1< PoseOP > & starting_pose_data_list,
	...
	while ( true ){
		...
		for ( Size n = 1; n <= FB_CC_JP_list.size(); n++ ){
			...
			SugarModeling const & curr_modeling = FB_CC_JP_list[n];
			Size const sugar_ID = sugar_ID_counter_list[n];

			tag_into_pose( start_pose, tag_from_pose( start_pose ) + tag_from_pose( *curr_modeling.PDL[sugar_ID] ) );

			pose::remove_variant_type_from_pose_residue( start_pose, "VIRTUAL_RIBOSE", curr_modeling.moving_res );
			copy_bulge_res_and_sugar_torsion( curr_modeling, start_pose, ( *curr_modeling.PDL[sugar_ID] ) );
...
setup_FB_CC_JP_list( pose::Pose const & pose, utility::vector1< std::string > const & sample_virtual_sugar_string_list, StepWiseRNA_JobParametersCOP & job_parameters ){

	using namespace ObjexxFCL;

	utility::vector1< SugarModeling > FB_CC_JP_list;

	std::string const & working_sequence = job_parameters->working_sequence();

	for ( Size n = 1; n <= sample_virtual_sugar_string_list.size(); n++ ){

		utility::vector1< std::string > const tokenize_list = tokenize( sample_virtual_sugar_string_list[n], "-" );
		...
		TR << "Case: " << sample_virtual_sugar_string_list[n];
		TR << " full_sugar_res = " << full_sugar_res << " full_bulge_res = " << full_bulge_res << " full_ref_res = " << full_ref_res;
		...
		if ( check_is_working_res( full_sugar_res, job_parameters ) ){

			Size const working_sugar_res = check_validity_and_get_working_res( full_sugar_res, job_parameters );

			bool const sugar_is_virtual = pose.residue( working_sugar_res ).has_variant_type( "VIRTUAL_RIBOSE" );

			TR << " | working_sugar_res = " << working_sugar_res;
			output_boolean( " sugar_is_virtual = ", sugar_is_virtual, TR );
			...
			if ( sugar_is_virtual ){
				...
				if ( is_prepend ){
					if ( working_sugar_res != ( working_bulge_res - 1 ) ) utility_exit_with_message( "prepend but working_sugar_res != ( working_bulge_res - 1 )" );
					if ( working_sugar_res != ( working_ref_res - 2 ) ) utility_exit_with_message( "prepend but working_sugar_res != ( working_ref_res - 2 )" );
				} else{
					if ( working_sugar_res != ( working_bulge_res + 1 ) ) utility_exit_with_message( "append  but working_sugar_res != ( working_bulge_res + 1 )" );
					if ( working_sugar_res != ( working_ref_res + 2 ) ) utility_exit_with_message( "prepend but working_sugar_res != ( working_ref_res + 2 )" );
				}

				//sugar_is_virtual=is_sugar_virtual(pose, working_sugar_res, working_bulge_res);
				...
				FB_CC_JP_list.push_back( curr_modeling );
			...
			TR << " | full_sugar_res is not a working res! ";
...
sample_user_specified_virtual_sugars(
	...
	utility::vector1< SugarModeling > FB_CC_JP_list = setup_FB_CC_JP_list( pose, sample_virtual_sugar_string_list, job_parameters );

	TR << "num_virtual_sugar = " << FB_CC_JP_list.size() << std::endl;

	if ( FB_CC_JP_list.size() == 0 ){
		TR << "no_virtual_sugar ( FB_CC_JP_list.size() == 0 ). EARLY RETURN/NO OUTPUT SILENT_FILE!" << std::endl;

		std::ofstream outfile;
		outfile.open( silent_file_out.c_str() ); //Opening the file with this command removes all prior content..
		outfile << "no_virtual_sugar ( FB_CC_JP_list.size() == 0 ).\n";
		outfile.flush();
		outfile.close();
	...
	for ( Size n = 1; n <= FB_CC_JP_list.size(); n++ ){
		...
		SugarModeling & curr_modeling = FB_CC_JP_list[n];

		curr_modeling.set_base_and_pucker_state( pose, job_parameters );

		curr_modeling.PDL = sample_virtual_sugar_and_bulge_and_close_chain(
				pose,  curr_modeling, "VIRT_RIBOSE_NUM_" + string_of( n ),	scorefxn,
				sampling_scorefxn, atr_rep_screening_scorefxn, chainbreak_scorefxn,
				job_parameters, integration_test_mode, use_phenix_geo,
				false /*virtual_sugar_is_from_prior_step*/ );
				//ACTUAL COMPUTATION OCCUR HERE!!


		std::sort( curr_modeling.PDL.begin(), curr_modeling.PDL.end(), sort_pose_by_score );

		if ( curr_modeling.PDL.size() == 0 ){
			TR << "Case n = " << n << " is_sugar_virt == True but curr_modeling.PDL.size() == 0. EARLY RETURN!" << std::endl;

			std::ofstream outfile;
			outfile.open( silent_file_out.c_str() ); //Opening the file with this command removes all prior content..
			outfile << "num_virtual_sugar != 0 but for one of the sampled virtual_sugar, curr_modeling.PDL.size() == 0.\n";
			outfile.flush();
			outfile.close();
			return;
		}
	...
	for ( Size n = 1; n <= starting_pose_data_list.size(); n++ ){
		...
		std::string starting_pose_tag = input_tag + "_sample_sugar" + tag_from_pose( *starting_pose_data_list[n]);
