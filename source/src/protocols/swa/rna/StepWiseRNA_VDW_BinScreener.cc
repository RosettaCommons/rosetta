...
static basic::Tracer TR( "protocols.swa.rna.StepWiseRNA_VDW_BinScreener" );
...
//////////////////////////////////////////////////////
  ...
  {}
	...
	StepWiseRNA_VDW_BinScreener::setup_using_working_pose(core::pose::Pose const & const_working_pose, StepWiseRNA_JobParametersCOP const & job_parameters ){
		...
		if(verbose) Output_title_text("Enter StepWiseRNA_VDW_BinScreener::setup_using_working_pose", TR );
		...
		if(verbose) Output_title_text("Exit StepWiseRNA_VDW_BinScreener::setup_using_working_pose", TR );
	...
	StepWiseRNA_VDW_BinScreener::get_matching_res_in_VDW_rep_screen_pose( core::pose::Pose const & VDW_rep_screen_pose,
		...
		Output_title_text("Enter StepWiseRNA_VDW_BinScreener::get_matching_res_in_VDW_rep_screen_pose", TR );
		...
		TR << "VDW_rep_ignore_matching_res_.size()=" << VDW_rep_ignore_matching_res_.size() << std::endl;
		for(Size n=1; n<=VDW_rep_ignore_matching_res_.size(); n++){
			TR << "VDW_rep_ignore_matching_res_[" << n << "]= " << VDW_rep_ignore_matching_res_[n] << std::endl;
		}

		if(VDW_rep_ignore_matching_res_.size()==1){
			if(VDW_rep_ignore_matching_res_[1]=="false"){
				TR << "VDW_rep_ignore_matching_res_[1]==\"false\", will NOT delete residues in VDW_rep_pose that exist in the working_pose" << std::endl;
		...
		Output_boolean("check_VDW_rep_no_match_res=", check_VDW_rep_no_match_res, TR ); TR << std::endl;

		core::Real const dist_cutoff=VDW_rep_alignment_RMSD_CUTOFF_;
		TR << "atom_atom_dist_cutoff= " << dist_cutoff << std::endl;
		...
		Output_seq_num_list("working_no_match_res_list= ", no_match_res_list, TR );
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	StepWiseRNA_VDW_BinScreener::align_VDW_rep_screen_pose(core::pose::Pose & VDW_rep_screen_pose,
		...
		for(Size n=1; n<=working_align_res.size(); n++){
			...
			if(rmsd>VDW_rep_alignment_RMSD_CUTOFF_ ){ //change on Sept 26, 2010..problem arise when use this in non-long-loop mode...
				TR << "rmsd= " << rmsd  << " >(" << VDW_rep_alignment_RMSD_CUTOFF_ << ") Angstrom between res " << res_num_1 << " of VDW_rep_screen_align_res and res " << res_num_2 << " of working_pose" << std::endl;
	...
	*/
	...
	StepWiseRNA_VDW_BinScreener::create_VDW_rep_screen_pose(VDW_RepScreeninfo & VDW_rep_screen_info, //This function update this class!
	  ...
	  using namespace core::chemical;
		...
		TR << "importing VDW_rep_screen_pose: " << VDW_rep_screen_info.pose_name << std::endl;
	...
	StepWiseRNA_VDW_BinScreener::FARFAR_setup_using_user_input_VDW_pose(utility::vector1< std::string > const & All_VDW_rep_screen_pose_info,
		...
		Output_title_text("Enter StepWiseRNA_VDW_BinScreener::FARFAR_setup_using_user_input_VDW_pose", TR );
		...
		for(Size n=1; n<=All_VDW_rep_screen_pose_info.size(); n+=3){
			...
			TR << "Adding VDW_screen_rep_pose_info #" << VDW_rep_screen_info.import_ID << ": " << VDW_rep_screen_info.input_string << std::endl;
	...
	StepWiseRNA_VDW_BinScreener::setup_using_user_input_VDW_pose(utility::vector1< std::string > const & All_VDW_rep_screen_pose_info,
	  ...
	  using namespace core::chemical;
		...
		Output_title_text("Enter StepWiseRNA_VDW_BinScreener::setup_using_user_input_VDW_pose", TR );
		...
		for(Size n=1; n<=All_VDW_rep_screen_pose_info.size(); n+=3){
			...
			TR << "Checking VDW_screen_rep_pose_info #" << VDW_rep_screen_info.import_ID << ": " << VDW_rep_screen_info.input_string << std::endl;
			...
			if(VDW_rep_screen_info.full_align_res.size()!=VDW_rep_screen_info.working_align_res.size()){
				TR << "Ignoring VDW_screen_rep_pose_info #" << VDW_rep_screen_info.import_ID << " since full_align_res.size()!=working_align_res.size()!";
				TR << " full_align_res.size()= " 		 << VDW_rep_screen_info.full_align_res.size();
				TR << " working_align_res.size()= " << VDW_rep_screen_info.working_align_res.size();
				TR << std::endl;
				continue;
			}
		...
		for(Size n=1; n<=VDW_rep_screen_info_list_.size(); n++){
			...
			TR << "Check if VDW screen_rep_pose_info #" << VDW_rep_screen_info.import_ID << ": " << VDW_rep_screen_info.input_string  << " is in root partition." << std::endl;
			...
			if(contain_moving_partition_res==false){
				TR << "VDW_screen_rep_pose_info #" << VDW_rep_screen_info.import_ID << " is in root partition! " << std::endl;
		...
		if(num_rep_screen_pose_info_in_root_partition==0){
			...
			if(working_fixed_res_in_root_partition.size()==0) {
				TR << "No VDW_screener_pose since working_fixed_res_in_root_partition.size()==0, probably build inward " << std::endl;
				user_inputted_VDW_screen_pose_=false;
				return;
			}else{
		...
		if(verbose) Output_title_text("Exit StepWiseRNA_VDW_BinScreener::setup_using_user_input_VDW_pose", TR );
	...
	StepWiseRNA_VDW_BinScreener::update_VDW_screen_bin(core::pose::Pose const & pose,
		...
		for(Size n=1; n<=pose.total_residue(); n++){
			...
			for(Size at=1; at<= rsd.natoms(); at++){ //include hydrogen atoms.
				...
				if(output_once==false){
					output_once=true;
					TR << "seq_num= " << n << " atom_name= " << rsd.type().atom_name(at) << " VDW_radius= " << VDW_radius;
					TR << " moving_atom_radius= " << moving_atom_radius << " clash_dist_cutoff= " << clash_dist_cutoff;
					TR << " max_binning_error= " << max_binning_error << " sum_radius " <<  sum_radius << std::endl;
					TR << "max_bin_offset= " << max_bin_offset << " min_bin_offset= " <<min_bin_offset << std::endl;
		...
		Output_seq_num_list("ignore_res_list=" ,ignore_res_list, TR );

		TR << "VIRTUAL_ATOM_LIST: " << std::endl;
		for(Size ii=1; ii<=virtual_atom_list.size(); ii++){
			TR << virtual_atom_list[ii].first << "-" << virtual_atom_list[ii].second << " ";
		}
		TR << std::endl;

		TR << "MOVING_PHOSPHATE_ATOM_LIST: " << std::endl;
		for(Size ii=1; ii<=moving_phosphate_atom_list.size(); ii++){
			TR << moving_phosphate_atom_list[ii].first << "-" << moving_phosphate_atom_list[ii].second << " ";
		}
		TR << std::endl;
	}
	...
	StepWiseRNA_VDW_BinScreener::create_VDW_screen_bin(utility::vector1< core::pose::Pose> const & pose_list,
		...
		TR << "------------------Enter create_pose_bin function------------------ " << std::endl;
		TR << "max_distance_= " << max_distance_ << std::endl;
		TR << "atom_bin_size_= " << atom_bin_size_ << std::endl;
		TR << "bin_min_= " << bin_min_ << std::endl;
		TR << "bin_max_= " << bin_max_ << std::endl;
		TR << "bin_offset_= " << bin_offset_ << std::endl;
		TR << "num_clash_atom_cutoff_= " << num_clash_atom_cutoff_ << std::endl;
		TR << "pose_list.size()= " << pose_list.size() << std::endl;
		Output_boolean("verbose=", verbose, TR ); TR << std::endl;
		Output_boolean("write_to_file_=", write_to_file_, TR ); TR << std::endl;
		Output_boolean("user_inputted_VDW_screen_pose_=", user_inputted_VDW_screen_pose_, TR ); TR << std::endl;
		Output_boolean("tolerate_off_range_atom_bin_=", tolerate_off_range_atom_bin_, TR ); TR << std::endl;
		/////////////////////////////////////////////
		...
		for(Size n=1; n<=pose_list.size(); n++){
			TR << "--------------Enter update_VDW_screen_bin() for pose #" << n << " (count only include VDW_poses in root partition!)--------------" << std::endl;
			update_VDW_screen_bin(pose_list[n], list_of_ignore_res_list[n], list_of_Is_prepend[n], outfile_act);
			TR << "--------------Exit  update_VDW_screen_bin() for pose #" << n << " (count only include VDW_poses in root partition!)--------------" << std::endl;
		}
		...
		TR << "VDW_screen_bin_: occupied_bin_count = " << occupied_bin_count << " out of total_bin_count = " << total_bin_count << std::endl;
		TR << "------------------Exit create_pose_bin function------------------ " << std::endl;
	...
	StepWiseRNA_VDW_BinScreener::VDW_rep_screen(core::pose::Pose const & screening_pose, //Warning..this pose coordinate is not update...use here for VIRTUAL atom screening.
		...
		for( Size n = 1; n <= xyz_list.size(); n++){
			...
			if(xyz_list[n].first.rsd()!=moving_res){
				TR << "xyz_list[n].first.rsd()= " << xyz_list[n].first.rsd() << " moving_res= " << moving_res << std::endl;
	...
	StepWiseRNA_VDW_BinScreener::VDW_rep_screen_with_act_pose(core::pose::Pose const & screening_pose,
		...
		if(VDW_rep_screen_with_physical_pose_verbose_){
			TR << "------------VDW_rep_screen_with_physical_pose_INFO------------" << std::endl;
			VDW_rep_screen_with_physical_pose_verbose_=false;
			TR << "physical_pose_clash_dist_cutoff_= " << physical_pose_clash_dist_cutoff_ << std::endl;
			TR << "num_clash_atom_cutoff_= " << num_clash_atom_cutoff_ << std::endl;
			TR << "VDW_rep_screen_info_list_.size()= " << VDW_rep_screen_info_list_.size() << std::endl;
			Output_seq_num_list("moving_res_list= ", moving_res_list, TR );
			TR << "--------------------------------------------------------------" << std::endl;
		}
	...
	StepWiseRNA_VDW_BinScreener::reference_xyz_consistency_check( numeric::xyzVector<core::Real> const & inputted_reference_xyz) const {
		...
		for(int n=0; n<=2; n++){
			if( ( (inputted_reference_xyz[n]-0.00001) > reference_xyz_[n] ) || ( (inputted_reference_xyz[n]+0.00001) < reference_xyz_[n]) ) {
				...
				TR << "inputted_reference_xyz= " << inputted_reference_xyz[0] << " " << inputted_reference_xyz[1] << " " << inputted_reference_xyz[2] << std::endl;
				TR << "reference_xyz_= " << reference_xyz_[0] << " " << reference_xyz_[1] << " " << reference_xyz_[2] << std::endl;
			}
		}
	}
 	...
 	StepWiseRNA_VDW_BinScreener::check_atom_bin_in_range(Atom_Bin const & atom_pos_bin){
			...
			if( !Is_atom_bin_in_range(atom_pos_bin) ){
				...
				if(num_atom_pos_bin_out_of_range_message_outputted_<=10){
					TR << "bin_max_*2= " << bin_max_*2 << std::endl;
					TR << " atom_pos_bin.x= " << atom_pos_bin.x << " atom_pos_bin.y= " << atom_pos_bin.y << " atom_pos_bin.z= " << atom_pos_bin.z << std::endl;
					TR << "atom_pos_bin out of range!" << std::endl;
					num_atom_pos_bin_out_of_range_message_outputted_++;
					TR << "num_atom_pos_bin_out_of_range_message_outputted_so_far= " << num_atom_pos_bin_out_of_range_message_outputted_ << std::endl;
				}
