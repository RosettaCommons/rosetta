...
DockIntoDensityMover::apply_multi( utility::vector1< core::pose::PoseOP > & poses) {
	...
	if ( passthrough_ ) {
		...
		for ( int i=1; i<=(int)poses.size(); ++i ) {
			...
			density_grid_search(
				...
				B_, nRsteps_, max_rot_per_trans_, delR_, cluster_radius_, laplacian_offset_,
				center_on_middle_ca_, convolute_single_residue_, symminfo_, native_, native_com_});
	...
	do_filter( results_refine, symminfo_, cluster_radius_ );
