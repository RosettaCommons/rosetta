...
namespace strand_assembly {
		...
		Size sheet_id_2)
	{
		...
		for (Size ii=0;	ii<vector_of_all_residues_in_sheet_1.size();	ii++)
		{
			...
			{
				Real distance = dssp_pose.residue(vector_of_all_residues_in_sheet_1[ii]).atom("CA").xyz().distance(dssp_pose.residue(vector_of_all_residues_in_sheet_2[jj]).atom("CA").xyz());
		...
		return min_dis;
		//return std::make_pair(min_dis, (sum_dis/cal_num));
	} //cal_min_dis_between_two_sheets_by_all_res
