...
#include <protocols/stepwise/legacy/sampling/rna/connection/StepWiseRNA_ConnectionSampler.hh>
...
static basic::Tracer TR( "protocols.stepwise.legacy.sampling.rna.connection.StepWiseRNA_ConnectionSampler" ) ;
...
//Constructor
StepWiseRNA_ConnectionSampler::StepWiseRNA_ConnectionSampler( stepwise::sampling::working_parameters::StepWiseWorkingParametersCOP & working_parameters ):
...
StepWiseRNA_ConnectionSampler::apply( core::pose::Pose & pose ){
	...
	stepwise::sampling::rna::output_title_text( "Enter StepWiseRNA_ConnectionSampler::floating_base_sampling", TR.Debug );
...
StepWiseRNA_ConnectionSampler::figure_out_reference_res( pose::Pose const & pose ){
	...
	rigid_body_sampling_ = ( rotamer_sampler::rigid_body::figure_out_reference_res_for_jump( pose, moving_res ) > 0 );
...
StepWiseRNA_ConnectionSampler::initialize_pose_level_screeners( pose::Pose & pose ) {
	...
	if ( rigid_body_sampling_ && virt_sugar_atr_rep_screen_ && options_->atr_rep_screen() ) {
	...
	for ( Size n = 1; n <= five_prime_chain_breaks_.size(); n++ ) {
		bool strict = rigid_body_sampling_ && cutpoints_closed_.has_value( five_prime_chain_breaks_[n] ) && (cutpoints_closed_.size() < 3);
...
StepWiseRNA_ConnectionSampler::initialize_poses_and_checkers( pose::Pose & pose  ){
	...
	bool const use_loose_rep_cutoff = ( kic_sampling_ || moving_partition_res_.size() > 1 );
...
StepWiseRNA_ConnectionSampler::get_num_pose_kept(){
	...
	if ( !rigid_body_sampling_ && working_parameters_ && working_parameters_->sample_both_sugar_base_rotamer() ) num_pose_kept *= 12;
...
StepWiseRNA_ConnectionSampler::initialize_moving_residue_pose_list( pose::Pose const & pose ){
	...
	if ( rigid_body_sampling_ && moving_partition_res_.size() == 1 ){ // single floating base [classic]
...
//////////////////////////////////////////////////////////////////
void
StepWiseRNA_ConnectionSampler::set_options( modeler_options::StepWiseModelerOptionsCOP options ){
	options_ = options;
}
...
StepWiseRNA_ConnectionSampler::presample_virtual_sugars( pose::Pose & pose ){
	using namespace sampling::rna::sugar;
