...
/// @file protocols/stepwise/legacy/sampling/rna/StepWiseRNA_PoseSetupFromCommandLine.cc
/// @brief
/// @detailed
...
static basic::Tracer TR( "protocols.stepwise.legacy.sampling.rna.StepWiseRNA_PoseSetupFromCommandLine" );
...
get_silent_file_tags(){
	...
	if ( option[ OptionKeys::stepwise::rna::job_queue_ID ].user() && option[ OptionKeys::stepwise::rna::filter_output_filename ].user() ){
		...
		stepwise::sampling::rna::output_title_text( "importing tag from filter_outfile", TR );
		...
		stepwise::sampling::rna::output_title_text( "", TR );
...
setup_simple_full_length_rna_working_parameters(){
  ...
  using namespace core::chemical;
	...
	stepwise::sampling::working_parameters::StepWiseWorkingParametersOP working_parameters = new stepwise::sampling::working_parameters::StepWiseWorkingParameters;
...
setup_pose_setup_class( stepwise::sampling::working_parameters::StepWiseWorkingParametersOP & working_parameters, bool const copy_DOF /*= true*/ ){
...
ensure_directory_for_out_silent_file_exists(){
	...
	if ( option[ out::file::silent ].user() ){
		...
		if ( outstream.fail() ){
			...
			#ifdef WIN32
				char * outdir;
			 	utility_exit_with_message( "protocols/stepwise/legacy/sampling/rna/StepWiseRNA_PoseSetupFromCommandLine.cc dirname is not implemented under Windows!" );
			#else
