...
static basic::Tracer TR( "protocols.stepwise.monte_carlo.RNA_O2PrimeMover" ) ;
  ...
  RNA_O2PrimeMover::apply( core::pose::Pose & pose, std::string & move_type )
	{
		...
		if ( sample_all_o2prime_ ){
			...
			std::cerr << "O2prime sampling is not yet properly worked out for align monte carlo case." << std::endl;
