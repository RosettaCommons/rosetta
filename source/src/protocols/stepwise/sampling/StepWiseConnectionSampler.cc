...
/// @file protocols/stepwise/sampling/StepWiseConnectionSampler.cc
/// @brief
/// @detailed
...
static basic::Tracer TR( "protocols.stepwise.sampling.StepWiseConnectionSampler" );
...
StepWiseConnectionSampler::StepWiseConnectionSampler( working_parameters::StepWiseWorkingParametersCOP & working_parameters ):
	...
	rigid_body_sampling_( false ),  // "useful_info" -- updated below
	reference_res_( 0 ),            // "useful_info" -- updated below
	kic_sampling_( false ),         // "useful_info" -- updated below
...
StepWiseConnectionSampler::initialize_useful_info( pose::Pose const & pose ){
	...
	kic_sampling_ = ( !rigid_body_sampling_ && options_->kic_sampling_if_relevant() && ( rna_cutpoints_closed_.size() > 0 ) );
}
...
StepWiseConnectionSampler::initialize_screeners( pose::Pose & pose )
{
	...
	screeners_.clear();
	if ( rigid_body_sampling_ ) initialize_residue_level_screeners( pose );
...
StepWiseConnectionSampler::initialize_pose_level_screeners( pose::Pose & pose ) {
	...
	if ( !rigid_body_sampling_ && base_centroid_checker_ && !protein_connection_ ){
	...
	bool const use_loose_rep_cutoff = ( kic_sampling_ || moving_partition_res_.size() > 1 /* is_internal */ );
	if ( rigid_body_sampling_ && virt_sugar_atr_rep_screen_ &&
	...
	for ( Size n = 1; n <= rna_five_prime_chain_breaks_.size(); n++ ) {
		bool strict = rigid_body_sampling_ && rna_cutpoints_closed_.has_value( rna_five_prime_chain_breaks_[n] ) && (rna_cutpoints_closed_.size() < 3);
	...
	if ( !rigid_body_sampling_ && !working_parameters_->rebuild_bulge_mode() &&
...
// also currently responsible for rna_chainbreaks_ & kic_sampling_ -- perhaps move this out.
bool
...
StepWiseConnectionSampler::initialize_protein_bond_sampler( pose::Pose const & pose ){
	...
	RotamerSamplerSizedOP sampler = get_basic_protein_sampler( pose, moving_res_list_,
																											working_parameters_, options_, input_streams_ );

	if ( protein_cutpoints_closed_.size() > 0 && options_->kic_sampling_if_relevant() ) {
...
StepWiseConnectionSampler::initialize_rna_bond_sampler( pose::Pose const & pose ){
	...
	RotamerSamplerBaseOP rotamer_sampler_ = rotamer_sampler::rna::setup_rotamer_sampler( pose, options_,
																																								working_parameters_, false /*build_pose_from_scratch_*/,
																																								kic_sampling_, (rna_cutpoints_closed_.size() > 0) );
...
void
StepWiseConnectionSampler::add_residue_alternative_set( rotamer_sampler::copy_dofs::ResidueAlternativeSet const & residue_alternative_set ){
	residue_alternative_sets_.push_back( residue_alternative_set );
}
