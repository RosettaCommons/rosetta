...
/// @file protocols/stepwise/sampling/working_parameters/StepWiseRNA_WorkingParametersUtil.cc
/// @brief
/// @detailed
...
static basic::Tracer TR( "protocols.stepwise.sampling.working_parameters.util" );
...
setup_working_parameters_for_swa( utility::vector1< Size > const & moving_res_list,
	...
	if ( native_pose ) {
		...
		for ( Size n = 1; n <= res_list.size(); n++ ) res_list_for_slicing.push_back( native_res_list.index( res_list[n] ) );
		protocols::stepwise::sampling::pdbslice( *working_native_pose, *native_pose, res_list_for_slicing );
	}
