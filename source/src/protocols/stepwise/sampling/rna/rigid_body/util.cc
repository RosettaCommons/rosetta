...
static basic::Tracer TR( "protocols.stepwise.sampling.rna.rigid_body.util" ) ;
...
get_euler_stub_bin( numeric::xyzVector< core::Real > const & centroid, rotamer_sampler::rigid_body::EulerAngles const & euler_angles ){
	...
	BaseBin base_bin;
	base_bin.centroid_x = int( centroid[0]/STANDARD_CENTROID_BIN_SIZE );
	base_bin.centroid_y = int( centroid[1]/STANDARD_CENTROID_BIN_SIZE );
	base_bin.centroid_z = int( centroid[2]/STANDARD_CENTROID_BIN_SIZE );

	base_bin.euler_alpha = int( euler_angles.alpha()/STANDARD_EULER_ANGLE_BIN_SIZE );
	base_bin.euler_gamma = int( euler_angles.gamma()/STANDARD_EULER_ANGLE_BIN_SIZE );
	base_bin.euler_z = int( euler_angles.z()/STANDARD_EULER_Z_BIN_SIZE );
...
setup_residue_at_origin_list(
	...
	utility::vector1< core::pose::PoseOP > pose_at_origin_list = setup_pose_at_origin_list( pose, moving_res, extra_chi, use_phenix_geo );
	for ( Size n = 1; n <= pose_at_origin_list.size(); n++ ) {
		core::conformation::ResidueOP rsd_at_origin = pose_at_origin_list[n]->residue( moving_res ).clone();
...
setup_pose_at_origin_list(
	...
	rotamer_sampler::rigid_body::set_to_origin( pose_at_origin, moving_res, false );
	return setup_pose_with_moving_residue_alternative_list( pose_at_origin, moving_res, extra_chi, use_phenix_geo );
}
