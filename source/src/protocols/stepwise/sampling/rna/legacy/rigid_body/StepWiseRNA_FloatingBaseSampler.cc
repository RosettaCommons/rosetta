...
/// @file protocols/stepwise/sampling/rna/legacy/rigid_body/StepWiseRNA_FloatingBaseSampler.cc
/// @brief
/// @detailed
...
static basic::Tracer TR( "protocols.stepwise.rna.StepWiseRNA_FloatingBaseSampler" ) ;
...
StepWiseRNA_FloatingBaseSampler::apply( core::pose::Pose & pose ){
	...
	for ( sampler_->reset(); sampler_->not_end(); ++(*sampler_) ){
		ntries++;
		if ( options_->choose_random() && ntries > max_ntries_ ) break;
		...
		if ( base_centroid_checker_ &&
				 ( !base_centroid_checker_->check_centroid_interaction( moving_res_base_stub, count_data_ ) ||
					 !base_centroid_checker_->check_that_terminal_res_are_unstacked() ) ) {
		...
		if ( anchor_sugar_modeling_.sample_sugar ){
			if ( !chain_closable_geometry_to_anchor_checker_->check_screen( anchor_sugar_modeling_.pose_list, moving_rsd_at_origin_list_, moving_res_base_stub, reference_res_ ) ) continue;
		} else {
			if ( !chain_closable_geometry_to_anchor_checker_->check_screen( *screening_pose_, moving_rsd_at_origin_list_, moving_res_base_stub, reference_res_ ) ) continue;
		}
		count_data_.chain_closable_geometry_count++;

		//		if ( close_chain_to_distal_ ){ // for consistency this should always be on! Rhiju Feb. 2014
		if ( chain_closable_geometry_to_distal_checker_ && !chain_closable_geometry_to_distal_checker_->check_screen( *screening_pose_, moving_rsd_at_origin_list_, moving_res_base_stub, chain_break_reference_res_ ) ) continue;
		//		}
		count_data_.chain_closable_geometry_count2++;

		//////////////////////////////////////////////////////////////////////////////////////////////////////
		// clash checks
		// Some trickiness with clashes to 3'-phosphate in case of chain closure -- parin has worked out these cases, fortunately.
		Residue const & screening_moving_rsd_at_origin = ( *screening_moving_rsd_at_origin_list_[ 1 /*5*/ /*magic number?*/ ] );
		if ( !VDW_bin_checker_->VDW_rep_screen( *screening_pose_, moving_res_, screening_moving_rsd_at_origin, moving_res_base_stub ) ) continue;
		...
		if ( ( user_input_VDW_bin_checker_->user_inputted_VDW_screen_pose() ) && ( !close_chain_to_distal_ ) && ( !is_internal_ )  &&
				 !user_input_VDW_bin_checker_->VDW_rep_screen( *screening_pose_, moving_res_, screening_moving_rsd_at_origin, moving_res_base_stub ) ) continue;
		...
		if ( native_rmsd_screen_ && get_native_pose() ){
			...
			if ( suite_rmsd( *get_native_pose(), *screening_pose_, moving_res_, is_prepend_, true /*ignore_virtual_atom*/ ) > ( options_->sampler_native_screen_rmsd_cutoff() ) ) continue;
			if ( rmsd_over_residue_list( *get_native_pose(), *screening_pose_, job_parameters_, true /*ignore_virtual_atom*/ ) > ( options_->sampler_native_screen_rmsd_cutoff() ) ) continue; //Oct 14, 2010
			count_data_.rmsd_count++;
		}

		//		TR << "NTRIES " << ntries << " PASS " << atr_rep_checker_->check_screen( *screening_pose_ ) << std::endl; screening_pose_->dump_pdb( "OLD_OUTPUT.pdb" ); exit( 0 );
		if ( options_->VDW_atr_rep_screen() && !atr_rep_checker_->check_screen( *screening_pose_ ) ) continue;
		...
		for ( Size n = 1; n <= moving_rsd_at_origin_list_.size(); n++ ){
			...
			if ( close_chain_to_distal_ ) if ( !chain_closable_geometry_to_distal_checker_->check_screen( *sugar_screening_pose_ ) ) continue;
			Size const anchor_sugar_number = screen_anchor_sugar_conformation( pose, tag );
			if ( !anchor_sugar_number ) continue; // this loops over anchor sugar models if there are more than one.
			//			if ( anchor_sugar_number == 2 ) { TR << " NTRIES " << ntries << std::endl; exit( 0 ); }
			if ( ok1 ) { count_data_.non_clash_sugar++; ok1 = false; }
			...
			if ( close_chain_to_distal_ ) sampler_->apply( chain_break_to_distal_checker_->pose(), moving_rsd_at_origin );
			if ( close_chain_to_anchor_ ) sampler_->apply( chain_break_to_anchor_checker_->pose(), moving_rsd_at_origin );
			if ( options_->sampler_perform_o2prime_pack() )  sampler_->apply( o2prime_packer_->pose(), moving_rsd_at_origin );
			...
			if ( close_chain_to_distal_ ) {
				// initiable closable check was not in there before, but need it for consistency with close_chain_to_anchor
				if ( !chain_closable_geometry_to_distal_checker_->check_screen( *sugar_screening_pose_, true /*strict*/ ) ) continue;
				if ( !chain_break_to_distal_checker_->check_screen() ) continue;
				if ( options_->sampler_perform_o2prime_pack() ) chain_break_to_distal_checker_->copy_CCD_torsions( o2prime_packer_->pose() );
				chain_break_to_distal_checker_->copy_CCD_torsions( pose );
			}

			/////////////////////////////////////////////////////////////////////////////////////
			// Try to close chain break from floating base to its anchor point, if they are directly connected.
			if ( close_chain_to_anchor_ ){
				if ( !chain_closable_geometry_to_anchor_checker_->check_screen( chain_break_to_anchor_checker_->pose(), true /*strict*/ ) ) continue;
				if ( !chain_break_to_anchor_checker_->check_screen() ) continue;
				if ( options_->sampler_perform_o2prime_pack() ) chain_break_to_anchor_checker_->copy_CCD_torsions( o2prime_packer_->pose() );
				chain_break_to_anchor_checker_->copy_CCD_torsions( pose );
			}

			if ( options_->sampler_perform_phosphate_pack() ){
				sampler_->apply( phosphate_sampler_->pose(), moving_rsd_at_origin );
				if ( close_chain_to_distal_ ) chain_break_to_distal_checker_->copy_CCD_torsions( phosphate_sampler_->pose() );
				if ( close_chain_to_anchor_ ) chain_break_to_anchor_checker_->copy_CCD_torsions( phosphate_sampler_->pose() );
	...
	TR << "Number of tries: " << ntries << ".  Passed centroid filters: " << count_data_.tot_rotamer_count++ << ". Number of successes: " << num_success << std::endl;
...
/////////////////////////////////////////////////////////////////////////////////////////
void
StepWiseRNA_FloatingBaseSampler::initialize_poses_and_stubs_and_checkers( pose::Pose & pose  ){
	...
	// rhiju checking -- trying to be totally consistent with distal & anchor! Should have caught this earlier!
	if ( close_chain_to_anchor_ ) add_variant_type_to_pose_residue( *screening_pose_, "VIRTUAL_PHOSPHATE", floating_base_five_prime_chain_break_ + 1 ); //May 31, 2010
	...
	if ( five_prime_chain_break_res_ > 0 ) chain_closable_geometry_to_distal_checker_ = new checker::ChainClosableGeometryChecker( five_prime_chain_break_res_, gap_size_ );
	...
	pose_selection_ = new StepWiseRNA_PoseSelection( job_parameters_, scorefxn_, count_data_ );
...
StepWiseRNA_FloatingBaseSampler::initialize_rigid_body_sampler( pose::Pose const & pose ){
	...
	rotamer_sampler::rigid_body::RigidBodyRotamerValueRange & value_range = sampler_->value_range();
	value_range.set_max_distance( max_distance_ );
	value_range.set_centroid_bin_size( centroid_bin_size_ );
	value_range.set_euler_angle_bin_size( euler_angle_bin_size_ );
	value_range.set_euler_z_bin_size( euler_z_bin_size_ );
...
StepWiseRNA_FloatingBaseSampler::break_early_for_integration_tests() {
	if ( options_->integration_test_mode() && count_data_.both_count > 10000 )     return true;
...
StepWiseRNA_FloatingBaseSampler::screen_anchor_sugar_conformation( pose::Pose & pose, std::string & tag ){
	...
	if ( !anchor_sugar_modeling_.sample_sugar ){
		...
		if ( !ok ) return 0;
		count_data_.chain_closable_geometry_count3++;

		if ( options_->VDW_atr_rep_screen() && !atr_rep_checker_with_instantiated_sugar_->check_screen( *sugar_screening_pose_ ) ) return 0; // wait a minute... why is this in here? oh, because base can move in different sampled sugar modeling conformations.
		return 1;
	}
	...
	//Ok, since anchor_sugar_modeling_.pose_list is sorted by SCORE, the lower energy conformations are tried first!
 	...
 	for ( Size n = 1; n <= anchor_sugar_modeling_.pose_list.size(); n++ ){
		...
		if ( options_->VDW_atr_rep_screen() && !atr_rep_checkers_for_anchor_sugar_models_[ n ]->check_screen( *sugar_screening_pose_ ) ) continue;

		copy_bulge_res_and_sugar_torsion( anchor_sugar_modeling_, pose, anchor_sugar_modeling_pose );
		if ( options_->sampler_perform_o2prime_pack() ) copy_bulge_res_and_sugar_torsion( anchor_sugar_modeling_, o2prime_packer_->pose(), anchor_sugar_modeling_pose );
...
StepWiseRNA_FloatingBaseSampler::initialize_other_residues_base_list( pose::Pose const & pose ) {
	...
	for ( Size seq_num = 1; seq_num <= pose.total_residue(); seq_num++ ){
		...
		base_info.v = core::chemical::rna::get_rna_base_centroid( residue_object, options_->verbose() );
		...
		if (  distance > ( max_distance_ + 6.364 ) ) continue; //6.364 is max centroid to centroid distance for base-stacking options_->centroid_screen()ing.
...
//////////////////////////////////////////////////////////////////
void
StepWiseRNA_FloatingBaseSampler::set_base_centroid_checker( checker::RNA_BaseCentroidCheckerOP & checker ){
...
void
StepWiseRNA_FloatingBaseSampler::set_anchor_sugar_modeling( sugar::SugarModeling const & anchor_sugar_modeling ){
...
StepWiseRNA_FloatingBaseSampler::output_count_data(){
	...
	TR << " bin_rep = " << count_data_.good_bin_rep_count <<
		" atr = " << atr_rep_checker_->count_data().good_atr_rotamer_count <<
		" rep = " << atr_rep_checker_->count_data().good_rep_rotamer_count;
	TR << " both = " << atr_rep_checker_->count_data().both_count
		 << " total_bin = " << atr_rep_checker_->count_data().tot_rotamer_count << " total_screen_bin = " << base_bin_map_.size();
	TR << "  closable1 = " << count_data_.chain_closable_geometry_count
		 << "  closable2 = " << count_data_.chain_closable_geometry_count2
		 << "  closable3 = " << count_data_.chain_closable_geometry_count3
		 << "  non_clash_sugar = " << count_data_.non_clash_sugar << std::endl;
	TR << " WARNING centroid_n count is severely UNDERESTIMATED...need to be multiply by ( euler_angle_bin_max_ - euler_angle_bin_min_ + 1 ): ";
	TR << ( euler_angle_bin_max_ - euler_angle_bin_min_ + 1 ) << std::endl;
}

/////////////////////////////////////////////////////////////////////////////////////
void
StepWiseRNA_FloatingBaseSampler::set_user_input_VDW_bin_checker( checker::RNA_VDW_BinCheckerOP const & user_input_VDW_bin_checker ){ user_input_VDW_bin_checker_ = user_input_VDW_bin_checker; }

//////////////////////////////////////////////////////////////////
void
StepWiseRNA_FloatingBaseSampler::set_options( StepWiseRNA_ModelerOptionsCOP options ){
	options_ = options;
}
