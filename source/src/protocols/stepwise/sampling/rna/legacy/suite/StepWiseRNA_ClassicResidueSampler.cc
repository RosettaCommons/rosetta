...
/// @file protocols/stepwise/sampling/rna/legacy/suite/StepWiseRNA_ClassicResidueSampler.cc
...
namespace suite {
	...
	StepWiseRNA_ClassicResidueSampler::apply( core::pose::Pose & pose ) {
		...
		for ( rotamer_sampler_->reset(); rotamer_sampler_->not_end(); ++( *rotamer_sampler_ ) ) {
			...
			if ( kic_sampling_ && !chain_closure_checker_->check_loop_closed( *screening_pose_ ) ) continue;
			...
			if ( native_rmsd_screen_ && get_native_pose() ){
				...
				if ( rmsd_over_residue_list( *get_native_pose(), *screening_pose_, job_parameters_, true /*ignore virtual atom -- was originally false*/ ) > ( options_->sampler_native_screen_rmsd_cutoff() ) ) continue; //Oct 14, 2010
			...
			if ( base_centroid_checker_ ){
				...
				found_a_centroid_interaction_partner = base_centroid_checker_->update_base_stub_list_and_check_centroid_interaction( *screening_pose_, count_data_ );
				...
				if ( !base_centroid_checker_->check_that_terminal_res_are_unstacked() ) continue;
			}
			...
			if ( close_chain_to_distal_ && !kic_sampling_ ){
				if ( !chain_closable_geometry_checker_->check_screen( *screening_pose_, options_->finer_sampling_at_chain_closure() ) ) continue;
			...
			if ( ( user_input_VDW_bin_checker_->user_inputted_VDW_screen_pose() ) && ( gap_size_ != 0 ) && ( !is_internal_ ) ){
				...
				if ( !user_input_VDW_bin_checker_->VDW_rep_screen( *screening_pose_, moving_res_ ) ) continue;
			...
			if ( close_chain_to_distal_ ) {
				if ( !kic_sampling_ ){ // kic sampling already should have closed chain.
					rotamer_sampler_->apply( chain_closure_checker_->pose() );
					if ( ! chain_closure_checker_->check_screen() ) continue;
					chain_closure_checker_->copy_CCD_torsions( pose );
					if ( options_->sampler_perform_o2prime_pack() ) chain_closure_checker_->copy_CCD_torsions( o2prime_packer_->pose() );
					if ( is_possible_bulge ){
						add_bulge = bulge_variant_decision( pose, atr_rep_checker_->delta_atr_score() ); /*further cut on atr, inside*/
					}
				}
			}
			...
			if ( options_->sampler_perform_phosphate_pack() ){
				...
				if ( close_chain_to_distal_ ) chain_closure_checker_->copy_CCD_torsions( phosphate_sampler_->pose() );
			...
			tag_into_pose( selected_pose, tag );
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
	void
	StepWiseRNA_ClassicResidueSampler::initialize_poses_and_checkers( pose::Pose & pose ){
...
//////////////////////////////////////////////////////////////////
void
StepWiseRNA_ClassicResidueSampler::set_base_centroid_checker( checker::RNA_BaseCentroidCheckerOP & checker ){
...
StepWiseRNA_ClassicResidueSampler::output_count_data(){
	...
	if ( gap_size_ <= 1 ) TR << " chain_closable_geometry_count = " << count_data_.chain_closable_geometry_count << std::endl;
	if ( gap_size_ == 0 ){
	...
	if ( options_->combine_long_loop_mode() && !close_chain_to_distal_ ) TR << "res_contact = " << count_data_.residues_contact_screen << " ";
	...
	TR << " atr = " << atr_rep_checker_->count_data().good_atr_rotamer_count;
	TR << " rep = " << atr_rep_checker_->count_data().good_rep_rotamer_count;
...
//////////////////////////////////////////////////////////////////
void
StepWiseRNA_ClassicResidueSampler::set_user_input_VDW_bin_checker( checker::RNA_VDW_BinCheckerOP const & user_input_VDW_bin_checker ){ user_input_VDW_bin_checker_ = user_input_VDW_bin_checker; }
