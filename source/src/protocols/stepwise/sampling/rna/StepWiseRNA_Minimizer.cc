...
#include <protocols/stepwise/sampling/rna/StepWiseRNA_ModelerOptions.hh> //Sept 26, 2011
...
#include <core/chemical/rna/RNA_Util.hh> // for DELTA
...
StepWiseRNA_Minimizer::pass_all_pose_screens( core::pose::Pose & pose, std::string const in_tag, core::io::silent::SilentFileData & silent_file_data ) const{
	...
	if ( job_parameters_->is_simple_full_length_job_params() == false ){
		...
		checker::ChainClosableGeometryChecker chain_closable_geometry_checker( five_prime_chain_break_res, gap_size );
		if ( gap_size == 1 &&  !chain_closable_geometry_checker.check_screen( pose ) ) pass_screen = false;

		if ( options_->sampler_native_rmsd_screen() && get_native_pose() ){	//Before have the (&& !is_chain_break condition). Parin Dec 21, 2009
			...
			if ( rmsd > options_->sampler_native_screen_rmsd_cutoff() || loop_rmsd > options_->sampler_native_screen_rmsd_cutoff() ){
				TR.Debug << in_tag << " discarded: fail sampler_native_rmsd_screen. rmsd = " << rmsd << " loop_rmsd = " << loop_rmsd << " options_->sampler_native_screen_rmsd_cutoff() = " << options_->sampler_native_screen_rmsd_cutoff() << std::endl;
				pass_screen = false;
			}
		}
	}
...
StepWiseRNA_Minimizer::figure_out_moving_residues( core::kinematics::MoveMap & mm, core::pose::Pose const & pose )
{
	...
	figure_out_stepwise_rna_movemap( mm, pose, allow_insert_ );
}
...
StepWiseRNA_Minimizer::output_parameters(){
	...
	output_boolean( " sampler_native_rmsd_screen = ", options_->sampler_native_rmsd_screen(), TR.Debug ); TR.Debug << std::endl;
	TR.Debug << " sampler_native_screen_rmsd_cutoff = " << options_->sampler_native_screen_rmsd_cutoff() << std::endl;
	...
	output_boolean( " perform_minimize = ", options_->perform_minimize(), TR.Debug ); TR.Debug << std::endl;
	output_boolean( " minimizer_rename_tag = ", options_->minimizer_rename_tag(), TR.Debug ); TR.Debug << std::endl;
}

//////////////////////////////////////////////////////////////////
void
StepWiseRNA_Minimizer::set_options( StepWiseRNA_ModelerOptionsOP options ){
	options_ = options;
}
