...
/// @file protocols/stepwise/sampling/align/StepWiseClusterer.cc
/// @brief
/// @detailed
...
static basic::Tracer TR( "protocols.stepwise.sampling.align.StepWiseClusterer" );
...
namespace align {
  ...
  StepWiseClusterer::cluster()
	{
		...
		for ( Size n = 1; n <= starting_pose_list.size(); n++ ){
  ...
  //////////////////////////////////////////////////////////////////////////
	void
	StepWiseClusterer::kick_out_pose_at_idx( Size const n ){

		runtime_assert( n >= 1 && n <= size() );
		utility::vector1< pose::PoseOP > const starting_pose_list = pose_list_;

		pose_list_.clear();
		for ( Size k = 1; k <= starting_pose_list.size(); k++ ){
