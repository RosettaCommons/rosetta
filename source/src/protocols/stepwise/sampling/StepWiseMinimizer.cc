...
namespace sampling {
		...
		protein_ccd_closer_( new protein::loop_close::StepWiseProteinCCD_Closer( working_parameters ) )
	{
		...
		runtime_assert( !modeler_options_->skip_coord_constraints() );
		runtime_assert( !modeler_options_->skip_minimize() );
		runtime_assert( !modeler_options_->move_jumps_between_chains() );
	}
  ...
  StepWiseMinimizer::apply( core::pose::Pose & pose ) {
		...
		if ( modeler_options_->output_minimized_pose_list() ) protein::output_pose_list( pose_list_, get_native_pose(),
																																										 modeler_options_->silent_file(), working_calc_rms_res_ );
	}
  ...
  //////////////////////////////////////////////////////////////////////////
	...
	StepWiseMinimizer::do_clustering( pose::Pose & pose ){
		...
		StepWiseClusterer clusterer( modeler_options_ );
  ...
  //////////////////////////////////////////////////////////////////////////
	...
	StepWiseMinimizer::do_full_minimizing( pose::Pose & pose ){
		...
		for ( Size n = 1; n <= pose_list_.size(); n++ ) {
			...
			if ( modeler_options_->num_pose_minimize() > 0 &&  n > modeler_options_->num_pose_minimize() ) break;

			pose = *pose_list_[ n ];
			Real const score_original = (*minimize_scorefxn_)( pose );
			if ( modeler_options_->rm_virt_phosphate() ) rna::remove_all_virtual_phosphates( pose ); // ERRASER.
  ...
  //////////////////////////////////////////////////////////////////////////
	...
	//  but also ones that *previously* were in contact before sampling -- they should relax too.
	//
	void
	StepWiseMinimizer::let_neighboring_side_chains_minimize( core::kinematics::MoveMap & mm,
																													 core::pose::Pose const & pose ) {
		using namespace core::scoring;
		if ( modeler_options_->global_optimize() ) working_pack_res_ = get_all_residues( pose );
