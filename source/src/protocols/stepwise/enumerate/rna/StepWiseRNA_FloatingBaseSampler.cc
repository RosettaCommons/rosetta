...
/// @file protocols/stepwise/enumerate/rna/StepWiseRNA_FloatingBaseSampler.cc
/// @brief
/// @detailed
...
StepWiseRNA_FloatingBaseSampler::apply( core::pose::Pose & pose ){
	...
	for ( sampler_->reset(); sampler_->not_end(); ++(*sampler_) ){
		...
		for ( Size n = 1; n <= moving_rsd_at_origin_list_.size(); n++ ){
			...
			if ( close_chain_to_distal_ ) {
				...
				if ( options_->sampler_perform_o2prime_pack() ) chain_break_to_distal_screener_->copy_CCD_torsions( o2prime_packer_->pose() );
			...
			if ( close_chain_to_anchor_ ){
				...
				if ( options_->sampler_perform_o2prime_pack() ) chain_break_to_anchor_screener_->copy_CCD_torsions( o2prime_packer_->pose() );
	...
	if ( options_->choose_random() ) TR << "Number of tries: " << ntries << ".  Passed centroid filters: " << count_data_.tot_rotamer_count++ << ". Number of successes: " << num_success << std::endl;
...
StepWiseRNA_FloatingBaseSampler::initialize_poses_and_stubs_and_screeners( pose::Pose & pose  ){
	...
	chain_break_to_distal_screener_->set_reinitialize_CCD_torsions( options_->reinitialize_CCD_torsions() );
	...
	chain_break_to_anchor_screener_->set_reinitialize_CCD_torsions( options_->reinitialize_CCD_torsions() );
	...
	if ( is_dinucleotide_ && base_centroid_screener_->allow_base_pair_only_screen() ) num_pose_kept_to_use_ = 4 * options_->sampler_num_pose_kept();
...
StepWiseRNA_FloatingBaseSampler::screen_anchor_sugar_conformation( pose::Pose & pose, std::string & tag ){
	...
	if ( !anchor_sugar_modeling_.sample_sugar ){
		...
		if ( options_->VDW_atr_rep_screen() && !atr_rep_screener_with_instantiated_sugar_->check_screen( *sugar_screening_pose_ ) ) return false; // wait a minute... why is this in here? oh, because base can move in different sampled sugar modeling conformations.
		return true;
	} else {
		...
		for ( Size n = 1; n <= anchor_sugar_modeling_.pose_list.size(); n++ ){
			...
			if ( options_->VDW_atr_rep_screen() && !atr_rep_screeners_for_anchor_sugar_models_[ n ]->check_screen( *sugar_screening_pose_ ) ) continue;
