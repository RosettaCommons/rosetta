...
/// @file protocols/stepwise/enumerate/rna/StepWiseRNA_StandardResidueSampler.cc
...
static basic::Tracer TR( "protocols.stepwise.rna.StepWiseRNA_StandardResidueSampler" ) ;
...
namespace rna {
	...
	StepWiseRNA_StandardResidueSampler::initialize_poses_and_screeners( pose::Pose & pose ){
		...
		atr_rep_screener_->set_sample_both_sugar_base_rotamer( options_->sample_both_sugar_base_rotamer() ); //Nov 12, 2010
	...
	StepWiseRNA_StandardResidueSampler::get_num_pose_kept(){
		...
		if ( options_->sample_both_sugar_base_rotamer() ){
...
StepWiseRNA_StandardResidueSampler::setup_rotamer_sampler( pose::Pose const & pose ) const {
	...
	if ( build_pose_from_scratch_ || options_->sample_both_sugar_base_rotamer()) {
		sample_sugar[1] = true;
		sample_sugar[2] = true;
...
//////////////////////////////////////////////////////////////////
void
StepWiseRNA_StandardResidueSampler::set_options( StepWiseRNA_ModelerOptionsCOP options ){
	options_ = options;
}
