...
/// @file protocols/stepwise/enumerate/rna/phosphate/PhosphateMover.cc
/// @brief
/// @detailed
...
static basic::Tracer TR( "protocols.stepwise.enumerate.rna.phosphate.PhosphateMover" );
...
namespace phosphate {
	...
	PhosphateMover::setup_atom_and_neighbor_list( pose::Pose & pose ) {
		...
		for ( Size m = 1; m <= pose.total_residue(); m++ ){
			...
			neighbor_residue_list_.push_back( m );
	...
	PhosphateMover::pass_clash_check( std::string const atom_name,
		...
		for ( Size k = 1; k <= neighbor_residue_list_.size(); k++ ){
			Size const & m = neighbor_residue_list_[ k ];
			if ( m == n ) continue;
