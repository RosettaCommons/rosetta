...
#include <protocols/stepwise/enumerate/rna/StepWiseRNA_OutputData.hh> //Sept 26, 2011
	...
	StepWiseRNA_Clusterer::initialize_max_memory_pose_num(){
		...
		while ( input_->has_another_pose() ) {
			...
			if ( found_valid_struct == false ){
				...
				if ( protocols::stepwise::enumerate::rna::check_for_messed_up_structure( (*pose_op), tag ) == true ) continue;
...
//		TR << "enter get_poseOP(" << n << ")" << std::endl;
	...
	StepWiseRNA_Clusterer::recalculate_rmsd_and_output_silent_file( std::string const & silent_file,
				                                                     protocols::stepwise::enumerate::rna::StepWiseRNA_PoseSetupOP & stepwise_rna_pose_setup,
		...
		for ( Size n = 1 ; n <= silent_struct_output_list_.size(); n++ ) {
			...
			if ( protocols::stepwise::enumerate::rna::check_for_messed_up_structure( pose, tag ) == true ) continue;
	...
	StepWiseRNA_Clusterer::get_best_neighboring_shift_RMSD_and_output_silent_file( std::string const & silent_file ){
		...
		for ( Size n = 1 ; n <= silent_struct_output_list_.size(); n++ ) {
			...
			if ( protocols::stepwise::enumerate::rna::check_for_messed_up_structure( current_pose, tag ) == true ) continue;
			...
			for ( Size other_pose_ID = 1 ; other_pose_ID  <= silent_struct_output_list_.size(); other_pose_ID ++ ) {
				...
				if ( protocols::stepwise::enumerate::rna::check_for_messed_up_structure( other_pose, other_tag ) == true ) continue;
	...
	//////////////////////////////////////////////
	void
	StepWiseRNA_Clusterer::set_job_parameters( protocols::stepwise::enumerate::rna::StepWiseRNA_JobParametersCOP & job_parameters ){
	...
	//////////////////////////////////////////////
	void
	SlicedPoseJobParameters::setup( protocols::stepwise::enumerate::rna::StepWiseRNA_JobParametersCOP & job_parameters ){
