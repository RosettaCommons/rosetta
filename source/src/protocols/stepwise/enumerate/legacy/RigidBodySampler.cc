...
//////////////////////////////////
#include <protocols/stepwise/enumerate/legacy/RigidBodySampler.hh>
...
static basic::Tracer TR( "protocols.stepwise.RigidBodySampler" ) ;
		...
		for ( delx_ = x_min_; delx_ <= x_max_; delx_ += x_increment_ ){
			for ( dely_ = y_min_; dely_ <= y_max_; dely_ += y_increment_ ){
				for ( delz_ = z_min_; delz_ <= z_max_; delz_ += z_increment_ ){
					...
					if ( o2prime_trials_ && check_o2prime_needs_optimization( pose ) ) protocols::stepwise::enumerate::rna::o2prime_trials( pose, o2prime_pack_scorefxn_ );
