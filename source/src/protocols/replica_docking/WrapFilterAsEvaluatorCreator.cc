...
#include <protocols/replica_docking/FnatFilter.hh>
#include <protocols/replica_docking/LrmsdFilter.hh>
#include <protocols/replica_docking/FnonnatFilter.hh>
#include <protocols/replica_docking/CaIrmsdFilter.hh>
...
static basic::Tracer tr( "protocols.replica_docking.WrapFilterAsEvaluatorCreator" );
...
void WrapFilterAsEvaluatorCreator::add_evaluators( protocols::evaluation::MetaPoseEvaluator & eval ) const {
	...
	if ( option[ OptionKeys::evaluation::I_sc ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		...
		protocols::filters::FilterOP f( new protocols::replica_docking::InteractionScoreFilter() );
		eval.add_evaluation( PoseEvaluatorOP( new protocols::replica_docking::WrapFilterAsEvaluator( f , "I_sc" ) ) );
	}
	if ( option[ OptionKeys::evaluation::Irms ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		tr << "Add Irms evaluator " << std::endl;
		eval.add_evaluation( PoseEvaluatorOP( new WrapFilterAsEvaluator( FilterOP( new protocols::replica_docking::IrmsdFilter() ), "Irms" ) ) );
	}

	if ( option[ OptionKeys::evaluation::Ca_Irms ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		tr << "Add Irms evaluator " << std::endl;
		eval.add_evaluation( PoseEvaluatorOP( new WrapFilterAsEvaluator( FilterOP( new protocols::replica_docking::CaIrmsdFilter() ), "Ca_Irms" ) ) );
	}
	...
	if ( option[ OptionKeys::evaluation::Lrmsd ].user() || option[ OptionKeys::evaluation::DockMetrics ].user() ) {
		...
		eval.add_evaluation( PoseEvaluatorOP( new WrapFilterAsEvaluator( FilterOP( new protocols::replica_docking::LrmsdFilter() ), "Lrmsd" ) ) );
	}
