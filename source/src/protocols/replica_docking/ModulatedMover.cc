...
#include <protocols/replica_docking/ModulatedMover.hh>
#include <protocols/replica_docking/ModulatedMoverCreator.hh>
...
static basic::Tracer tr( "protocols.replica_docking.ModulatedMover" );
...
ModulatedMover::parse_my_tag(
	...
	for ( auto & key : keys ) {
		...
		if ( interps_1_.find( key ) == interps_1_.end() ) {
			interps_1_[ key ] = protocols::replica_docking::TempInterpolatorBaseOP( new protocols::replica_docking::TempFixValue( 1 ) );
			tr.Debug << "parameter for " << key << " not provided for 1st dim, will use fix value 1" << std::endl;
		}
		if ( interps_2_.find( key ) == interps_2_.end() ) {
			interps_2_[ key ] = protocols::replica_docking::TempInterpolatorBaseOP( new protocols::replica_docking::TempFixValue( 1 ) );
