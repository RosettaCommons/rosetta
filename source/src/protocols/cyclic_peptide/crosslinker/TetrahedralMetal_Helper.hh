...
enum TetrahedralMetal_Helper_Metal {
	TMH_Zn = 1, //Keep this first.
	//TMH_Cu,
	TMH_unknown_metal, //Keep this second-to-last.
	TMH_end_of_list = TMH_unknown_metal //Keep this last.
};

/// @brief Metal-liganding atom types compatible with this helper.
/// @details If you add to this list, you need to add entries to
/// the map in ideal_bond_length().
enum TetrahedralMetal_Helper_MetalLigand {
	TMHLigand_Nd_histidine = 1, //Keep first.
	TMHLigand_Ne_histidine,
	TMHLigand_O_carboxyl,
	TMHLigand_S_cysteine,
	TMHLigand_unknown_ligand, //Keep second-to-last.
	TMHLigand_end_of_list = TMHLigand_unknown_ligand //Keep last.
};

/// @brief A helper class for setting up tetrahedrally-coordinated metals
/// like Zn or Cu.
class TetrahedralMetal_Helper : public CrosslinkerMoverHelper {
...
public: // public defined methods
	...
	/// @brief Get the current metal type, by enum.
	inline TetrahedralMetal_Helper_Metal metal_type() const { return metal_type_; }
...
private: // private methods
	...
	void check_residue_indices_valid( utility::vector1< core::Size > const &indices, core::pose::Pose const &pose ) const;
	...
	bool is_allowed_type( core::chemical::ResidueType const &type ) const;
	...
	void add_angle_constraints( core::pose::Pose &pose, utility::vector1< core::Size > const &res_indices, core::Size const i, core::Size const j) const;
	...
	core::Real const & ideal_bond_length( TetrahedralMetal_Helper_Metal const metal_type, TetrahedralMetal_Helper_MetalLigand const ligand_type ) const;
	...
	/// atom enum.
	TetrahedralMetal_Helper_MetalLigand liganding_atom_from_restype( core::chemical::ResidueType const &restype ) const;

	/// @brief Given a metal type enum, return the corresponding string.
	/// @details This method must be updated if the enum is expanded.
	std::string metal_type_string_from_enum( TetrahedralMetal_Helper_Metal const metal_type ) const;
