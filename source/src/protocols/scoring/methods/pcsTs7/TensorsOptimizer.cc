...
// Unit headers
#include <protocols/scoring/methods/pcsTs7/TensorsOptimizer.hh>
#include <protocols/scoring/methods/pcsTs7/PseudocontactShiftData.hh>
#include <protocols/scoring/methods/pcsTs7/PseudocontactShiftInput.hh> // REQUIRED FOR WINDOWS 
// Package headers
...
basic::Tracer TR_tsr_opt_Ts7("protocols.scoring.methods.pcsTs7.TensorsOptimizer_Ts7");

TensorsOptimizer_Ts7::TensorsOptimizer_Ts7():
		pcs_d_(PCS_data_Ts7())
{
  utility_exit_with_message("You shouldn't call the empty constructor for class TensorsOptimizer_Ts7");
}


TensorsOptimizer_Ts7::TensorsOptimizer_Ts7(PCS_data_Ts7 const & pcs_d):
	pcs_d_(pcs_d)
{
}
...
core::Real
TensorsOptimizer_Ts7::operator ()( core::optimization::Multivec const & vars ) const{
	core::Size i, j, idx;
	...
	if(n_la != (vars.size() - 3) / 5){
		TR_tsr_opt_Ts7 << "n_la: " << n_la << " (vars.size() - 3) / 5):" << (vars.size() - 3) / 5 << " vars.size(): " << vars.size() <<std::endl;
	...
	const utility::vector1<PCS_data_per_lanthanides_Ts7> & pcs_d_p_l_a(pcs_d_.get_pcs_data_per_lanthanides_all());
  ...
  const utility::vector1<core::Real> & Z_all(pcs_d_.get_Z_all());
	...
	for (i = 1; i <= n_la; i++){
		...
		PCS_data_per_lanthanides_Ts7 const & pcs_d_p_l(pcs_d_p_l_a[i]);
...
void
TensorsOptimizer_Ts7::dfunc(core::optimization::Multivec const & vars,
...
void
TensorsOptimizer_Ts7::dfunc_test(optimization::Multivec const & vars) const{
  ...
  const utility::vector1<core::Real> & Z_all(pcs_d_.get_Z_all());
	...
	for (i = 1; i <= n_la; i++){
		...
		core::Real Xyy(vars[3 + 5*(i-1) + 4]);
		core::Real Xyz(vars[3 + 5*(i-1) + 5]);
		const PCS_data_per_lanthanides_Ts7 & pcs_d_p_l(pcs_d_p_l_a[i]);
