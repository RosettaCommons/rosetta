...
// Unit headers
#include <protocols/scoring/methods/pcsTs8/PseudocontactShiftEnergy.hh>
#include <protocols/scoring/methods/pcsTs8/PseudocontactShiftEnergyCreator.hh>

// Package headers
#include <protocols/scoring/methods/pcsTs8/PseudocontactShiftData.hh>
#include <protocols/scoring/methods/pcsTs8/PseudocontactShiftInput.hh>
#include <protocols/scoring/methods/pcsTs8/PseudocontactShiftTensor.hh>
#include <protocols/scoring/methods/pcsTs8/GridSearchIterator.hh>
#include <protocols/scoring/methods/pcsTs8/TensorsOptimizer.hh>
...
PseudocontactShiftEnergyCreator_Ts8::create_energy_method(
	core::scoring::methods::EnergyMethodOptions const &
) const {
	return new PCS_Energy_Ts8;
}

core::scoring::ScoreTypes
PseudocontactShiftEnergyCreator_Ts8::score_types_for_method() const {
	core::scoring::ScoreTypes sts;
	sts.push_back( core::scoring::pcsTs8 );
	return sts;
}

basic::Tracer TR_PCS_Energy_Ts8("protocols.scoring.methods.pcsTs8.PCS_Energy_Ts8");

void
PCS_Energy_Ts8::indicate_required_context_graphs( utility::vector1< bool > & ) const{
}

PCS_Energy_Ts8 &
PCS_Energy_Ts8::operator=(PCS_Energy_Ts8 const & other){
	std::cerr << "Error, == operator not correctly implemented in the PCS_Energy_Ts8" << std::endl;
...
PCS_Energy_Ts8::PCS_Energy_Ts8(PCS_Energy_Ts8 const & src ):
	parent( src )
{
}

PCS_Energy_Ts8::~PCS_Energy_Ts8(){
}

/// c-tor
PCS_Energy_Ts8::PCS_Energy_Ts8() :
	parent( new PseudocontactShiftEnergyCreator_Ts8 )
{}

/// clone
core::scoring::methods::EnergyMethodOP
PCS_Energy_Ts8::clone() const{
	return new PCS_Energy_Ts8;
}
...
void
PCS_Energy_Ts8::finalize_total_energy(core::pose::Pose & pose,
	core::scoring::ScoreFunction const &,
	core::scoring::EnergyMap & totals
) const {
	//using namespace conformation;
	totals[ core::scoring::pcsTs8 ] = calculate_pcs_score( pose, false );
...
PCS_data_Ts8 &
PCS_Energy_Ts8::PCS_data_from_pose(core::pose::Pose & pose) const{

	bool have_exclusions_changed = PCS_Energy_parameters_manager_Ts8::get_instance()->has_exclude_residues_vector_changed();

	if ( (!have_exclusions_changed) &&
			 ( pose.data().has( core::pose::datacache::CacheableDataType::TS8_PSEUDOCONTACT_SHIFT_DATA ) ) ){
		return *( static_cast< PCS_data_Ts8 * >( pose.data().get_ptr( core::pose::datacache::CacheableDataType::TS8_PSEUDOCONTACT_SHIFT_DATA )() ) );
	}

	bool has_exclude_residues = PCS_Energy_parameters_manager_Ts8::get_instance()->has_exclude_residues_vector();
	...
	vec_filename = PCS_Energy_parameters_manager_Ts8::get_instance()->get_vector_filename();
	vec_weight = PCS_Energy_parameters_manager_Ts8::get_instance()->get_vector_weight();

	TR_PCS_Energy_Ts8 << "Initialization of PCS_data_Ts8" << std::endl;
        
        

	if(vec_filename.size() == 0){
		utility_exit_with_message("Missing input file for PCS@Ts8. Review your setup file");
	}

	PCS_data_input_Ts8 pcs_d_i = PCS_data_input_manager_Ts8::get_instance()->get_input_data(vec_filename, vec_weight);

	PCS_data_Ts8OP pcs_d;
	if (has_exclude_residues) {
		utility::vector1< bool > exclude_residues;
		exclude_residues = PCS_Energy_parameters_manager_Ts8::get_instance()->get_vector_exclude_residues();
		pcs_d = new PCS_data_Ts8(pcs_d_i, exclude_residues);
	} else {
		pcs_d = new PCS_data_Ts8(pcs_d_i);
	}

	if ( have_exclusions_changed ) {
		PCS_Energy_parameters_manager_Ts8::get_instance()->exclude_residues_vector_is_current();
	}

	pose.data().set( core::pose::datacache::CacheableDataType::TS8_PSEUDOCONTACT_SHIFT_DATA, pcs_d );
...
) const
{
	...
	if( option[ basic::options::OptionKeys::PCSTS8::write_extra ].user() ){

		std::string file_dump (option[ basic::options::OptionKeys::PCSTS8::write_extra ]());
		...
		for (i = 1 ; i <= pcs_d.get_n_lanthanides(); ++i){
			PCS_data_per_lanthanides_Ts8 const & PCS_d_p_l (pcs_d.get_pcs_data_per_lanthanides_all()[i]);
			utility::vector1<PCS_line_data_Ts8> const & PCS_d_l_a_s (pcs_d.get_PCS_data_line_all_spin());
...
core::Real
PCS_Energy_Ts8::calculate_pcs_score(core::pose::Pose & pdb, bool print_to_tracer) const{
  ...
  utility::vector1<PCS_tensor_Ts8> vec_tensor;
	...
	core::Real pcs_weight (PCS_Energy_parameters_manager_Ts8::get_instance()->get_pcs_weight());

	PCS_data_Ts8 &pcs_d = PCS_data_from_pose(pdb);
	...
	for ( i = 1; i <= pcs_d.get_n_lanthanides(); ++i){
		PCS_tensor_Ts8 PCS_t = PCS_tensor_Ts8(0, 0, 0, 0, 0, ((pcs_d.get_pcs_data_per_lanthanides_all())[i]).get_filename());
	...
	minimize_best_tensor = PCS_Energy_parameters_manager_Ts8::get_instance()->get_minimize_best_tensor();
	...
	if(minimize_best_tensor){
		if(print_to_tracer){//Only called in PCS_main at the moment, quick flag.
			...
			TR_PCS_Energy_Ts8 << "*** Before minimization of the tensor ***" << std::endl;
			TR_PCS_Energy_Ts8 << "Score: " << pcs_score_total << std::endl;
			TR_PCS_Energy_Ts8 << "Sum of: ";
			for(i = 1; i <= vec_score.size(); ++i){
				TR_PCS_Energy_Ts8	<< vec_score[i] << " ";
			}
			TR_PCS_Energy_Ts8	<< std::endl;
			TR_PCS_Energy_Ts8 << "Score weighted: " << pcs_weight * pcs_score_total << std::endl;
			TR_PCS_Energy_Ts8 << "Tensors found:" << std::endl;
			for(i = 1; i <= vec_tensor.size(); ++i){
				TR_PCS_Energy_Ts8	<< vec_tensor[i] << std::endl;
			}
			TR_PCS_Energy_Ts8 << "Lanthanide position: " << best_coo.x() << " " << best_coo.y() << " " << best_coo.z() << std::endl;
			}
		...
		if((pcs_score_total + tolerance) < optimized_score){
			TR_PCS_Energy_Ts8 << "Warning, optimized score has a higher value than starting position. Problem with minimizer?" << std::endl;
			TR_PCS_Energy_Ts8 << pcs_score_total << " -> " << optimized_score << std::endl;
		...
		if(print_to_tracer){//Only called in PCS_main at the moment, quick flag.
			TR_PCS_Energy_Ts8 << "*** After minimization of the tensor ***" << std::endl;
			TR_PCS_Energy_Ts8 << "Score: " << optimized_score << std::endl;
			TR_PCS_Energy_Ts8 << "Sum of: NOT AVAILABLE";
			TR_PCS_Energy_Ts8	<< std::endl;
			TR_PCS_Energy_Ts8 << "Score weighted: " << pcs_weight * optimized_score << std::endl;
			TR_PCS_Energy_Ts8 << "Tensors found:" << std::endl;
			for(i = 1; i <= vec_tensor.size(); ++i){
				TR_PCS_Energy_Ts8	<< vec_tensor[i] << std::endl;
			}
...
core::Real
PCS_Energy_Ts8::minimize_tensors_from_PCS_data(	utility::vector1<PCS_tensor_Ts8> & vec_best_tensor,
																						numeric::xyzVector< core::Real > & best_coo,
																						PCS_data_Ts8 const & pcs_d
																						) const{
	...
	TensorsOptimizer_Ts8 tensors_opt(pcs_d);
...
core::Real
PCS_Energy_Ts8::calculate_scores_and_tensors_from_pose_and_PCS_data(	utility::vector1<core::Real> & vec_best_score,
																																	utility::vector1<PCS_tensor_Ts8> & vec_best_tensor,
																																	numeric::xyzVector< core::Real > & best_coo,
																																	core::pose::Pose const & pdb,
																																	PCS_data_Ts8 & pcs_d) const{
	...
	utility::vector1<PCS_tensor_Ts8> vec_tensor_temp;

	//ref to do atomic best switch in order to avoid copy stuff
	utility::vector1<core::Real> * vec_score_ref_current;
	utility::vector1<PCS_tensor_Ts8> * vec_tensor_ref_current;

	//PCS_Energy_parameters_manager_Ts8::get_instance()->print_grid_param();
	...
	//	TR_PCS_Energy_Ts8 << "Reading grid search paramaters" << std::endl;

	core::Real grid_edge (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_edge());
	core::Real grid_step (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_step());
	core::Real grid_small_cutoff (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_small_cutoff());
	core::Real grid_large_cutoff (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_large_cutoff());
	core::Real grid_cone_angle_cutoff (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_cone_angle_cutoff());
	std::string grid_atom_name_1 (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_atom_name_1());
	std::string grid_atom_name_2 (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_atom_name_2());
	core::Size grid_residue_num_1 (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_residue_num_1());
	core::Size grid_residue_num_2 (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_residue_num_2());
	core::Real grid_k_vector (PCS_Energy_parameters_manager_Ts8::get_instance()->get_grid_k_vector());
	...
	GridSearchIterator_Ts8 grid_it(coo1, coo2, grid_k_vector, grid_edge, grid_step, grid_small_cutoff, grid_large_cutoff, grid_cone_angle_cutoff);
	...
	while(grid_it.next_center(x, y, z) == true){
		...
		//TR_PCS_Energy_Ts8 << "trying x= " << x << "y= " << y << "z= " << z << std::endl;
...
core::Size
PCS_Energy_Ts8::version() const
{
	return 1; // Initial versioning
}

PCS_Energy_parameters_manager_Ts8 *
PCS_Energy_parameters_manager_Ts8::get_instance(){
	if ( instance_ == 0 ){
		instance_ = new PCS_Energy_parameters_manager_Ts8();
	}
	return instance_;
}

PCS_Energy_parameters_manager_Ts8::PCS_Energy_parameters_manager_Ts8(){
	/*
...
void
PCS_Energy_parameters_manager_Ts8::set_vector_exclude_residues(utility::vector1< core::Size > const vec_exclude) {
...
void
PCS_Energy_parameters_manager_Ts8::remove_vector_exclude_residues() {
...
bool
PCS_Energy_parameters_manager_Ts8::has_exclude_residues_vector() {
	return vec_exclude_residues_exists_;
}

bool
PCS_Energy_parameters_manager_Ts8::has_exclude_residues_vector_changed() {
	return vec_exclude_residues_changed_;
}


utility::vector1< bool >
PCS_Energy_parameters_manager_Ts8::get_vector_exclude_residues() {
	return vec_exclude_residues_;
}

void
PCS_Energy_parameters_manager_Ts8::exclude_residues_vector_is_current() {
	vec_exclude_residues_changed_ = false;
}
...
void
PCS_Energy_parameters_manager_Ts8::set_vector_name_and_weight(utility::vector1<std::string> const vec_filename,
...
void
PCS_Energy_parameters_manager_Ts8::set_grid_param(core::Real const grid_edge,
...
void
PCS_Energy_parameters_manager_Ts8::print_grid_param() const{
	std::cout <<
		grid_edge_<< " " <<
		grid_step_<<" " <<
		grid_small_cutoff_<<" " <<
	...
	core::Real
	PCS_Energy_parameters_manager_Ts8::get_grid_edge() const{
		return grid_edge_;
	}

	core::Real
	PCS_Energy_parameters_manager_Ts8::get_grid_step() const{
		return grid_step_;
	}

	core::Real
	PCS_Energy_parameters_manager_Ts8::get_grid_small_cutoff() const{
		return grid_small_cutoff_;
	}

	core::Real
	PCS_Energy_parameters_manager_Ts8::get_grid_large_cutoff() const{
		return grid_large_cutoff_;
	}

	core::Real
	PCS_Energy_parameters_manager_Ts8::get_grid_cone_angle_cutoff() const{
		return grid_cone_angle_cutoff_;
	}

	std::string
	PCS_Energy_parameters_manager_Ts8::get_grid_atom_name_1() const{
		return grid_atom_name_1_;
	}

	std::string
	PCS_Energy_parameters_manager_Ts8::get_grid_atom_name_2() const{
		return grid_atom_name_2_;
	}

	core::Size
	PCS_Energy_parameters_manager_Ts8::get_grid_residue_num_1() const{
		return grid_residue_num_1_;
	}

	core::Size
	PCS_Energy_parameters_manager_Ts8::get_grid_residue_num_2() const{
		return grid_residue_num_2_;
	}

	core::Real
	PCS_Energy_parameters_manager_Ts8::get_grid_k_vector() const{
		return grid_k_vector_;
	}

	bool
	PCS_Energy_parameters_manager_Ts8::get_minimize_best_tensor() const{
		return minimize_best_tensor_;
	}

	core::Real
	PCS_Energy_parameters_manager_Ts8::get_pcs_weight() const{
		return pcs_weight_;
	}

	utility::vector1<std::string> const &
	PCS_Energy_parameters_manager_Ts8::get_vector_filename() const{
		return vec_filename_;
	}

	utility::vector1<core::Real> const &
	PCS_Energy_parameters_manager_Ts8::get_vector_weight() const{
		return vec_individual_weight_;
	}

PCS_Energy_parameters_manager_Ts8 * PCS_Energy_parameters_manager_Ts8::instance_( 0 );
