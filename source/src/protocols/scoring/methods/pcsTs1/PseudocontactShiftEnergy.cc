...
static thread_local basic::Tracer TR_PCS_Energy_Ts1( "protocols.scoring.methods.pcsTs1.PCS_Energy_Ts1" );
...
PCS_Energy_Ts1::PCS_Energy_Ts1(PCS_Energy_Ts1 const & src ):
	parent( src )
{
}
...
/// c-tor
PCS_Energy_Ts1::PCS_Energy_Ts1() :
	parent( core::scoring::methods::EnergyMethodCreatorOP( new PseudocontactShiftEnergyCreator_Ts1 ) )
{}
...
PCS_Energy_Ts1::PCS_data_from_pose(core::pose::Pose & pose) const{
	...
	if ( (!have_exclusions_changed) &&
		...
		return *( static_cast< PCS_data_Ts1 * >( pose.data().get_ptr( core::pose::datacache::CacheableDataType::TS1_PSEUDOCONTACT_SHIFT_DATA )() ) );
	}
	...
	if (has_exclude_residues) {
		...
		pcs_d = new PCS_data_Ts1(pcs_d_i, exclude_residues);
	} else {
		pcs_d = new PCS_data_Ts1(pcs_d_i);
	}
