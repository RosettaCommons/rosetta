...
TensorsOptimizer_Ts2::TensorsOptimizer_Ts2():
		pcs_d_(PCS_data_Ts2())
{
  utility_exit_with_message("You shouldn't call the empty constructor for class TensorsOptimizer_Ts2");
}
...
void
TensorsOptimizer_Ts2::dfunc_test(optimization::Multivec const & vars) const{
  ...
  const utility::vector1<core::Real> & Z_all(pcs_d_.get_Z_all());
	...
	for (i = 1; i <= n_la; i++){
		...
		core::Real Xyy(vars[3 + 5*(i-1) + 4]);
		core::Real Xyz(vars[3 + 5*(i-1) + 5]);
		const PCS_data_per_lanthanides_Ts2 & pcs_d_p_l(pcs_d_p_l_a[i]);
