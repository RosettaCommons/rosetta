...
static thread_local basic::Tracer TR_PCS_Energy_Ts2( "protocols.scoring.methods.pcsTs2.PCS_Energy_Ts2" );
...
PCS_Energy_Ts2::PCS_data_from_pose(core::pose::Pose & pose) const{
	...
	if ( (!have_exclusions_changed) &&
		...
		return *( static_cast< PCS_data_Ts2 * >( pose.data().get_ptr( core::pose::datacache::CacheableDataType::TS2_PSEUDOCONTACT_SHIFT_DATA )() ) );
	}
	...
	if (has_exclude_residues) {
		...
		pcs_d = new PCS_data_Ts2(pcs_d_i, exclude_residues);
	} else {
		pcs_d = new PCS_data_Ts2(pcs_d_i);
	}
...
PCS_Energy_parameters_manager_Ts2::get_instance(){
	if ( instance_ == 0 ){
		instance_ = new PCS_Energy_parameters_manager_Ts2();
	}
	return instance_;
}
...
PCS_Energy_parameters_manager_Ts2 * PCS_Energy_parameters_manager_Ts2::instance_( 0 );
