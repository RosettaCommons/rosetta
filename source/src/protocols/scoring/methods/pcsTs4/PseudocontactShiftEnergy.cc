...
basic::Tracer TR_PCS_Energy_Ts4("protocols.scoring.methods.pcsTs4.PCS_Energy_Ts4");
...
/// c-tor
PCS_Energy_Ts4::PCS_Energy_Ts4() :
	parent( new PseudocontactShiftEnergyCreator_Ts4 )
{}
...
PCS_Energy_Ts4::PCS_data_from_pose(core::pose::Pose & pose) const{
	...
	if ( (!have_exclusions_changed) &&
		...
		return *( static_cast< PCS_data_Ts4 * >( pose.data().get_ptr( core::pose::datacache::CacheableDataType::TS4_PSEUDOCONTACT_SHIFT_DATA )() ) );
	}
	...
	if (has_exclude_residues) {
		...
		pcs_d = new PCS_data_Ts4(pcs_d_i, exclude_residues);
	} else {
		pcs_d = new PCS_data_Ts4(pcs_d_i);
	}
...
PCS_Energy_parameters_manager_Ts4::get_instance(){
	if ( instance_ == 0 ){
		instance_ = new PCS_Energy_parameters_manager_Ts4();
	}
	return instance_;
}
...
PCS_Energy_parameters_manager_Ts4 * PCS_Energy_parameters_manager_Ts4::instance_( 0 );
