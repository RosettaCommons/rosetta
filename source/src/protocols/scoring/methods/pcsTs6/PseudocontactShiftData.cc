...
// Unit headers
#include <protocols/scoring/methods/pcsTs6/PseudocontactShiftData.hh>

// Package headers
#include <protocols/scoring/methods/pcsTs6/PseudocontactShiftInput.hh>
#include <protocols/scoring/methods/pcsTs6/PseudocontactShiftTensor.hh>
...
basic::Tracer TR_pcs_d_p_l_Ts6("protocols.scoring.methods.pcsTs6.PCS_data_per_lanthanides_Ts6");
basic::Tracer TR_pcs_d_Ts6("protocols.scoring.methods.pcsTs6.PCS_data_Ts6");

PCS_data_per_lanthanides_Ts6::~PCS_data_per_lanthanides_Ts6(){
}

PCS_data_per_lanthanides_Ts6::PCS_data_per_lanthanides_Ts6():
	filename_(""), weight_(0)
{
	utility_exit_with_message( "You shouldn't call the empty constructor for PCS_data_per_lanthanides_Ts6 class" );
}

PCS_data_per_lanthanides_Ts6::PCS_data_per_lanthanides_Ts6(PCS_data_per_lanthanides_Ts6 const &other):
...
core::Real
PCS_data_per_lanthanides_Ts6::get_weight() const{
	return weight_;
}

core::Real
PCS_data_per_lanthanides_Ts6::get_normalization_factor() const{
	return normalization_factor_;
}

	/*
core::Real
PCS_data_per_lanthanides_Ts6::get_normalization_1() const{
	return normalization_1_;
}

core::Real
PCS_data_per_lanthanides_Ts6::get_normalization_2() const{
	return normalization_2_;
}

core::Real
PCS_data_per_lanthanides_Ts6::get_normalization_3() const{
	return normalization_3_;
}
	*/

PCS_data_per_lanthanides_Ts6 &
PCS_data_per_lanthanides_Ts6::operator=( PCS_data_per_lanthanides_Ts6 const & other )
{
	if ( this != &other ) {
		n_pcs_ = other.n_pcs_;
		A_index_ = other.A_index_;
...
PCS_data_Ts6::PCS_data_Ts6(){
	utility_exit_with_message( "You shouldn't call the empty constructor for PCS_data_Ts6 class" );
}
...
PCS_data_Ts6 &
PCS_data_Ts6::operator=( PCS_data_Ts6 const &other )
{
	if ( this != &other ) {
...
PCS_data_Ts6::PCS_data_Ts6(PCS_data_Ts6 const &other):
	CacheableData()
{
	n_lanthanides_ = other.n_lanthanides_;
	n_pcs_spin_ = other.n_pcs_spin_;
...
PCS_data_Ts6::clone() const {
	return new PCS_data_Ts6( *this );
}
...
void
PCS_data_per_lanthanides_Ts6::update_my_A_matrix(utility::vector1< utility::vector1<core::Real> > & A_all){
	core::Size i, j;
...
core::Real
PCS_data_per_lanthanides_Ts6::calculate_tensor_and_cost_with_svd(PCS_tensor_Ts6 &PCS_t){
...
utility::vector1<PCS_data_per_lanthanides_Ts6> &
PCS_data_Ts6::get_pcs_data_per_lanthanides_all() {
	return (PCS_data_per_lanthanides_all_);
}

const utility::vector1<PCS_line_data_Ts6> &
PCS_data_Ts6::get_PCS_data_line_all_spin() const{
	return (PCS_data_line_all_spin_);
}

const utility::vector1<PCS_data_per_lanthanides_Ts6>&
PCS_data_Ts6::get_pcs_data_per_lanthanides_all() const {
	return (PCS_data_per_lanthanides_all_);
}

void
PCS_data_Ts6::update_matrix_A(){
	core::Size i;
...
void
PCS_data_Ts6::update_matrix_A_all(core::Real const X,
															core::Real const Y,
															core::Real const Z){
	core::Size i;
	core::Real x, y, z;
...
void
PCS_data_Ts6::update_X_Y_Z_all(core::pose::Pose const & pose){
	core::Size i, res;
	std::string at;
...
core::Size
PCS_data_per_lanthanides_Ts6::get_n_pcs() const{
	return (n_pcs_);
}


const utility::vector1<core::Size> &
PCS_data_per_lanthanides_Ts6::get_A_index() const{
	return(A_index_);
}

const FArray1D< core::Real > &
PCS_data_per_lanthanides_Ts6::get_fstyle_b() const{
	return(fstyle_b_);
}
...
void
PCS_data_per_lanthanides_Ts6::print_index_A() const{
	core::Size i;
	for (i = 1; i <= n_pcs_; ++i){
		TR_pcs_d_p_l_Ts6 << i << " -> "  <<A_index_[i] << std::endl;
	}
}
*/

core::Size
PCS_data_Ts6::where_is_line(PCS_line_data_Ts6 & P_l_d){
	utility::vector1<PCS_line_data_Ts6>::iterator it;
	core::Size index;
	index = 1;
...
// PCS_data_per_lanthanides_Ts6::PCS_data_per_lanthanides_Ts6(std::string filename,
// 																									 PCS_file_data_Ts6 & PCS_f_d,
...
// 	utility::vector1<PCS_line_data_Ts6> & PCS_d_l_a = PCS_f_d.get_PCS_data_line_all_reference();
...
// 		PCS_line_data_Ts6 PCS_l_d = PCS_d_l_a[i];
...
// 	TR_pcs_d_p_l_Ts6 << "n_pcs: " << n_pcs_ << std::endl;
// 	TR_pcs_d_p_l_Ts6 << "Normalization 1: " << normalization_1_ << std::endl;
// 	TR_pcs_d_p_l_Ts6 << "Normalization 2: " << normalization_2_ << std::endl;
// 	TR_pcs_d_p_l_Ts6 << "Normalization 3: " << normalization_3_ << std::endl;

// 	if( option[ basic::options::OptionKeys::PCSTS6::normalization_id ].user() ){
// 		core::Size norma_id (option[ basic::options::OptionKeys::PCSTS6::normalization_id ]());

// 		switch (norma_id){
// 		case 1:{
// 			TR_pcs_d_p_l_Ts6 << "Using Normalization '1': " << normalization_1_ << ". Normalize each data set by SQRT( SUM( PCScalc(i)^2 ) ) " << std::endl;
// 			normalization_factor_ = normalization_1_;
// 			break;
// 		}
// 		case 2:{
// 			TR_pcs_d_p_l_Ts6 << "Using Normalization '2': " << normalization_2_ << ". Normalize each data set by Standard Deviation" << std::endl;
// 			normalization_factor_ = normalization_2_;
// 			break;
// 		}
// 		case 3:{
// 			TR_pcs_d_p_l_Ts6 << "Using Normalization '3': " << normalization_3_ << ". Normalize each data set by SQRT( SUM( PCScalc(i)^2 ) / N) " << std::endl;
// 			normalization_factor_ = normalization_3_;
// 			break;
// 		}
// 		default:{
// 			TR_pcs_d_p_l_Ts6 << "Normalization '"<< norma_id << "' id not recognized " << std::endl;
...
// 	}
// 	else{
// 		TR_pcs_d_p_l_Ts6 << "Normalization NOT used in calculations " << std::endl;
...
PCS_data_per_lanthanides_Ts6::PCS_data_per_lanthanides_Ts6(std::string filename,
																									 core::Real const weight,
																									 utility::vector1< PCS_line_data_Ts6 > & PCS_d_l_a):
...
std::string
PCS_data_per_lanthanides_Ts6::get_filename() const {
	return (filename_);
}

void
PCS_data_per_lanthanides_Ts6::set_A_index(core::Size j,
																			core::Size index){
	A_index_[j] = index;
}

PCS_data_Ts6::PCS_data_Ts6(PCS_data_input_Ts6 & P_d_i){
	std::map< std::string, PCS_file_data_Ts6 >::iterator it;

	std::string filename;
	PCS_line_data_Ts6 * P_l_d_temp;
	core::Size index, i, j;

	n_pcs_spin_ = 0;
	n_lanthanides_ = 0;

	std::map< std::string, PCS_file_data_Ts6 > & P_f_a_d = P_d_i.get_PCS_data_input_reference();

	for ( it = P_f_a_d.begin(); it != P_f_a_d.end(); ++it ) {
		filename = it->first;

		PCS_file_data_Ts6 & P_f_d = (it->second);
		n_lanthanides_ = n_lanthanides_ + 1;
		TR_pcs_d_Ts6 << "Filename " << filename  <<std::endl;
		core::Real weight(P_f_d.get_weight());

		utility::vector1<PCS_line_data_Ts6>::iterator it2;
		utility::vector1<PCS_line_data_Ts6> & PCS_d_l_a = P_f_d.get_PCS_data_line_all_reference();

		PCS_data_per_lanthanides_Ts6 P_d_p_l = PCS_data_per_lanthanides_Ts6(filename, weight, PCS_d_l_a );
	...
	TR_pcs_d_Ts6 << "Total spin independent: " <<	n_pcs_spin_ << std::endl;
...
PCS_data_Ts6::PCS_data_Ts6(PCS_data_input_Ts6 & P_d_i, utility::vector1< bool > const exclude_residues ){
	std::map< std::string, PCS_file_data_Ts6 >::iterator it;
	...
	for ( it = P_f_a_d.begin(); it != P_f_a_d.end(); ++it ) {
		...
		utility::vector1< PCS_line_data_Ts6 >::iterator it2;
		utility::vector1< PCS_line_data_Ts6 > & file_PCS_d_l_a = P_f_d.get_PCS_data_line_all_reference();
		utility::vector1< PCS_line_data_Ts6 > PCS_d_l_a;
...
core::Size
PCS_data_Ts6::get_n_lanthanides() const{
	return (n_lanthanides_);
}

const utility::vector1<core::Real> &
PCS_data_Ts6::get_X_all() const{
	return(X_all_);
}

const utility::vector1<core::Real> &
PCS_data_Ts6::get_Y_all() const{
	return(Y_all_);
}

const utility::vector1<core::Real> &
PCS_data_Ts6::get_Z_all() const{
	return(Z_all_);
}

std::ostream &
operator<<(std::ostream& out, const PCS_data_Ts6 & P_d){
	core::Size i;
...
std::ostream &
operator<<(std::ostream& out, const PCS_data_per_lanthanides_Ts6 &PCS_d_p_l){
	core::Size i, j;
