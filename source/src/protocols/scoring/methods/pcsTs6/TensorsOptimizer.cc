...
#include <protocols/scoring/methods/pcsTs6/PseudocontactShiftInput.hh> // REQUIRED FOR WINDOWS 
// Package headers
...
basic::Tracer TR_tsr_opt_Ts6("protocols.scoring.methods.pcsTs6.TensorsOptimizer_Ts6");

TensorsOptimizer_Ts6::TensorsOptimizer_Ts6():
		pcs_d_(PCS_data_Ts6())
{
  utility_exit_with_message("You shouldn't call the empty constructor for class TensorsOptimizer_Ts6");
}


TensorsOptimizer_Ts6::TensorsOptimizer_Ts6(PCS_data_Ts6 const & pcs_d):
	pcs_d_(pcs_d)
{
}
...
core::Real
TensorsOptimizer_Ts6::operator ()( core::optimization::Multivec const & vars ) const{
	core::Size i, j, idx;
	...
	if(n_la != (vars.size() - 3) / 5){
		TR_tsr_opt_Ts6 << "n_la: " << n_la << " (vars.size() - 3) / 5):" << (vars.size() - 3) / 5 << " vars.size(): " << vars.size() <<std::endl;
	...
	const utility::vector1<PCS_data_per_lanthanides_Ts6> & pcs_d_p_l_a(pcs_d_.get_pcs_data_per_lanthanides_all());
  ...
  const utility::vector1<core::Real> & Z_all(pcs_d_.get_Z_all());
	...
	for (i = 1; i <= n_la; i++){
		...
		PCS_data_per_lanthanides_Ts6 const & pcs_d_p_l(pcs_d_p_l_a[i]);
...
void
TensorsOptimizer_Ts6::dfunc(core::optimization::Multivec const & vars,
...
void
TensorsOptimizer_Ts6::dfunc_test(optimization::Multivec const & vars) const{
  ...
  const utility::vector1<core::Real> & Z_all(pcs_d_.get_Z_all());
	...
	for (i = 1; i <= n_la; i++){
		...
		core::Real Xyy(vars[3 + 5*(i-1) + 4]);
		core::Real Xyz(vars[3 + 5*(i-1) + 5]);
		const PCS_data_per_lanthanides_Ts6 & pcs_d_p_l(pcs_d_p_l_a[i]);
