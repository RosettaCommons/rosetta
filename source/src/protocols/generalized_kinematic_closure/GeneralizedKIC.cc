...
GeneralizedKIC::doKIC(
	...
	for ( core::Size iattempt = 1; (n_closure_attempts_>0 ? iattempt<=n_closure_attempts_ : true); ++iattempt ) { //Loop for closure attempts
		...
		for ( utility::vector1< utility::vector1< core::Real > >::iterator it=t_ang[iattempt].begin(); it!=t_ang[iattempt].end(); ++it ) {
			++solcount;
			if ( solutions_to_delete[solcount] ) t_ang[iattempt].erase( it );
		}
		solcount=0;
		for ( utility::vector1< utility::vector1< core::Real > >::iterator it=b_ang[iattempt].begin(); it!=b_ang[iattempt].end(); ++it ) {
			++solcount;
			if ( solutions_to_delete[solcount] ) b_ang[iattempt].erase( it );
		}
		solcount=0;
		for ( utility::vector1< utility::vector1< core::Real > >::iterator it=b_len[iattempt].begin(); it!=b_len[iattempt].end(); ++it ) {
			++solcount;
