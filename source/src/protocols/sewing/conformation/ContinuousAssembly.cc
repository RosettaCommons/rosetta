...
ContinuousAssembly::append_model(
	...
	if ( mobile_match_segments.find(model.segments_[1]) == mobile_match_segments.end() ) {
		...
		add_these_to_front.push_back(model.segments_[1]);
		add_these_to_front.push_back(model.segments_[2]);
		segments_.insert(segments_.begin(), add_these_to_front.begin(), add_these_to_front.end());

		//Update all_segments vector
		utility::vector1<SewSegment> model_seg_vec1st;
		model_seg_vec1st.push_back(model.segments_[1]);
		utility::vector1<SewSegment> model_seg_vec2nd;
		model_seg_vec2nd.push_back(model.segments_[2]);
		all_segments_.insert(all_segments_.begin(), model_seg_vec2nd);
		all_segments_.insert(all_segments_.begin(), model_seg_vec1st);
		if ( TR.Debug.visible() ) {
			TR.Debug << "Model added to the N-terminal" << std::endl;
		}
	} else if ( mobile_match_segments.find(model.segments_.back()) == mobile_match_segments.end() ) {
		//Appending to c_terminus
		segments_.push_back(model.segments_[2]);
		segments_.push_back(model.segments_[3]);

		//Update all_segments vector
		utility::vector1<SewSegment> model_seg_vec2;
		model_seg_vec2.push_back(model.segments_[2]);
		all_segments_.push_back(model_seg_vec2);
		utility::vector1<SewSegment> model_seg_vec3;
		model_seg_vec3.push_back(model.segments_[3]);
		all_segments_.push_back(model_seg_vec3);
		if ( TR.Debug.visible() ) {
