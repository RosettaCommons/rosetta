...
SmartAssembly::get_abbreviated_assembly(){
	utility::vector1< BasisPair > bp_list;
	SmartSegmentOP current_segment = this->get_n_terminal_segment();
	while ( current_segment ) {
		if ( current_segment->is_chimaeric() ) {
			Basis n_basis;
			Basis c_basis;
			BasisPair current_bp;
			if ( current_segment->get_n_terminal_parent()->is_chimaeric() ) {
				// add n_term parent basis pair first
				bp_list.push_back( current_segment->get_n_terminal_parent()->get_basis_pair() );
				// then make basis pair for c_term
				current_bp = current_segment->get_basis_pair();
				if ( current_segment->get_basis_pair().first.segment_id() == current_segment->get_n_terminal_parent()->get_segment_id() ) {
					//first basis is n_terminal
					current_bp.first.segment_id( current_segment->get_n_terminal_parent()->get_c_terminal_parent()->get_segment_id() );
				} else {
					//second basis is n_terminal
					current_bp.second.segment_id( current_segment->get_n_terminal_parent()->get_c_terminal_parent()->get_segment_id() );
				}
				// add c_term basis pair
				bp_list.push_back( current_bp );
			} else {
				if ( current_segment->get_c_terminal_parent()->is_chimaeric() ) {
					current_bp = current_segment->get_basis_pair();
					int new_basis_res;
					if ( current_segment->get_basis_pair().first.segment_id() == current_segment->get_c_terminal_parent()->get_segment_id() ) {
						//first basis is c_terminal
						current_bp.first.segment_id( current_segment->get_c_terminal_parent()->get_n_terminal_parent()->get_segment_id() );
						//second basis is n_terminal
						new_basis_res = current_bp.second.resnum() - current_bp.first.resnum();
					} else {
						//second basis is c_terminal
						current_bp.second.segment_id( current_segment->get_c_terminal_parent()->get_n_terminal_parent()->get_segment_id() );
						//first basis is n_terminal
						new_basis_res = current_bp.first.resnum() - current_bp.second.resnum();
					}
					bp_list.push_back( current_bp );
					current_bp = current_segment->get_c_terminal_parent()->get_basis_pair();
					if ( current_bp.first.segment_id() == current_segment->get_c_terminal_parent()->get_n_terminal_parent()->get_segment_id() ) {
						//first basis is n_terminal
						new_basis_res = new_basis_res + current_bp.first.resnum();
						if ( !new_basis_res ) {
							utility_exit_with_message( "basis pair calculated is invalid! " + utility::to_string( new_basis_res ) );
						}
						current_bp.first.resnum( core::Size( new_basis_res ) );
					} else {
						//second basis is n_terminal
						new_basis_res = new_basis_res + current_bp.second.resnum();
						if ( !new_basis_res ) {
							utility_exit_with_message( "basis pair calculated is invalid! " + utility::to_string( new_basis_res ) );
						}
						current_bp.second.resnum( core::Size( new_basis_res ) );
					}
					bp_list.push_back( current_bp );
				} else {
					bp_list.push_back( current_segment->get_basis_pair() );
				}
			}
		}
		current_segment = current_segment->get_c_terminal_neighbor();
...
void
SmartAssembly::revert_to_abbreviated_assembly( utility::vector1< BasisPair > bp_list ){

	TR << "REVERTING ASSEMBLY" << std::endl;
	TR << this->get_comprehensive_assembly_string() << std::endl;
	if ( bp_list.size()>0 ) {}
	// //first, delete back to starting segments
	// TR << "DELETING OLD ASSEMBLY" << std::endl;
	// //deleteing from n_terminus first
	// TR << "Deleting from n term" << std::endl;
	// SmartSegmentOP current_segment = this->get_n_terminal_segment();
	// bool n_term = true;
	// while( !current_segment->is_vital() && current_segment != nullptr ){
	//  this->delete_segment( n_term );
	//  current_segment = this->get_n_terminal_segment();
	//  TR << this->get_comprehensive_forward_assembly() << std::endl;
	//  TR << this->get_comprehensive_reverse_assembly() << std::endl;
	// }
	// //deleting from c_terminus
	// TR << "Deleting from c term" << std::endl;
	// current_segment = this->get_c_terminal_segment();
	// n_term = false;
	// while( !current_segment->is_vital() && current_segment != nullptr ){
	//  this->delete_segment( n_term );
	//  current_segment = this->get_c_terminal_segment();
	...
	// }
	//
	// TR << "Finished Deleting old assembly: " << std::endl;
	// TR << this->get_comprehensive_forward_assembly() << std::endl;
	// TR << this->get_comprehensive_reverse_assembly() << std::endl;

	//create new assembly
	// TR << "RECOVERING ASSEMBLY FROM SAVE" << std::endl;
	// //recovering while preserving vital segments
	// if( this->get_n_terminal_segment() ){
	//  //Step 1: find the n_terminal starting point
	//  utility::vector1< BasisPair>::iterator bp_iter = bp_list.begin();
	//  bool found_vital = false;
	//  bool at_least_one_vital_terminus = false;
	//  while( !found_vital && bp_iter != bp_list.end()){
	//  // if( bp_iter == bp_list.end() ){
	//  //  TR << "Hit end of basis pair list before finding vital segment match!!!" << std::endl;
	//  //  utility_exit_with_message( "Couldn't recover saved assembly" );
	//  // }
	//   if( bp_iter->first.segment_id() == this->get_n_terminal_segment()->get_segment_id() || bp_iter->second.segment_id() == this->get_n_terminal_segment()->get_segment_id() ){
	//    found_vital = true;
	//    at_least_one_vital_terminus = true;
	//   }
	//   else{
	//    ++bp_iter;
	//   }
	//  }
	//  //Step2:find the c_terminal starting point
	//  utility::vector1< BasisPair>::reverse_iterator bp_riter = bp_list.rbegin();
	//  found_vital = false;
	//  while( !found_vital && bp_riter != bp_list.rend()){
	// //  if( bp_riter == bp_list.rend() ){
	// //   TR << "Hit end of basis pair list before finding vital segment match!!!" << std::endl;
	// //   utility_exit_with_message( "Couldn't recover saved assembly" );
	// //  }
	//   if( bp_riter->first.segment_id() == this->get_c_terminal_segment()->get_segment_id() || bp_riter->second.segment_id() == this->get_c_terminal_segment()->get_segment_id() ){
	//    found_vital = true;
	//    at_least_one_vital_terminus = true;
	//   }
	//   else{
	...
	//   }
	//  }
	//  if(!at_least_one_vital_terminus){
	//   TR << "Hit end of basis pair list before finding vital segment match!!!" << std::endl;
	//   utility_exit_with_message( "Couldn't recover saved assembly" );
	//  }
	//  //Step 3: re-add segments to n_terminal side
	//  n_term = true;
	//  while( bp_iter != bp_list.begin() && bp_iter != bp_list.end()){
	//   this->add_segment( n_term, bp_iter->second.segment_id(), bp_iter->first.resnum(), bp_iter->second.resnum() );
	//   --bp_iter;
	//  }
	//  this->add_segment( n_term, bp_iter->second.segment_id(), bp_iter->first.resnum(), bp_iter->second.resnum() );
	//
	//  //Step 4: re-add segments to c_terminal side
	//  n_term = false;
	//  while( bp_riter != bp_list.rbegin() && bp_riter != bp_list.rend()){
	//   this->add_segment( n_term, bp_riter->second.segment_id(), bp_riter->first.resnum(), bp_riter->second.resnum() );
	//   --bp_riter;
	//  }
	//  this->add_segment( n_term, bp_riter->second.segment_id(), bp_riter->first.resnum(), bp_riter->second.resnum() );
	// }
	// else {
	//  //recreating assembly from scratch
	//  //Step 1: set starting segment
	//  this->set_starting_segment( segment_vector_->at( bp_list.at( 1 ).first.segment_id() ) );
	//  for( utility::vector1< BasisPair >::iterator bp_iter = bp_list.begin(); bp_iter != bp_list.end(); ++bp_iter ){
	//   this->add_segment( n_term, bp_iter->second.segment_id(), bp_iter->first.resnum(), bp_iter->second.resnum() );
	//  }
	// }
}
