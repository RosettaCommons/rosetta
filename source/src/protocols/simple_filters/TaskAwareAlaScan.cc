...
#include <protocols/simple_filters/TaskAwareAlaScan.hh>
...
TaskAwareAlaScan::report( std::ostream & out, core::pose::Pose const & const_pose ) const
{
  ...
  for (core::Size resi = 1; resi <= pose.n_residue(); resi++) {
		...
		if( task->pack_residue( resi ) ) {
			...
			// Output to pdb file
			if ( write2pdb() ) { write_to_pdb( pose, resi, res_type, output_ddG ); }
			// Output to tracer
			...
			out<<" "<<res_type<<" "<<output_resi<<" "<<chain<<" : "<< ObjexxFCL::format::F (9,4,output_ddG)<<std::endl;
		}
	}
	out<<std::endl;
}

void TaskAwareAlaScan::write_to_pdb( core::pose::Pose const & pose, core::Size const & residue, std::string const & residue_name, core::Real const & ddG ) const
{
  ...
  std::string output_string = filter_name + " " + user_name + ": " + residue_name + ObjexxFCL::string_of(output_resi) + mut_name + " = " + ObjexxFCL::format::F (9,4,ddG);
