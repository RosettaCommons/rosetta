...
namespace pockets {
  ...
  class FingerprintBase : public utility::pointer::ReferenceCount {
    ...
    friend class MultiFingerprintMultifunc;
  ...
  public:
    ...
    std::list< spherical_coor_triplet > const & triplet_fingerprint_data() const { return triplet_fingerprint_data_; };
		...
		std::list< NonPlaidFingerprint > const & npf_list() const { return npf_list_;};
		std::list< PlaidFingerprint > const & pf_list() const { return pf_list_;};
  ...
  protected:
    ...
    numeric::xyzVector<core::Real> CoM_;
		std::list< NonPlaidFingerprint > npf_list_;
		std::list< PlaidFingerprint > pf_list_;
    std::list< spherical_coor_triplet > triplet_fingerprint_data_;
    std::list< numeric::xyzVector<core::Real> > multi_origin_list_;
  ...
  public:
    ...
    core::Real multi_fp_compare( FingerprintBase & fp, core::Real const & missing_point_weight, core::Real const & steric_weight, core::Real const & extra_point_weight ) const;
    ...
    core::Size compute_ligand_nconformers() const { return compute_ligand_nconformers(pose_); };
		...
		void move_ligand_and_update_rhos_for_multiple_origin( FingerprintBase & fp, numeric::xyzVector<core::Real> const & CoM_offset, core::Real const & angle1_offset, core::Real const & angle2_offset, core::Real const & angle3_offset, core::Size const & conformer );
  ...
  private:
    ...
    core::conformation::ResidueCOP select_conf_and_rotate_ligand_( core::Real const & angle1_offset, core::Real const & angle2_offset, core::Real const & angle3_offset, core::Size const & conformer );
	...
	class FingerprintManager : public FingerprintBase {
	...
	public:
		...
		numeric::xyzVector<core::Size> esp_dim_;
		numeric::xyzVector<core::Real> pocket_com_;
		std::list< triplet_and_originnum > const & triplet_originnum_data() const { return triplet_originnum_data_; };
    std::list< triplet_and_originnum > triplet_originnum_data_;
    std::list< numeric::xyzVector<core::Real> > origin_list_;
    void initialize(FingerprintBase & fp);

#ifdef USEOPENCL
		void gpu_setup( core::Real const & missing_point_weight, core::Real const & steric_weight, core::Real const & extra_point_weight, int & num_particles, PlaidFingerprint & pf, core::Real const & electrostatics_weight );
		int gpu_calculate_particle_scores(core::optimization::ParticleOPs & particles, std::vector<basic::gpu::float4> &particles_rotation_offset, std::vector<basic::gpu::float4> &particles_translation_offset, std::vector<basic::gpu::float4> &eatoms);
    void gpu_setup_rays(FingerprintBase & fp);
		void gpu_setup_atomcoords(PlaidFingerprint & pf);
    void gpu_setup_espGrid(NonPlaidFingerprint const & npf);
#endif
