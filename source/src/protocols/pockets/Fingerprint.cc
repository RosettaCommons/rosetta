...
void FingerprintBase::print_to_file(std::string const & output_filename) const {
  ...
  out_stream.open(output_filename, std::ios::out);
	...
	for (std::list< NonPlaidFingerprint >::const_iterator fi = npf_list().begin(); fi != npf_list().end(); ++fi) {
		out_stream<<"/ORI/"<<std::fixed<<std::setprecision(4)<< fi->origin_.x() << "\t" <<std::fixed<<std::setprecision(4)<< fi->origin_.y() << "\t"<<std::fixed<<std::setprecision(4)<< fi->origin_.z() <<std::endl;
		for (std::list<spherical_coor_triplet>::const_iterator gi = fi->triplet_fingerprint_data_.begin(); gi != fi->triplet_fingerprint_data_.end(); ++gi) {
			out_stream<< gi->phi << "\t" <<gi->psi << "\t"<< gi->rho <<std::endl;
		}
		out_stream<< "TER" <<std::endl;
	}
  out_stream.close();
  out_stream.clear();
...
void FingerprintBase::print_to_pdb(std::string const & output_pdbname, numeric::xyzVector<core::Real> const & translation) const {
  ...
  out_stream.open(output_pdbname, std::ios::out);
	...
	for (std::list< NonPlaidFingerprint >::const_iterator fi = npf_list().begin(); fi != npf_list().end(); ++fi) {
		...
		out_stream<<"HETATM   "<<std::setw(2)<<1<<"  C   ORI X   0    "<<std::setw(8)<<std::fixed<<std::setprecision(3)<<fi->origin_.x()+translation.x()<<std::setw(8)<<std::fixed<<std::setprecision(3)<<fi->origin_.y()+translation.y()<<std::setw(8)<<std::fixed<<std::setprecision(3)<<fi->origin_.z()+translation.z()<<std::endl;

		for (std::list<spherical_coor_triplet>::const_iterator pd = fi->triplet_fingerprint_data_.begin(); pd != fi->triplet_fingerprint_data_.end(); ++pd) {
...
void NonPlaidFingerprint::setup_from_EggshellGrid() {
  ...
  if (option[ OptionKeys::fingerprint::multiple_origin ]()){
		...
		for (std::list<numeric::xyzVector<core::Real> >::const_iterator i_mori = multi_origin_list_.begin(); i_mori != multi_origin_list_.end(); ++i_mori) {
			...
			//for eggshell
			for (std::list< numeric::xyzVector<core::Real> >::const_iterator pd = eggshell_list_.begin(); pd != eggshell_list_.end(); ++pd) {
				spherical_coor_triplet best_triplet;
...
#ifdef USEOPENCL
	void FingerprintManager::gpu_setup( core::Real const & missing_point_weight, core::Real const & steric_weight, core::Real const & extra_point_weight, int & num_particles, PlaidFingerprint & pf, core::Real const & electrostatics_weight ) {
		...
		core::Size const ligand_natoms = pf.compute_ligand_natoms();
		...
		float elec_weights[4] = {electrostatics_weight,num_particles,ligand_natoms,0};
	...
	void FingerprintManager::gpu_setup_atomcoords(PlaidFingerprint & pf) {
		if(!gpu_.use()) return;
		...
		core::Size const ligand_natoms = pf.compute_ligand_natoms();
		gpu_memory_.num_ligatoms = ligand_natoms;

		std::vector<basic::gpu::float4> atomcoords;
		std::vector<basic::gpu::float4> ligCoM;
		atomcoords.clear();
		ligCoM.clear();
		for ( int c = 0; c < nconformers; ++c ) {
			...
			for (Size i = 1; i <= ligand_natoms; ++i) {
				core::Size const curr_array_inx = ( ligand_natoms * c ) + ( i-1 );
				basic::gpu::float4 atomcoord = { ligand_rsd->atom(i).xyz()(1), ligand_rsd->atom(i).xyz()(2), ligand_rsd->atom(i).xyz()(3), ligand_rsd->atom_type(i).lj_radius() };
				atomcoords.push_back(atomcoord);
				ligand_com.x() += ligand_rsd->atom(i).xyz()(1);
				ligand_com.y() += ligand_rsd->atom(i).xyz()(2);
				ligand_com.z() += ligand_rsd->atom(i).xyz()(3);
			}
			ligand_com /= ligand_natoms;
			basic::gpu::float4 lig_conf_com = { ligand_com.x(), ligand_com.y(), ligand_com.z(), ligand_natoms };
		...
		gpu_.AllocateMemoryReuse(gpu_memory_.atomcoords, gpu_memory_.atomcoords_size, sizeof(basic::gpu::float4) * atomcoords.size());
		gpu_.WriteData(gpu_memory_.atomcoords, &atomcoords[0], gpu_memory_.atomcoords_size);

		return;

	}

	void FingerprintManager::gpu_setup_rays(FingerprintBase & fp) {

		if(!gpu_.use()) return;

		std::vector<basic::gpu::float4> rays;
		std::vector<basic::gpu::float4> RAYorigins;
		core::Size origin_index = 0;
		for (std::list< NonPlaidFingerprint >::const_iterator fi = fp.npf_list().begin(); fi != fp.npf_list().end(); ++fi) {
			basic::gpu::float4 ray_origin = { fi->origin().x(), fi->origin().y(), fi->origin().z(), 0.0 };
			RAYorigins.push_back(ray_origin);
			for (std::list<spherical_coor_triplet>::const_iterator gi = fi->triplet_fingerprint_data().begin(); gi != fi->triplet_fingerprint_data().end(); ++gi) {
				basic::gpu::float4 ray = { gi->phi, gi->psi, gi->rho, origin_index };
				rays.push_back(ray);
			}
			++origin_index;
		}
	...
	void FingerprintManager::gpu_setup_espGrid(NonPlaidFingerprint const & npf) {
		...
		std::vector<basic::gpu::float4> gpu_espGrid;
		std::vector<basic::gpu::float4> gpu_typGrid;

		for (Size ix=0; ix<npf.esp_dim_.x(); ++ix){
			for (Size iy=0; iy<npf.esp_dim_.y(); ++iy){
				for (Size iz=0; iz<npf.esp_dim_.z(); ++iz){
					basic::gpu::float4 esp = {npf.espGrid_[ix][iy][iz],0,0,0};
					basic::gpu::float4 typ = {npf.typGrid_[ix][iy][iz],0,0,0};
		...
		unsigned int gpu_espGrid_size = sizeof(basic::gpu::float4) * gpu_espGrid.size();
		...
		gpu_memory_.num_gpu_espGrid = gpu_espGrid.size();
		unsigned int gpu_typGrid_size = sizeof(basic::gpu::float4) * gpu_typGrid.size();
		...
		gpu_memory_.num_gpu_typGrid = gpu_typGrid.size();
	}

	int FingerprintManager::gpu_calculate_particle_scores( core::optimization::ParticleOPs & particles, std::vector<basic::gpu::float4> &particles_rotation_offset, std::vector<basic::gpu::float4> &particles_translation_offset, std::vector<basic::gpu::float4> &eatoms){
		...
		// Allocate GPU memory (reused) and copy ray coordinates over to GPU
		unsigned int eatoms_size = sizeof(basic::gpu::float4) * eatoms.size();
		gpu_.AllocateMemoryReuse(gpu_memory_.eatoms, gpu_memory_.eatoms_size, eatoms_size);
		gpu_.WriteData(gpu_memory_.eatoms, &eatoms[0], eatoms_size);
		gpu_memory_.num_eatoms = eatoms.size();
		gpu_.AllocateMemoryReuse(gpu_memory_.elec_scores, gpu_memory_.elec_scores_size, sizeof(float) * gpu_memory_.num_particles * gpu_memory_.num_eatoms);

		// Execute kernels
		if(!gpu_.ExecuteKernel("get_ray_score", gpu_memory_.num_rays, gpu_memory_.num_rays, 64,
													 GPU_DEVMEM, gpu_memory_.rays,
													 GPU_DEVMEM, gpu_memory_.atomcoords,
		...
		if(!gpu_.ExecuteKernel("calc_electrostatics", gpu_memory_.num_eatoms, gpu_memory_.num_eatoms, 64,
                           GPU_DEVMEM, gpu_memory_.eatoms,
                           GPU_DEVMEM, gpu_memory_.elec_scores,
													 ...
													 GPU_IN | GPU_INT, gpu_memory_.num_eatoms,
                           NULL)) {
			std::cout << "Failed to launch kernel: " << gpu_.lastErrorStr() << std::endl;
			return 0;
		}
		...
		if(!gpu_.ExecuteKernel("Get_scores", gpu_memory_.num_particles, gpu_memory_.num_particles, 64,
													 ...
													 GPU_IN | GPU_INT, gpu_memory_.num_ligatoms,
													 GPU_IN | GPU_INT, gpu_memory_.num_rays,
...
void FingerprintManager::initialize(FingerprintBase & fp){

	pocket_com_ = fp.CoM();
	//for reading into GPU, create a single data structre for all rays
  triplet_and_originnum new_triplet_originnum;
	triplet_originnum_data_.clear();
	numeric::xyzVector<core::Real> new_multi_origin;
	origin_list_.clear();

	core::Size orinum =0 ;
	for (std::list< NonPlaidFingerprint >::const_iterator fi = fp.npf_list().begin(); fi != fp.npf_list().end(); ++fi) {
		origin_list_.push_back(origin_);
		++orinum;
		for (std::list<spherical_coor_triplet>::const_iterator gi = fi->triplet_fingerprint_data().begin(); gi != fi->triplet_fingerprint_data().end(); ++gi) {
			new_triplet_originnum.phi = gi->phi;
			new_triplet_originnum.psi = gi->psi;
			new_triplet_originnum.rho = gi->rho;
			new_triplet_originnum.originnum = orinum;
			triplet_originnum_data_.push_back(new_triplet_originnum);
		}
	}
...
void PlaidFingerprint::move_ligand_and_update_rhos_for_multiple_origin( FingerprintBase & fp, numeric::xyzVector<core::Real> const & CoM_offset, core::Real const & angle1_offset, core::Real const & angle2_offset, core::Real const & angle3_offset, core::Size const & conformer ) {

	PlaidFingerprint pair_list(pose_, fp);
	pf_list_.clear();
	for (std::list< NonPlaidFingerprint >::const_iterator pro = fp.npf_list().begin(); pro != fp.npf_list().end(); ++pro) {
    NonPlaidFingerprint tmp = *pro;
		...
		origin_ = tmp.origin() + CoM_ - fp.CoM();
		origin_ += CoM_offset;
		//		std::cout<<"pocket com : "<<fp.CoM().x()<<" "<<fp.CoM().y()<<" "<<fp.CoM().z()<<std::endl;
		//std::cout<<"ligand com : "<<CoM_.x()<<" "<<CoM_.y()<<" "<<CoM_.z()<<std::endl;
		//std::cout<<"com offset : "<<CoM_offset.x()<<" "<<CoM_offset.y()<<" "<<CoM_offset.z()<<std::endl;
		//std::cout<<"npf origin : "<<tmp.origin().x()<<" "<<tmp.origin().y()<<" "<<tmp.origin().z()<<std::endl;
		...
		core::conformation::ResidueCOP curr_ligand_rsd = new core::conformation::Residue( tmp_pose->conformation().residue(lig_res_num) );
	...
	std::list< spherical_coor_triplet > tmp_triplet;
		tmp_triplet.clear();
  ...
  for (std::list<spherical_coor_triplet>::const_iterator ni = tmp.triplet_fingerprint_data().begin(); ni != tmp.triplet_fingerprint_data().end(); ++ni) {

		//std::cout<<ni->phi<<" "<<ni->psi<<" "<<ni->rho<<" "<<pro->origin().x()<<" "<<pro->origin().y()<<" "<<pro->origin().z()<<std::endl;
    ...
    tmp_triplet.push_back(new_triplet);
	}

	//done test UR

	//update_rhos_(tmp, ligand_rsd, false );
		pair_list.origin_ = origin_;
		pair_list.triplet_fingerprint_data_ = tmp_triplet;
		pf_list_.push_back(pair_list);
	}
...
core::conformation::ResidueCOP PlaidFingerprint::select_conf_and_rotate_ligand_( core::Real const & angle1_offset, core::Real const & angle2_offset, core::Real const & angle3_offset, core::Size const & conformer ) {
...
core::Real PlaidFingerprint::multi_fp_compare( FingerprintBase & fp, core::Real const & missing_point_weight, core::Real const & steric_weight, core::Real const & extra_point_weight ) const {

  core::Real Total_score = 0;
  core::Size num_rays = 0;
	core::Size missing_ligand_rays = 0;
	core::Size missing_pocket_rays = 0;
  core::Real underpack_dist = 0;
  core::Real steric_dist = 0;
  ...
  bool square  = option[ OptionKeys::fingerprint::square_score ]();
	...
	for (std::list< NonPlaidFingerprint >::const_iterator pro = fp.npf_list().begin(); pro != fp.npf_list().end(); ++pro) {
		++i;
		core::Size j = 0;
		for (std::list< PlaidFingerprint >::const_iterator lig = pf_list().begin(); lig != pf_list().end(); ++lig) {
			++j;
			core::Real score = 0;
			if(i == j){
				for (std::list<spherical_coor_triplet>::const_iterator pi = pro->triplet_fingerprint_data().begin(), li = lig->triplet_fingerprint_data_.begin(); (pi != pro->triplet_fingerprint_data().end()) && (li != lig->triplet_fingerprint_data_.end()); ++pi, ++li) {
					// jk note: these are no longer necessarily true, since we alter the Plaid one by shifting up/down by 2*pi
					// these are useful asserts though, it's worth thinking about how to make them valid again...
					// assert( std::abs( pi->phi - li->phi ) < 0.001 );
					// assert( std::abs( pi->psi - li->psi ) < 0.001 );
					...
					if ( (li->rho < 0.001) && (pi->rho < 0.001) ) {
						...
						++missing_ligand_rays;
						score += missing_point_weight;
					} else if (pi->rho < 0.001 ) {
						score += extra_point_weight;
						++missing_pocket_rays;
					} else {
						core::Real dist_deviation = std::abs( pi->rho - li->rho );
						if (square){
						...
						score += dist_deviation;
					}
					num_rays++;
				}
				Total_score += score;
				//				std::cout<< "multi_fp_compare : "<< i <<" "<< j << " "<< score <<" "<< Total_score <<" "<<num_rays<<std::endl;
			}
		}
	}
	...
	if (get_darc_components){
		std::cout<<"OPT "<<underpack_dist<<" "<<steric_dist<< " "<<missing_ligand_rays<<" "<<missing_pocket_rays<<" "<<num_rays;
	}
...
core::Real NonPlaidFingerprint::get_electrostatics_energy(core::pose::Pose const & ligand_pose){
  ...
  using namespace basic::options;
	Size ligand_total_atoms;
  if (option[ OptionKeys::fingerprint::include_hydrogens ]()){
    ligand_total_atoms = ligand_rsd.natoms();
  } else {
    ligand_total_atoms = ligand_rsd.nheavyatoms();
  }
...
core::Real NonPlaidFingerprint::get_interpolated_esp_energy_with_type(numeric::xyzVector<core::Real> const & ligand_atom, core::Real const & atom_charge){
	...
	core::Real const esp_wt(option[ OptionKeys::fingerprint::esp_weight ]);
	core::Real const esp_prot_wt(option[ OptionKeys::fingerprint::esp_protein_wt ]);
	core::Real const esp_surf_wt(option[ OptionKeys::fingerprint::esp_surface_wt ]);
