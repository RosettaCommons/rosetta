...
void DarcParticleSwarmMinimizer::score_all_particles(core::optimization::Multifunc & , core::optimization::ParticleOPs & particles) {
  ...
  std::vector<basic::gpu::float4> eatoms(ligand_natoms_ * N);
  ...
  for (Size j = 1; j <= N; ++j) {
    ...
    core::conformation::ResidueCOP ligand_rsd = pfp_.select_conf_and_move_ligand_( nfp_, origin_offset, vars[4], vars[5], vars[6], conformer );
		...
		//fill_atom_arrays_multiple_origin_(j-1, ligand_rsd_new, atoms, atom_maxmin_phipsi, ligand_maxmin_phipsi, origin_offset);//(j-1, ligand_rsd, N, k-1 );
		...
		//particles_num++;
		core::Size const curr_array_inx = j-1;
    ...
    particles_translation_offset[j-1].w = conformer;
...
#ifdef USEOPENCL
	if(!fpmanager_.gpu_calculate_particle_scores(particles, particles_rotation_offset, particles_translation_offset, eatoms)) {
#endif
		...
		Size dim_x = nfp_.esp_dim_.x();
		Size dim_y = nfp_.esp_dim_.y();
		Size dim_z = nfp_.esp_dim_.z();
		...
		for (core::Size j = 1; j <= N; ++j) {
			...
			if (option[ OptionKeys::fingerprint::darc_shape_only ].user()){
				...
				particle_score = pfp_.multi_fp_compare( nfp_, missing_pt_, steric_, extra_pt_ );
				//			std::cout<<"particle_darc_score : "<<particle_score<<std::endl;
			}
				...
				//				std::cout<<"darc_elsts_score : "<<particle_score<<std::endl;
			}
			else {
				pfp_.move_ligand_and_update_rhos_for_multiple_origin( nfp_, origin_offset, vars[4], vars[5], vars[6], conformer );
				core::Real particle_darc_score = pfp_.multi_fp_compare( nfp_, missing_pt_, steric_, extra_pt_ );
				core::Real particle_elsts_score = DarcPSO_elsts_score_( j-1, dim_x, dim_y, dim_z, mid_x, mid_y, mid_z, spacing, espGrid, typGrid, eatoms );
				particle_score = particle_darc_score + particle_elsts_score;
			}
			//std::cout<<"particle_score : "<<particle_score<<std::endl;
  ...
  for (Size i = 1; i <= ligand_natoms_; ++i) {
	...
	void DarcParticleSwarmMinimizer::fill_atom_arrays_multiple_origin_( core::Size particle_inx, core::conformation::ResidueCOP ligand_rsd, std::vector<basic::gpu::float4> & atoms, std::vector<basic::gpu::float4> & atom_maxmin_phipsi, std::vector<basic::gpu::float4> & ligand_maxmin_phipsi, numeric::xyzVector<core::Real> const & origin_offset ) {
  using namespace basic::options;
  core::Real const radius_scale = option[ OptionKeys::fingerprint::atom_radius_scale ];
  core::Real const atom_buffer = option[ OptionKeys::fingerprint::atom_radius_buffer ];
  ...
  for (Size i = 1; i <= ligand_natoms_; ++i) {
    ...
    numeric::xyzVector<core::Real> this_atomcoors = ligand_rsd->atom(i).xyz() - origin_offset;
    core::Real const this_atom_radius = ( ligand_rsd->atom_type(i).lj_radius() - atom_buffer ) * radius_scale;
    atoms[curr_array_inx].x = this_atomcoors.x();
    atoms[curr_array_inx].y = this_atomcoors.y();
    atoms[curr_array_inx].z = this_atomcoors.z();
...
core::Real
DarcParticleSwarmMinimizer::DarcPSO_fp_compare_multiple_origin_(
		core::Size particle_inx,
		core::Real const & missing_point_weight,
		core::Real const & steric_weight,
		core::Real const & extra_point_weight,
	 ...
	 for (std::list< NonPlaidFingerprint >::const_iterator fi = nfp_.npf_list().begin(); fi != nfp_.npf_list().end(); ++fi) {
		...
		core::Real curr_phi = gi->phi;
		core::Real curr_psi = gi->psi;
		core::Real best_rho_sq(9999.);
		//    core::Size best_intersecting_atom(0);
		for (core::Size i = 1, i_end = ligand_natoms_; i <= i_end; ++i) {
			core::Size const curr_array_inx = ( ligand_natoms_ * particle_inx ) + ( i-1 );
      ...
      if ( curr_psi > atom_maxmin_phipsi[curr_array_inx].y ) continue;
		...
		if ( (plaid_rho > 9998.) && (gi->rho > 0.001) ) {
			Total_score += missing_point_weight;
			num_rays++;
		}
		if ( (plaid_rho < 9999.) && (gi->rho < 0.001 ) ) {
			Total_score += extra_point_weight;
			num_rays++;
		}
		if ( (plaid_rho < 9999.) && (gi->rho > 0.001 ) ) {
			core::Real dist_deviation = ( plaid_rho - gi->rho );
			if (dist_deviation < 0.0) dist_deviation = ( gi->rho - plaid_rho ) * steric_weight;
			Total_score += dist_deviation;
			num_rays++;
		}
		 }
