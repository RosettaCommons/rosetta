...
/// @file LegacyLoophashAssemblyMover.cc
///
...
static THREAD_LOCAL basic::Tracer TR( "protocols.legacy_sewing.LegacyLoophashAssemblyMover" );

////////////////////////////////////////////////////////////////////////////////////
///////////////////////////  Boiler Plate Code   ///////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////
protocols::moves::MoverOP
LegacyLoophashAssemblyMoverCreator::create_mover() const
{
	return new LegacyLoophashAssemblyMover;
}

std::string
LegacyLoophashAssemblyMoverCreator::keyname() const
{
	return LegacyLoophashAssemblyMoverCreator::mover_name();
}

std::string
LegacyLoophashAssemblyMoverCreator::mover_name()
{
	return "LegacyLoophashAssemblyMover";
}
...
std::string
LegacyLoophashAssemblyMover::get_name() const {
	return "LegacyLoophashAssemblyMover";
}

/////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////  LegacyLoophashAssemblyMover functions   //////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////

LegacyLoophashAssemblyMover::LegacyLoophashAssemblyMover():
	LegacyAssemblyMover(),
	lh_library_(0),
...
void
LegacyLoophashAssemblyMover::init(){
...
bool
LegacyLoophashAssemblyMover::complete_assembly(
	AssemblyOP & assembly
){

	//First filter using base-class filters
	bool complete = LegacyAssemblyMover::complete_assembly(assembly);
	if ( !complete ) {
		return false;
	}

	//Now check for connectability and add loops
	if ( ! basic::options::option[ basic::options::OptionKeys::legacy_sewing::skip_loop_generation ].value() ) {
...
void
LegacyLoophashAssemblyMover::output_stats(
...
bool
LegacyLoophashAssemblyMover::rearrange_assembly(
	AssemblyOP & assembly
) const {
	...
	for ( core::Size i=1; i<=orders.size(); ++i ) {
		...
		//if((int)num_lh_frags >= basic::options::option[ basic::options::OptionKeys::legacy_sewing::min_lh_fragments].value()) {
...
core::Size
LegacyLoophashAssemblyMover::count_loophash_fragments(
	AssemblyOP const assembly,
...
LegacyLoophashAssemblyMover::add_loophash_segments(
	AssemblyOP & assembly,
	core::pose::Pose & pose
) const{
...
LegacyLoophashAssemblyMover::add_single_loop(
	core::pose::Pose & pose,
...
void
LegacyLoophashAssemblyMover::trim_bb_segs(
	core::pose::Pose const & pose,
	core::Size loop_anchor,
...
LegacyLoophashAssemblyMover::BackboneSegments
LegacyLoophashAssemblyMover::get_backbone_segments(
	core::pose::Pose & pose,
...
LegacyLoophashAssemblyMover::build_loop_pose(
	core::pose::Pose const & pose,
	core::Size loop_anchor,
	core::Size /*n_segment_start*/,
	core::Size /*c_segment_end*/,
