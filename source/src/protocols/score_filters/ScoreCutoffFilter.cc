...
ScoreCutoffFilter::output_residue_pair_energies( std::ostream & ostr, core::pose::Pose const & pose ) const
{
	...
	for ( core::Size res1 = 1; res1 <= pose.size(); ++res1 ) {
		std::string res1name("");
		if ( pdb_numbering_ ) {
			res1name = std::to_string( pose.residue_type( res1 ).name1() ) + "_" + pose.pdb_info()->chain( res1 ) + std::to_string( pose.pdb_info()->number( res1 ) );
		} else {
		...
		for ( std::map<  core::Size, EnergyMap >::const_iterator map_it = upper_interactions.begin(); map_it != upper_interactions.end(); ++map_it ) {
			...
			std::string res2name("");
			if ( pdb_numbering_ ) {
				res2name = std::to_string( pose.residue_type( res2 ).name1() ) + "_" + pose.pdb_info()->chain( res2 ) + std::to_string( pose.pdb_info()->number( res2 ) );
			} else {
