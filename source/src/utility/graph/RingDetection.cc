...
// const util::SiPtr< const util::ObjectInterface> RingDetection::s_Instance
//(
//GetObjectInstances().AddInstance( new RingDetection())
// );
...
RingDetection::RingDetection(const core::chemical::LightWeightResidueGraph &graph) :
...
//! @brief initialize paths_ with all edges
void RingDetection::Initialize( const core::chemical::LightWeightResidueGraph &graph )
{
	...
	core::chemical::lwrg_vd_iter v_start, v_end;
	boost::tie(v_start, v_end) = boost::vertices(graph);
	size_t index(0);
	for(core::chemical::lwrg_vd_iter it = v_start; it != v_end; ++it){
	...
	)
	{
		...
		//which is bonded to
		core::chemical::lwrg_out_edge_iter oe_start, oe_end;
		...
		for(core::chemical::lwrg_out_edge_iter itr_edge_target = oe_start; itr_edge_target != oe_end; ++itr_edge_target){
			core::chemical::lwrg_VD target = boost::target(*itr_edge_target, graph); //get the vertex that is the target of edge (boost::source would return original vertex)
...
void RingDetection::Remove( const size_t vertex)
{
	// BCL_MessageDbg
	//(
...
std::vector< size_t> RingDetection::CombinePaths(
...
void RingDetection::SetupAdjacencyVector
(
...
size_t RingDetection::LengthOfSmallestCycleWithVertex
(
		const core::chemical::LightWeightResidueGraph &graph,
		const size_t &vertex,
		const std::vector< size_t> CAN_VISIT
)
{
...

	{
		...
		for(core::chemical::lwrg_out_edge_iter itr_edge_target = oe_start; itr_edge_target != oe_end; ++itr_edge_target)
		{
			core::chemical::lwrg_VD target = boost::target(*itr_edge_target, graph);
...
/*
         ...
         std::istream &RingDetection::Read( std::istream &ISTREAM)
         {
         // read members
         io::Serialize::Read( paths_, ISTREAM);
         io::Serialize::Read( rings_, ISTREAM);
         io::Serialize::Read( graph_size_, ISTREAM);
         ...
         std::ostream &RingDetection::Write( std::ostream &OSTREAM, const size_t INDENT) const
         {
         // write
         io::Serialize::Write( paths_, OSTREAM, INDENT) << '\n';
         io::Serialize::Write( rings_, OSTREAM, INDENT) << '\n';
         io::Serialize::Write( graph_size_, OSTREAM, INDENT);
