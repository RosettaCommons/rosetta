...
option.add( basic::options::OptionKeys::antibody::design::base_instructions, "The Default/Baseline instructions file. Should not need to be changed." ).def("/sampling/antibodies/design/default_instructions.txt");
option.add( basic::options::OptionKeys::antibody::design::instructions, "Path to CDR Instruction File" );
...
option.add( basic::options::OptionKeys::antibody::design::paper_ab_db, "Force the use the Antibody Database with data from the North clustering paper.  This is included in Rosetta, but is not recommended for general use. If a newer antibody database is not found, we will use this. The full ab db is available at http://dunbrack2.fccc.edu/PyIgClassify/" ).def(false);
...
option.add( basic::options::OptionKeys::antibody::design::design_protocol, "Set the main protocol to use.  Note that deterministic is currently only available for the grafting of one CDR." ).legal("generalized_monte_carlo").legal("deterministic_graft").def("generalized_monte_carlo");
option.add( basic::options::OptionKeys::antibody::design::run_snugdock, "Run snugdock on each ensemble after designing." ).def(false);
option.add( basic::options::OptionKeys::antibody::design::run_relax, "Run Dualspace Relax on each ensemble after designing (after snugdock if run)." ).def(false);
...
option.add( basic::options::OptionKeys::antibody::design::benchmark_graft_design, "Start graft design with a new set of CDRs as to not bias the run with native CDRs." ).def(false);
...
inline void add_rosetta_options_6( utility::options::OptionCollection &option ) {option.add( basic::options::OptionKeys::backrub::trajectory, "record a trajectory" ).def(false);
...
inline void add_rosetta_options_7( utility::options::OptionCollection &option ) {option.add( basic::options::OptionKeys::cyclization::add_constraints, "The add constraints to maintain cycle geometry" ).def(true);
...
inline void add_rosetta_options_8( utility::options::OptionCollection &option ) {option.add( basic::options::OptionKeys::edensity::whole_structure_allatom_wt, "wt of edens centroid (allatom) scoring" ).def(0.0);
...
inline void add_rosetta_options_9( utility::options::OptionCollection &option ) {option.add( basic::options::OptionKeys::fold_cst::no_recover_low_at_constraint_switch, "dont recover low when max_seq_sep is increased" ).def(false);
...
inline void add_rosetta_options_10( utility::options::OptionCollection &option ) {option.add( basic::options::OptionKeys::match::geometric_constraint_file, "File describing the geometry of the downstream object relative to the upstream object" );
...
inline void add_rosetta_options_11( utility::options::OptionCollection &option ) {option.add( basic::options::OptionKeys::magnesium::all_hydration_frames, "in -hydration test mode, Sample all hydration frames (slow)" ).def(false);
...
inline void add_rosetta_options_12( utility::options::OptionCollection &option ) {option.add( basic::options::OptionKeys::pepspec::n_cgrelax_loop, "No description" ).def(1);
...
inline void add_rosetta_options_13( utility::options::OptionCollection &option ) {option.add( basic::options::OptionKeys::relax::sequence, "Do a preset, small cycle number FastRelax" );
