...
Splice::chainbreak_check( core::pose::Pose const & pose , core::Real const tolerance , bool fail_retry_if_found , bool crash_if_found){
	...
	for ( core::Size resj = pose.conformation().chain_begin( chain_id ); resj <= pose.conformation().chain_end( chain_id )-1; ++resj ) {
		...
		if ( cuts > 1 ) {
			TR << TR.Red << "WARNING! There is more than one cutpoint"<<TR.Reset << std::endl;
			if ( crash_if_found ) {
	...
	if ( ccd() ) {
		...
		*/
		...
		if ( rtmin() ) {  //To prevent rtmin when not needed - Assaf Alon
			...
			tf_in->push_back(TaskOperationCOP(new core::pack::task::operation::RestrictToRepacking )); //Don't want to do design on the entire protein, Gideon Sep14
		...
		if ( torsion_database_fname_ == "" ) { // no use computing rms if coming from a database (no coordinates)
			...
			if ( !(boost::iequals(tail_segment_, "c")) ) { //When splicing in C-ter tails we don't insert chain-break therefore no need to check chain break filter, gideon
				if ( (protein_family_=="antibodies" ) ) {
					tail_fold_tree(pose,cut_vl_vh_after_llc,cut_site); // why is this necessary? SJF 22Jul14
				}
		...
		if ( min_seg_ ) {
			...
			mm->set_jump(false);
			if ( debug_==1 ) {
				pose.dump_pdb(mover_name_+source_pdb()+"_Before_min_seg.pdb");
			}
			...
			//TR << "CNDEBUG to res with dofs " << from_res()+dofs.size()-1 << std::endl;
			//TR << "CNDEBUG to res residue diff  " << to_res() + residue_diff << std::endl;
			//TR << "CNDEBUG total res " << pose.conformation().chain_end(1) << std::endl;
			//pose.dump_pdb("added_constraints.pdb");
			...
			//TR << "CNDEBUG cutsite before being set in min_seg is " << cut_site << std::endl;
			//TR << "CNDEBUG will attempt to set it to " << dofs.cut_site() - dofs.start_loop() + from_res() << std::endl;
			//core::Size cut_site=0;
			if ( boost::iequals(tail_segment_, "n") ) { //This only matters for n-ter tail because C-ter tail residue are included in the previous mm
				...
				for ( core::Size i =find_nearest_disulfide(pose,from_res())-tail_dofs.size(); i<find_nearest_disulfide(pose,from_res()) - 1 ; ++i ) { // No flexibility for the residue before the C and the C.
			...
			add_sequence_constraints(pose);
			if ( debug_==1 ) {
				pose.dump_pdb(mover_name_+source_pdb()+"_after_after_min_seg.pdb");
			}
		}
	}
...
void Splice::parse_my_tag(TagCOP const tag, basic::datacache::DataMap &data, protocols::filters::Filters_map const & filters, protocols::moves::Movers_map const &, core::pose::Pose const & pose) {
	...
	*/
	...
	//TR<<"Deltas from xml: ";
	//for( int const d : delta_lengths_ )
	//    TR<<d<<',';
	//TR<<std::endl;
...
void Splice::add_coordinate_constraints(core::pose::Pose & pose, core::pose::Pose const & source_pose, core::Size from, core::Size to, core::Size anchor, std::string atom_type, core::pack::task::PackerTaskOP task) {
	...
	for ( core::Size i = from; i <= to; ++i ) {
		...
		if ( atom_type=="CG" ) {
			...
			} else if ( (intersect.size()==allowed_aas_names.size())&&(std::find(aromatic_and_his.begin(), aromatic_and_his.end(),source_pose.residue(i + res_diff).name1())!=aromatic_and_his.end()) ) { //fi allowed_aas_name//if same size then all allowed identities are in the aromatic vector
		...
		core::scoring::func::FuncOP coor_cont_fun( new core::scoring::func::HarmonicFunc(0.0, 1) );
		cst.push_back(
			core::scoring::constraints::ConstraintOP( new core::scoring::constraints::CoordinateConstraint(core::id::AtomID(pose.residue(i).atom_index(atom_type), i),
