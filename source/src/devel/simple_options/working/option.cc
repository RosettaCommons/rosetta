...
void OptionBlock::write_entries( std::ostream &out) {
	...
	*/
	...
	sprintf(buf,"%-24s\n",line.name.c_str()); out << buf;
			} else {
				for ( int i=0; i<line.values.size(); i++ ) {
		sprintf(buf,"%-24s = %s\n",line.name.c_str(),line.values[i].c_str()); out << buf;
	}
			}
		} else {
...
void CommandLineOptions::process_options(int& argc, char**argv) {
	processed_ = true;
	for (t_map_blocks::iterator it=blocks.begin();it!=blocks.end();++it) {
		OptionBlock &ablock=*(it->second);
		if (ablock.isKnown()) {
...
void OptionBlock::process_entries( int &argc, char* argv[] ) {
	...
	name = opt;
			...
			std::cerr<<"found " << opt << " process as " << name << std::endl;
			std::cerr<<" pos " << pos << " argc " << argc << std::endl;
...
void OptionBlock::reg_opt(const base_opt& opt) {
