	...
	for ( core::Size i=1; i<= pose.size(); ++i ) {
		...
		if ( ! new_rsd_type ) {
			if ( generated_restypes.count( rsd.name() ) == 1 ) {
				...
				if ( gen_restype ) {
					TR.Warning << "[ WARNING ] When switching to " << type_set.mode() << " mode, a normal replacement type for residue " << rsd.name() << " can't be found." << std::endl;
					TR.Warning << "[ WARNING ]     an autogenerated replacement type is being used instead." << std::endl;
