	...
	for ( core::Size i=1; i<= pose.size(); ++i ) {
		...
		if ( ( rsd.aa() == aa_unk ) || ( rsd.name().substr(0,5) == "HIS_D" ) ) {
			...
			if ( type_set.has_name( rsd.name() ) ) {
				core::chemical::ResidueType const & new_rsd_type( type_set.name_map( rsd.name() ) );
			...
			core::chemical::ResidueTypeCOP new_rsd_type( type_set.get_representative_type_name3( rsd.name().substr(0,3), rsd.type().variant_types() ) );
			if ( new_rsd_type ) {
				new_rsd = core::conformation::ResidueFactory::create_residue( *new_rsd_type, rsd, pose.conformation() );
			}
