...
FoldTree::delete_jump_seqpos( int const seqpos )
{
	...
	if ( edge_list_.size() == 1 && begin()->is_jump() &&
		...
		edge_list_.clear();
		edge_list_.emplace_back( 1, 1, Edge::PEPTIDE );
		new_topology = true;
	}
	...
	std::string const & root_atom )  // ditto
{
	...
	edge_list_.emplace_back( old2new_res[ anchor_pos ], root_pos, anchor_jump_number );
...
)
{
	...
	edge_list_.emplace_back( start, stop, label );
}
...
)
{
	...
	runtime_assert( stop  > 0 );
	new_topology = true; // book-keeping
	edge_list_.emplace_back( start, stop, start_atom, stop_atom );
}
