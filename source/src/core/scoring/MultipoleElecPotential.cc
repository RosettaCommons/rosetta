...
MultipoleElecPotential::read_in_amoeba_parameters() {
	...
	while ( getline( types_file, input_line ) ) {
		...
		Size const num_variant_qualifiers( num_tokens - 4 );
...
) const
{
	...
	for ( Size iat = 1 ; iat <= rsd.natoms() ; ++iat ) {
		...
		if ( rsd.atom_is_hydrogen( iat ) ||
			...
			mp.set_coord_frame_ref( iat, 0 );
			//TR << "Coord frame ref for " << rsd.atom_name( iat ) << " is " << rsd.atom_name( rsd.atom_base( iat ) ) << std::endl;
		} else {
...
) const
{
	...
	Vector const v2( ( pose.residue(res2).xyz( atom2 ) - rsd.xyz( orig_atom ) ).normalize() );
	Vector const v3( ( atom3 == 0 ? Vector( 0.0 ) : ( pose.residue(res3).xyz( atom3 ) - rsd.xyz( orig_atom ) ).normalize() ) );
...
) const
{
	...
	//
	// Fllowing the treatment of Ren and Ponder, "Polarizable
	...
	Real const fm( coul * (1.0-dwater) / dwater );
	Real const fd( coul * 2.0*(1.0-dwater) / (1.0 + 2.0*dwater) );
	...
	for ( Size atm1 = 1 ; atm1 <= natoms1 ; ++atm1 ) {
		...
		for ( Size atm2 = (same_res ? atm1 : 1 ) ; atm2 <= natoms2 ; ++atm2 ) {
			...
			if ( cpfxn->count( atm1, atm2, weight, path_dist ) &&
				...
				Real const polar_weight( weight > 0.0 ? 1.0 : 0.0 );
...
#ifdef NOTDEF
			if( atom_atomE != atom_atomE ) {
				TR << "Interaction between " << rsd1.xyz( atm1 ) << " and " << rsd2.xyz( atm2 ) << std::endl;
				TR << "Interaction between " << rsd1.atom_name( atm1 ) << " " << rsd1.name( )  << " and " << rsd2.atom_name( atm2 ) << " " << rsd2.name( ) << std::endl;
				TR << "Rkirks are " << rkirk1 << " and " << rkirk2 << std::endl;
				TR << "Charges are " << q1 << " and " << q2 << std::endl;
...
) const
{
	...
	Real const gkc( 2.455 );
	Real const dwater( 78.3 );
	Real const fm( (1.0-dwater) / dwater );
	...
	for ( Size atomi = 1 ; atomi <= rsd1.natoms() ; ++ atomi ) {
		...
		for ( Size atomj=start_atom, atomj_end=rsd2.natoms(); atomj<= atomj_end; ++atomj ) {
			...
			if ( cpfxn->count( atomi, atomj, cp_weight, path_dist ) ) {
				...
				Real const polar_weight( cp_weight > 0.0 ? 1.0 : 0.0 );
				...
				r1_at_derivs[ atomi ].f2() += f2;
				r2_at_derivs[ atomj ].f1() += -1*f1;
				r2_at_derivs[ atomj ].f2() += -1*f2;
			...
			if ( use_gen_kirkwood && (!same_res || (atomi != atomj) ) ) {
				...
				r1_at_derivs[ atomi ].f1() += factor * rf_f1;
				r1_at_derivs[ atomi ].f2() += factor * rf_f2;
				r2_at_derivs[ atomj ].f1() += -1.0 * factor * rf_f1;
				r2_at_derivs[ atomj ].f2() += -1.0 * factor * rf_f2;
