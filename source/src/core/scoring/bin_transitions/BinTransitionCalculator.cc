		...
		namespace bin_transitions {
			...
			static thread_local basic::Tracer TR( "core.scoring.bin_transitions.BinTransitionCalculator" );
					...
					core::Size BinTransitionCalculator::random_bin_based_on_previous (
						...
						if(!bin_params_loaded() || ndata < 1) utility_exit_with_message(
						...
						if(!normal_bond_found) utility_exit_with_message(
						...
						if(data_index==0) utility_exit_with_message(
					...
					core::Size BinTransitionCalculator::random_bin_based_on_prev_and_next(
						...
						if(res_index==0 || res_index>conformation.size()) utility_exit_with_message(
						...
						if(!thisres.has_lower_connect() || !thisres.has_upper_connect() ) utility_exit_with_message(
						...
						if(prev_index==0 || next_index==0) utility_exit_with_message(
						...
						if(data_index_i == 0) utility_exit_with_message(
						...
						if(curbin_index_i != curbin_index_iplus1) utility_exit_with_message(
					...
					bool BinTransitionCalculator::has_another_matrix( utility::vector1 <std::string> const &lines, core::Size &firstline, core::Size &lastline ) const {
						...
						if(begin_found) runtime_assert_string_msg(end_found,
