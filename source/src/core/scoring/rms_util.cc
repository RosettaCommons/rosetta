...
core::Real rnagdt(
	...
	if ( !ref.is_fullatom() || !mod.is_fullatom() ) {
		tr.Warning << "Reference and model must be fullatom for rnagdt()" << std::endl;
		return -1;
	}

	// Retrieve ref, mod coordinates
	utility::vector1< core::Vector > coords_ref_initial;
	utility::vector1< core::Vector > coords_mod_initial;
	int count = 1;
	for ( auto i = residues.begin(); i != residues.end(); ++i, ++count ) {
		const Size ref_idx = i->first;
		const Size mod_idx = i->second;
		...
		// Heavyatom iteration
		for ( Size jj = 1; jj <= ref.residue_type(ref_idx).nheavyatoms(); ++jj ) {
			coords_ref_initial.emplace_back(ref.residue(ref_idx).xyz(jj));
			coords_mod_initial.emplace_back(mod.residue(mod_idx).xyz(jj));
		}
	}

	int actual_num_atoms = coords_ref_initial.size();
	FArray2D<Real> coords_ref(3, actual_num_atoms);
	FArray2D<Real> coords_mod(3, actual_num_atoms);

	// int count = 1;
	for ( Size ii = 1; ii <= coords_ref_initial.size(); ++ii ) {
		const xyzVector<Real> & xyz_ref = coords_ref_initial[ii];
		coords_ref(1, ii) = xyz_ref.x();
		coords_ref(2, ii) = xyz_ref.y();
		coords_ref(3, ii) = xyz_ref.z();

		const xyzVector<Real>& xyz_mod = coords_mod_initial[ii];
		coords_mod(1, ii) = xyz_mod.x();
		coords_mod(2, ii) = xyz_mod.y();
		coords_mod(3, ii) = xyz_mod.z();
	}
	...
	return static_cast<Real>( nali ) / static_cast<Real>( actual_num_atoms );
}
