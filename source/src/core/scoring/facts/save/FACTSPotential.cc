...
FACTSPotential::set_default(){
	...
	do_apprx_ = option[ score::facts_apprx ]();
	...
	dshift2_saltbridge_ = option[ score::facts_dshift_sb ]()*option[ score::facts_dshift_sb ]();
	...
	adjsc_elec_scale_.resize( 5 );
	adjsc_elec_scale_ = option[ score::facts_adjsc_elec_scale ]();
	adjsc_solv_scale_.resize( 5 );
	adjsc_solv_scale_ = option[ score::facts_adjsc_solv_scale ]();
...
void FACTSPotential::setup_for_scoring(pose::Pose & pose, bool const & packing) const
{
	...
	for( res1 = 1; res1 <= nres; ++ res1 ){
		...
		for( Size atm1 = 1; atm1 <= pose.residue(res1).natoms(); ++atm1 ){
			...
			//if( !atmname.compare("Hapo") == 0 || !atmname.compare("Haro") == 0 ) continue;
			//if( //atmname.compare("HNbb") == 0 || atmname.compare("Hpol") == 0 ||
			//	 atmname.compare("OOC") == 0 ){
			cerr << "Res/Atm/Type/" << setw(4) << res1 << setw(4) << rsd1.name() << setw(4) << atm1 << " " << setw(4) << atmname;
			cerr << "| " << setw(10) << facts1.BR(atm1);
			cerr << "| " << setw(10) << facts1.sasa(atm1);
			cerr << "|" << setw(10) << facts1.Ai(atm1) << "|" << setw(10) << facts1.Bi(atm1);
			cerr << "|" << setw(10) << facts1.Ci(atm1) << "|" << setw(10) << facts1.Di(atm1);
			cerr << "|" << setw(10) << facts1.esolvE(atm1);
			cerr << std::endl;
			//}
		}
	}
	...
	for ( Size atm1 = 1; atm1 <= rsd1.natoms(); ++atm1 ) {
		...
		for ( Size atm2 = 1; atm2 <= rsd2.natoms(); ++atm2 ) {
			...
			if( same_res || adjacent ||
				...
				atompair_scale( factstype1, cpfxn14, 
												rsd1, rsd2, atm1, atm2, 
			...
			if( adjacent ){
				...
				Real cpweight4( 0.0 );
				bool is_cp = cpfxn14->count( atm1, atm2, cpweight4, path_dist );
				std::cout << rsd1.seqpos() << " " << rsd2.seqpos() << " " << atm1 << " " << atm2;
				std::cout << " " << rsd1.atom_is_backbone(atm1) << " " <<  rsd2.atom_is_backbone(atm2);
				std::cout << " " << path_dist << " " << cpweight4 << " " << scale_elec;
				std::cout << " " << felec*sf2;
				std::cout << std::endl;
			}
			*/
...
void FACTSPotential::setup_for_derivatives( pose::Pose & pose ) const
{
	...
	//cout << "Setup for derivative: " << elapsedTime << std::endl;
}
...
void FACTSPotential::evaluate_polar_otf_energy(Residue const & rsd1,
	...
	for ( Size atm1 = 1; atm1 <= rsd1.natoms(); ++atm1 ) {
		...
		for ( Size atm2 = 1; atm2 <= rsd2.natoms(); ++atm2 ) {
			...
			Real dshift2( 0.0 );
			if( !rsd1.atom_is_backbone(atm1) && !rsd2.atom_is_backbone(atm2) ){
				dshift2 = std::min(dshift2_, 1.0);
			} else if ( !rsd1.atom_is_backbone(atm1) || !rsd2.atom_is_backbone(atm2) ){
				dshift2 = std::min(dshift2_, 1.0);
			} else {
				dshift2 = 0.0;
