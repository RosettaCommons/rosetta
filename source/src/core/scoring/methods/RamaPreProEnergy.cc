...
RamaPreProEnergy::residue_pair_energy(
	...
	potential_.eval_rpp_rama_score( pose.conformation(), res_lo.type().get_self_ptr(), res_hi.type().get_self_ptr(), mainchain_torsions, rama_score, gradient, false /*Don't return gradient*/ );
...
) const
{
	...
	if ( tor_id.valid() && tor_id.type() == id::BB ) {
		...
		utility::vector1 < core::Real > gradient; //Dummy argument for below.
		potential_.eval_rpp_rama_score( pose.conformation(), res_lo.type().get_self_ptr(), res_hi.type().get_self_ptr(), mainchain_torsions, rama_score, gradient, true /*Do return gradient*/ );
