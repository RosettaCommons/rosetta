...
MainchainScoreTable::parse_rama_map_file_shapovalov(
	...
	do {
		...
		if ( line[0] == '@' ) {
			...
			for ( core::Size i=1; i<=n_mainchain_torsions; ++i ) {
				scoring_grid_indices[i] = static_cast< core::Size >( std::ceil( scoring_grid_torsion_values[i] / 360.0 * static_cast<core::Real>(dimensions[i] ) - offsets[i] ) /*+ 1*/ ); //Note that MathNTensors are 0-based.
			}
