...
ResidueTypeSet::generate_residue_type_write_locked( std::string const & rsd_name ) const
{
	...
	if ( ! patched_mut_type ) { return nullptr; }
	else {
...
ResidueTypeSet::apply_patch(
	...
	if ( rsd_base_ptr == nullptr ) {
		TR.Debug << "The base ResidueType needing the patch '" << patch_name <<
			"' is itself an invalid ResidueType" << std::endl;
		return nullptr;
	}
	...
	if ( patch_name.find( "MP-" ) != std::string::npos ) {
		...
		if ( metapatch_mapping.count( metapatch_name ) == 0 ) {
			// Can't generate this with this ResidueTypeSet
			TR.Debug << "Cannot patch ResidueType using metapatch '" <<
				metapatch_name << "' (patch name '" << patch_name <<
				"') given ResidueTypeSet of mode '" <<
				mode() << "'. Cannot generate patch with this ResidueTypeSet" << std::endl;
			return nullptr;
		}
		...
		if ( rsd_instantiated == nullptr ) {
			TR.Debug << "Cannot patch ResidueType using metapatch '" <<
				metapatch_name << "' (patch name '" << patch_name <<
				"') given ResidueTypeSet of mode '" <<
				mode() << "'. Patch targets atom name '" << atom_name << "'" << std::endl;
			return nullptr;
		}
		...
		// Not a metapatch, apply standard patch
		if ( patch_mapping.find( patch_name ) == patch_mapping.end() ) {
			TR.Debug << "Cannot patch ResidueType using patch name '" <<
				patch_name << "' given ResidueTypeSet of mode '" <<
				mode() << "'. Cannot generate patch with this ResidueTypeSet" << std::endl;
			return nullptr;
		}
		...
		for ( PatchCOP p : patches ) {
			...
			if ( rsd_instantiated == nullptr ) {
				TR.Debug << "Cannot patch ResidueType using patch name '" <<
					patch_name << "' given ResidueTypeSet of mode '" <<
					mode() << "'. Could not apply patch to ResidueType" << std::endl;
				return nullptr;
			}
...
/// @param[in] variants A list of VariantTypes that the returned ResidueTypes *must* have,
/// used for looking up the variant.
