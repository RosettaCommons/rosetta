...
void
core::pose::CrystInfo::save( Archive & arc ) const {
...
template< class Archive >
void
core::pose::CrystInfo::load( Archive & arc ) {
	arc( A_ ); // Real
	arc( B_ ); // Real
	arc( C_ ); // Real
	arc( alpha_ ); // Real
...
SAVE_AND_LOAD_SERIALIZABLE( core::pose::CrystInfo );
#endif // SERIALIZATION
