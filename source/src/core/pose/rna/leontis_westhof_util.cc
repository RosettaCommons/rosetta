...
get_leontis_westhof_table() {
	...
	if ( !init ) {
		...
		LW_Table::const_iterator it_end = lw_table.end();
		for ( LW_Table::const_iterator it = lw_table.begin(); it != it_end; ++it )  {
			std::pair< BaseEdge, BaseEdge > const & key = it->first;
			lw_table[ std::make_pair( key.second, key.first ) ] = it->second;
		}
