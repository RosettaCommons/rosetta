...
void RNA_SuiteName::init() {
	...
	all_suites_.push_back( RNA_SuiteInfo( "1a", 330, fixedsizearray1< Real, 7 >{ { 81.495, 212.250, 288.831, 294.967, 173.990, 53.550, 81.035 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1m", 330, fixedsizearray1< Real, 7 >{ { 83.513, 218.120, 291.593, 292.247, 222.300, 58.067, 86.093 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1L", 330, fixedsizearray1< Real, 7 >{ { 85.664, 245.014, 268.257, 303.879, 138.164, 61.950, 79.457 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "&a", 330, fixedsizearray1< Real, 7 >{ { 82.112, 190.682, 264.945, 295.967, 181.839, 51.455, 81.512 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "7a", 330, fixedsizearray1< Real, 7 >{ { 83.414, 217.400, 222.006, 302.856, 160.719, 49.097, 82.444 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "3a", 330, fixedsizearray1< Real, 7 >{ { 85.072, 216.324, 173.276, 289.320, 164.132, 45.876, 84.956 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "9a", 330, fixedsizearray1< Real, 7 >{ { 83.179, 210.347, 121.474, 288.568, 157.268, 49.347, 81.047 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1g", 330, fixedsizearray1< Real, 7 >{ { 80.888, 218.636, 290.735, 167.447, 159.565, 51.326, 85.213 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "7d", 330, fixedsizearray1< Real, 7 >{ { 83.856, 238.750, 256.875, 69.562, 170.200, 52.800, 85.287 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "3d", 330, fixedsizearray1< Real, 7 >{ { 85.295, 244.085, 203.815, 65.880, 181.130, 54.680, 86.035 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "5d", 330, fixedsizearray1< Real, 7 >{ { 79.671, 202.471, 063.064, 68.164, 143.450, 49.664, 82.757 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1e", 331, fixedsizearray1< Real, 7 >{ { 80.514, 200.545, 280.510, 249.314, 82.662, 167.890, 85.507 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1c", 331, fixedsizearray1< Real, 7 >{ { 80.223, 196.591, 291.299, 153.060, 194.379, 179.061, 83.648 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1f", 331, fixedsizearray1< Real, 7 >{ { 81.395, 203.030, 294.445, 172.195, 138.540, 175.565, 84.470 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "5j", 331, fixedsizearray1< Real, 7 >{ { 87.417, 223.558, 80.175, 66.667, 109.150, 176.475, 83.833 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "5n", 331, fixedsizearray1< Real, 7 >{ { 86.055, 246.502, 100.392, 73.595, 213.752, 183.395, 85.483 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1b", 320, fixedsizearray1< Real, 7 >{ { 84.215, 215.014, 288.672, 300.420, 177.476, 58.307, 144.841 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1[", 320, fixedsizearray1< Real, 7 >{ { 82.731, 220.463, 288.665, 296.983, 221.654, 54.213, 143.771 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "3b", 320, fixedsizearray1< Real, 7 >{ { 84.700, 226.400, 168.336, 292.771, 177.629, 48.629, 147.950 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1z", 320, fixedsizearray1< Real, 7 >{ { 83.358, 206.042, 277.567, 195.700, 161.600, 50.750, 145.258 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "5z", 320, fixedsizearray1< Real, 7 >{ { 82.614, 206.440, 52.524, 163.669, 148.421, 50.176, 147.590 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "7p", 320, fixedsizearray1< Real, 7 >{ { 84.285, 236.600, 220.400, 68.300, 200.122, 53.693, 145.730 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "5p", 320, fixedsizearray1< Real, 7 >{ { 84.457, 213.286, 69.086, 75.500, 156.671, 57.486, 147.686 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1t", 321, fixedsizearray1< Real, 7 >{ { 81.200, 199.243, 288.986, 180.286, 194.743, 178.200, 147.386 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "5q", 321, fixedsizearray1< Real, 7 >{ { 82.133, 204.933, 69.483, 063.417, 115.233, 176.283, 145.733 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "1o", 322, fixedsizearray1< Real, 7 >{ { 83.977, 216.508, 287.192, 297.254, 225.154, 293.738, 150.677 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "7r", 322, fixedsizearray1< Real, 7 >{ { 84.606, 232.856, 248.125, 63.269, 181.975, 295.744, 149.744 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "5r", 322, fixedsizearray1< Real, 7 >{ { 83.000, 196.900, 65.350, 60.150, 138.425, 292.550, 154.275 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "2a", 230, fixedsizearray1< Real, 7 >{ { 145.399, 260.339, 288.756, 288.444, 192.733, 53.097, 84.067 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "4a", 230, fixedsizearray1< Real, 7 >{ { 146.275, 259.783, 169.958, 298.450, 169.583, 50.908, 83.967 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "0a", 230, fixedsizearray1< Real, 7 >{ { 149.286, 223.159, 139.421, 284.559, 158.107, 47.900, 84.424 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "#a", 230, fixedsizearray1< Real, 7 >{ { 148.006, 191.944, 146.231, 289.288, 150.781, 42.419, 84.956 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "4g", 230, fixedsizearray1< Real, 7 >{ { 148.028, 256.922, 165.194, 204.961, 165.194, 49.383, 82.983 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "6g", 230, fixedsizearray1< Real, 7 >{ { 145.337, 262.869, 79.588, 203.863, 189.688, 58.000, 84.900 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "8d", 230, fixedsizearray1< Real, 7 >{ { 148.992, 270.596, 240.892, 62.225, 176.271, 53.600, 87.262 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "4d", 230, fixedsizearray1< Real, 7 >{ { 149.822, 249.956, 187.678, 80.433, 198.133, 61.000, 89.378 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "6d", 230, fixedsizearray1< Real, 7 >{ { 146.922, 241.222, 88.894, 59.344, 160.683, 052.333, 83.417 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "2g", 230, fixedsizearray1< Real, 7 >{ { 141.900, 258.383, 286.517, 178.267, 165.217, 48.350, 84.783 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "2h", 231, fixedsizearray1< Real, 7 >{ { 147.782, 260.712, 290.424, 296.200, 177.282, 175.594, 86.565 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "4n", 231, fixedsizearray1< Real, 7 >{ { 143.722, 227.256, 203.789, 73.856, 216.733, 194.444, 80.911 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "0i", 231, fixedsizearray1< Real, 7 >{ { 148.717, 274.683, 100.283, 80.600, 248.133, 181.817, 82.600 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "6n", 231, fixedsizearray1< Real, 7 >{ { 150.311, 268.383, 84.972, 63.811, 191.483, 176.644, 85.600 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "6j", 231, fixedsizearray1< Real, 7 >{ { 141.633, 244.100, 66.056, 71.667, 122.167, 182.200, 83.622 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "0k", 232, fixedsizearray1< Real, 7 >{ { 149.070, 249.780, 111.520, 278.370, 207.780, 287.820, 86.650 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "2[", 220, fixedsizearray1< Real, 7 >{ { 146.383, 259.402, 291.275, 291.982, 210.048, 54.412, 147.760 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "4b", 220, fixedsizearray1< Real, 7 >{ { 145.256, 244.622, 162.822, 294.159, 171.630, 45.900, 145.804 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "0b", 220, fixedsizearray1< Real, 7 >{ { 147.593, 248.421, 112.086, 274.943, 164.764, 56.843, 146.264 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "4p", 220, fixedsizearray1< Real, 7 >{ { 150.077, 260.246, 213.785, 71.900, 207.638, 56.715, 148.131 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "6p", 220, fixedsizearray1< Real, 7 >{ { 146.415, 257.831, 89.597, 67.923, 173.051, 55.513, 147.623 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "2z", 220, fixedsizearray1< Real, 7 >{ { 142.900, 236.550, 268.800, 180.783, 185.133, 54.467, 143.350 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "4s", 221, fixedsizearray1< Real, 7 >{ { 149.863, 247.562, 170.488, 277.938, 84.425, 176.413, 148.087 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "2u", 221, fixedsizearray1< Real, 7 >{ { 143.940, 258.200, 298.240, 279.640, 183.680, 183.080, 145.120 } } ) );
	all_suites_.push_back( RNA_SuiteInfo( "2o", 222, fixedsizearray1< Real, 7 >{ { 147.342, 256.475, 295.508, 287.408, 194.525, 293.725, 150.458 } } ) );
...
RNA_SuiteName::closest_suite( Pose const & pose, Size const res ) const {
	...
	torsions[ 2 ] = numeric::principal_angle_degrees(
		pose.residue( res - 1 ).mainchain_torsion( EPSILON ) );
	torsions[ 3 ] = numeric::principal_angle_degrees(
		pose.residue( res - 1 ).mainchain_torsion( ZETA ) );
	torsions[ 4 ] = numeric::principal_angle_degrees(
		pose.residue( res ).mainchain_torsion( ALPHA ) );
	torsions[ 5 ] = numeric::principal_angle_degrees(
		pose.residue( res ).mainchain_torsion( BETA ) );
	torsions[ 6 ] = numeric::principal_angle_degrees(
		pose.residue( res ).mainchain_torsion( GAMMA ) );
	torsions[ 7 ] = numeric::principal_angle_degrees(
