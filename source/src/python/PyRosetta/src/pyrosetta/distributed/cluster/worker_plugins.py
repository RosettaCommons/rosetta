...
class TaskSecurityPlugin(WorkerPlugin, NonceCache):
    ...
    def __init__(self, instance_id: str, prk: MaskedBytes, max_nonce: int):
    ...
    def setup(self, worker: Worker) -> None:
        ...
        if _maybe_existing_plugin is not None and _maybe_existing_plugin is not self:
            raise NameError(f"The `TaskSecurityPlugin` worker plugin name already exists: {self.instance_id!r}")
        # The `self.__getstate__` method must be overridden after instantiating `TaskSecurityPlugin` on worker since
        # WorkerPlugin registration first pickles `self.prk` with the default `__getstate__` method:
