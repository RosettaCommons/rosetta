...
def _parse_environment(obj: Any) -> Union[str, NoReturn]:
    ...
    def _parse_str(obj: str) -> Union[str, NoReturn]:
        ...
        if obj == "":
            _warning_msg = (
                "The input 'environment' keyword argument value is an empty string, "
            ...
            if obj != get_yml():
                _err_msg = (
                    ...
                    "decoy may not necessarily be reproduced. Please set the 'environment' keyword argument "
...
def _parse_yield_results(yield_results: Any) -> Union[bool, NoReturn]:
    @singledispatch
    def converter(objs: Any) -> NoReturn:
        raise ValueError("'yield_results' argument value must be of type `bool`.")
        ...
        if obj == "":
            ...
            if obj != _pyrosetta_version_string:
                raise AssertionError(
                    ...
                    + "argument value to an empty string ('') to bypass PyRosetta build validation."
                )
            else:
                logging.debug(
                    "The 'pyrosetta_build' keyword argument value correctly validated against the original PyRosetta build!"
                )
                return obj
