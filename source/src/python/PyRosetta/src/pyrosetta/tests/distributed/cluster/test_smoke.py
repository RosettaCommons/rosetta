...
class SaveAllTest(unittest.TestCase):
    def test_save_all(self):
        ...
        with tempfile.TemporaryDirectory() as workdir:
            ...
            from pyrosetta.distributed.cluster.init_files import InitFileSigner, METADATA_INPUT_DECOY_KEY, METADATA_OUTPUT_DECOY_KEY
            from pyrosetta.distributed.cluster import export_init_file
            ...
            if ".init" not in output_decoy_types:
                ...
                self.assertIn(METADATA_OUTPUT_DECOY_KEY, metadata)
            ...
            self.assertTrue(signer.verify_sha256(metadata["sha256"]))
            self.assertTrue(signer.verify_signature(metadata["signature"]))

            # Export PyRosetta initialization file with output decoy
            output_file = data["metadata"]["output_file"]
            export_init_file(output_file, output_init_file=None)
            exported_init_file = os.path.splitext(output_file)[0] + ".init"
            ...
            self.assertTrue(exported_signer.verify_sha256(exported_metadata["sha256"]))
            self.assertTrue(exported_signer.verify_signature(exported_metadata["signature"]))
