...
def compute_dG_transfer_energy( zcoords, angles, energies ): 
	...
	bilayer_energy = energies[first_index[1]]

	# Find the index of zmax, ange = 270
	angle_is_270_indices = set(np.where( angles == 270.0 )[0])
	z_is_max = set(np.where( zcoords == np.max(zcoords) )[0] )
	second_index = list( angle_is_270_indices.intersection( z_is_max ))
