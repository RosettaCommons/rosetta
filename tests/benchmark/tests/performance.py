...
def run_performance_tests(rosetta_dir, working_dir, platform, config, hpc_driver, verbose=False, debug=False):
    ...
    if res:
        ...
        command_line = '{rosetta_dir}/source/bin/performance_benchmark.{ext} -database {rosetta_dir}/database -benchmark_scale 11 -mute core protocols -in:file:extra_res_path extra_params 2>&1 >{output_log_file}'.format(**vars())
