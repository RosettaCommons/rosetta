...
def run_submodule_regression_test(rosetta_dir, working_dir, platform, config, hpc_driver=None, verbose=False, debug=False):
    ...
    # These are the Rosetta-originated modules which we're likely interested in tracking the most recent submodule main
    ...
    res, merge_base = execute('Getting main merge-base...', f'cd {rosetta_dir} && git merge-base origin/main HEAD', return_='tuple')
    ...
    for submodule in submodules_to_check:
        ...
        # (Test against the common anscestors of current commit and main)
        regression = False
        ...
        #############
        # Test to see if we're include all of the commits in the submodule which the most recent version of main's main has
        # (Test against the most recent version of main's main)

        res, main_sha1 = execute("Getting main's submodule commit...", f'cd {rosetta_dir} && git ls-tree origin/main {submodule}', return_='tuple')
        ...
        if res: submodule_status[_LogKey_] = f"Error getting relationship between {current_sha1} and main's main's version ({main_sha1}) in submodule {submodule}: " + base; continue
        base = base.strip()
        ...
        if res: retval[_LogKey_] = f"Error getting SHA1 for main branch of submodule {submodule}: " + submain_sha1; return retval
