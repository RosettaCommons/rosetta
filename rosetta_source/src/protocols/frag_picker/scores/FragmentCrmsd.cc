...
bool FragmentCrmsd::score(FragmentCandidateOP f, FragmentScoreMapOP empty_map) {
	...
	for (Size i = 1; i <= f->get_length(); i++) {
		...
		frag_pos_coordinates_(1, i) = reference_coordinates_(1, i
					+ f->get_first_index_in_query() - 1);
		fragment_coordinates_(2, i) = r->y();
		frag_pos_coordinates_(2, i) = reference_coordinates_(2, i
					+ f->get_first_index_in_query() - 1);
		fragment_coordinates_(3, i) = r->z();
		frag_pos_coordinates_(3, i) = reference_coordinates_(3, i
					+ f->get_first_index_in_query() - 1);
	}
