...
static basic::Tracer TR("SixDHasher");
...
std::vector< SBin6D > SixDOffsetTree::lookup( Size radius, Bin6D const & center, Bin6D const & bounds ){
	...
	if (radius > data_.size() ) {
		TR << "Tree: Radius out of bounds" << std::endl;
	} else {
		Bin6D idx ( 0 );
		SBin6D depth ( 0 );
...
core::Size SixDOffsetTree::sum_radius( SBin6D & input, core::Size range ){
	Size radius = 0;
	if (range > 6) {
		TR.Debug << "Bad range in sum_radius" << std::endl;
		return 1000;
	}
