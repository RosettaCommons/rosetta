...
DetectProteinLigandInterface::parse_tag( TagPtr tag )
{
  ...
  catres_only_ = tag->getOption< bool >( "catres_only", 0 );
...
ProteinLigandInterfaceUpweighter::parse_tag( TagPtr tag )
{
	...
	if( tag->hasOption("catres_weight") ) catres_weight_ = tag->getOption< core::Real >( "catres_weight", 1.0 );
}
...
PackerTask & task) const
{
	...
	} //if different catres weights are asked for
	if (catres_weight_ !=1.0){
		 ...
		 core::pack::task::IGEdgeReweighterOP catres_up = new protocols::toolbox::ResidueGroupIGEdgeUpweighter( catres_weight_, catres , design_residues );
     task.set_IGEdgeReweights()->add_reweighter( catres_up );

	  tr.Info << "Packer Energies between catalytic residues and design residues are upweighted by factor " << catres_weight_ << "." << std::endl;
	}
