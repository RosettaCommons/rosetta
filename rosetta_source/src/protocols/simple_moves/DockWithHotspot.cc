...
// Unit headers
#include <protocols/simple_moves/DockWithHotspot.hh>
#include <protocols/simple_moves/DockWithHotspotCreator.hh>
...
static basic::Tracer TR( "protocols.simple_moves.DockWithHotspot" );

std::string
DockWithHotspotCreator::keyname() const
{
	return DockWithHotspotCreator::mover_name();
}

protocols::moves::MoverOP
DockWithHotspotCreator::create_mover() const {
	return new DockWithHotspot;
}

std::string
DockWithHotspotCreator::mover_name()
{
	return "DockWithHotspot";
}

///@brief default ctor
DockWithHotspot::DockWithHotspot() :
	protocols::moves::Mover( DockWithHotspotCreator::mover_name() )
{ 
}

DockWithHotspot::~DockWithHotspot() {}

void DockWithHotspot::apply( Pose & pose ) {
  ...
  for (Size i=1; i <= hotspot_filenames_.size(); i++) {
    ...
    hotspot_stub_setOP->read_data( hotspot_filenames_[i] );
		hotspot_stub_setOP->add_hotspot_constraints_to_wholepose( pose, chain_to_redesign, hotspot_stub_setOP, hotspot_distcb_weight_[i], worst_allowed_stub_bonus, apply_self_energies, bump_cutoff, apply_ambiguous_constraints );
	}
...
protocols::docking::DockingLowResOP docking_lowres_mover = new protocols::docking::DockingLowRes( scorefxn, rb_move_jump );
  ...
  if ( CenScore > centroidscore_filter_ || CstScore > hotspotcst_filter_ ) {
			...
			TR<< protocols::jd2::JobDistributor::get_instance()->current_job()->input_tag() << " Did not pass filter CentoridScore ( " << centroidscore_filter_ <<  " ): " << CenScore << "HotspotCstScore ( " << hotspotcst_filter_ << " ): " << CstScore << std::endl;
			return;
	}
...
std::string
DockWithHotspot::get_name() const {
	return DockWithHotspotCreator::mover_name();
}
 ...
 */
void DockWithHotspot::parse_my_tag( utility::tag::TagPtr const tag,
		...
		Pose const & pose)
{
  ...
  centroidscore_filter_=tag->getOption<core::Real>( "centroidscore_filter", 0 );
	...
	TR << "Setup DockWithHotspot with backbone_stub_linear_constraint with weight "<< hotspot_score_weight_ <<" from:" << std::endl;
