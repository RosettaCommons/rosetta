...
namespace sic_dock {
  ...
  SICFast::SICFast() :
	...
	CTD2(sqr(CTD)),CLD2(sqr(CLD)),BIN(CLD*basic::options::option[basic::options::OptionKeys::sicdock::hash_2D_vs_3D]())
      {}
      void SICFast::init(
		core::pose::Pose const & cmp1in, vector1<Vec> cmp1cbs, vector1<double> const & cmp1wts, Vec cmp1axs,
		core::pose::Pose const & cmp2in, vector1<Vec> cmp2cbs, vector1<double> const & cmp2wts, Vec cmp2axs
		)
      {
	xh1_bb_ = new xyzStripeHashPose(basic::options::option[basic::options::OptionKeys::sicdock::  clash_dis]()+0.5);
	xh2_bb_ = new xyzStripeHashPose(basic::options::option[basic::options::OptionKeys::sicdock::  clash_dis]()+0.5);
	xh1_cb_ = new xyzStripeHashPose(basic::options::option[basic::options::OptionKeys::sicdock::contact_dis]());
	...
	// for(vector1<Vec>::const_iterator i = cmp1cbs.begin(); i != cmp1cbs.end(); ++i) {
	// 	xmn = min(i->x(),xmn);
	// 	ymn = min(i->y(),ymn);
	// 	zmn = min(i->z(),zmn);
	// }
	...
	// for(vector1<Vec>::const_iterator i = cmp2cbs.begin(); i != cmp2cbs.end(); ++i) {
	// 	xmn = min(i->x(),xmn);
	// 	ymn = min(i->y(),ymn);
	// 	zmn = min(i->z(),zmn);
	// }
	...
	for(vector1<Vec>::const_iterator i = cbb.begin(); i != cbb.end(); ++i,++iwb) {
	  Vec v = R*((*i)-mindis*ori) + xh->translation();
	...
	while(true){
	  ...
	  for(vector1<Vec>::const_iterator ib = pb.begin(); ib != pb.end(); ++ib) {
	    Vec const v = R*Rori*((*ib)-Vec(0,0,mindis)) + xh->translation();
	    Vec const b = Rori * Rinv * v;
	    if( v.x() < -xh->grid_size_ || v.y() < -xh->grid_size_ || v.z() < -xh->grid_size_ ) continue; // worth it?
	    if( v.x() >  xh->xmx_       || v.y() >  xh->ymx_       || v.z() >  xh->zmx_       ) continue; // worth it?
		...
		for(int i = igl; i < igu; ++i) {
		  xyzStripeHash<double>::float4 const & a2 = xh->grid_atoms_[i];
		  float const d2 = (v.x()-a2.x)*(v.x()-a2.x) + (v.y()-a2.y)*(v.y()-a2.y) + (v.z()-a2.z)*(v.z()-a2.z);
		  Vec const a = Rori * Rinv * Vec(a2.x,a2.y,a2.z);
	...
	return mindis;
      ...
      double SICFast::slide_into_contact(vector1<Vec> pa, vector1<Vec> pb, vector1<Vec> const & cba, vector1<Vec> const & cbb, Vec ori, double & score,
				double anga, double angb, Vec axsa, Vec axsb, bool cmp1or2_a, bool cmp1or2_b)
      {
