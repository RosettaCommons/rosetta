...
WorkUnit_BatchRelax::WorkUnit_BatchRelax(): WorkUnit_SilentStructStore() {}
...
WorkUnit_BatchRelax_and_PostRescore::rescore_all_decoys(){
	...
	for( SilentStructStore::iterator struc = decoys().begin(); struc != decoys().end(); ++ struc ){
		core::pose::Pose pose;
		...
		if( ref_pose_.total_residue() > 0 ){
			protocols::moves::SuperimposeMover sm;
