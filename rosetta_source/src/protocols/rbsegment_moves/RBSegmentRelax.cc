...
protocols::viewer::add_conformation_viewer( pose_noloops.conformation() );
	...
	// remap rbsegs
	protocols::rbsegment_moves::remap_rb_segments( rbsegs_input_, rbsegs_remap_, resmap);
	...
	protocols::rbsegment_moves::FragInsertAndAlignMoverOP frag_ins;
	if ( option[ OptionKeys::loops::vall_file ].user() && !option[ OptionKeys::RBSegmentRelax::skip_fragment_moves ]() ) {
		frag_ins = new protocols::rbsegment_moves::FragInsertAndAlignMover(rbsegs_remap_, pose_noloops, randomness_ );
		doFragInserts = true;
	...
	if (no_lr_ || loops_input_.size() == 0) {
		...
		// call looprelax
		protocols::loops::LoopRelaxMover mover;
