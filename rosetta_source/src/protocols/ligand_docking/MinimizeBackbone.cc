...
{
	if ( tag->getName() != "MinimizeBackbone" ) utility_exit_with_message("This should be impossible");

	if ( ! tag->hasOption("interface") ) utility_exit_with_message("'HighResDocker' requires 'interface' tag (comma separated chains to dock)");
