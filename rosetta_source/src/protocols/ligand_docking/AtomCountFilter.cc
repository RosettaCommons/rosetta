...
void
AtomCountFilter::parse_my_tag( utility::tag::TagPtr const tag, protocols::moves::DataMap &, protocols::filters::Filters_map const &, protocols::moves::Movers_map const &, core::pose::Pose const & )
{
	...
	if ( ! (tag->hasOption("chain") && tag->hasOption("atom_limit") ) ){
		utility_exit_with_message("AtomCount filter needs a 'chain' and an 'atom_limit' option");
	}
