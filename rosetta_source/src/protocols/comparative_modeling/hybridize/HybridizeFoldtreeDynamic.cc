...
utility::vector1 < core::Size > HybridizeFoldtreeDynamic::decide_cuts(core::Size n_residues) {
    ...
    // not allowing cuts in the middle of any contigs
	for (core::Size icontigs = 1; icontigs<=template_contigs_.size(); ++icontigs) {
        for (core::Size ichunk = 1; ichunk<=template_contigs_[icontigs].num_loop(); ++ichunk) {
            for (core::Size ires = template_contigs_[icontigs][ichunk].start(); ires <= template_contigs_[icontigs][ichunk].stop()-1; ++ires) {
                cut_options[ires] = false;
            }
        }
    }
...
protocols::loops::Loops HybridizeFoldtreeDynamic::make_complete_chunks( utility::vector1 < core::Size > cut_positions,
...
void HybridizeFoldtreeDynamic::initialize(core::pose::Pose & pose,
