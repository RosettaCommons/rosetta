...
/// @file protocols/moves/SidechainMetropolisHastingsMover.cc
...
#include <protocols/moves/SidechainMetropolisHastingsMoverCreator.hh>
...
static basic::Tracer tr( "protocols.moves.SidechainSidechainMetropolisHastingsMover" );
...
SidechainMetropolisHastingsMover::apply( core::pose::Pose & pose )
{
	...
	for ( Size ct = 1; ct <= ntrials(); ct++) {
		SidechainMoverBaseOP move = dynamic_cast< SidechainMoverBase* >( random_mover().get() );
