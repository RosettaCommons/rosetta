...
static basic::Tracer TR("protocols.moves.symmetry.SymmetricRMSMover");
...
SymmetricRMSMover::apply( core::pose::Pose & pose )
{
	...
	if ( get_native_pose() ) {
		...
		score_map[ "rms" ] = rms;
		pose.data().set(core::pose::datacache::CacheableDataType::SCORE_MAP, new basic::datacache::DiagnosticData( score_map ));
	}
