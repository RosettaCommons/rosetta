...
/// @file protocols/moves/BackrubMover.cc
...
static basic::Tracer TR("protocols.moves.BackrubMover");

namespace protocols {
namespace moves {

std::string
BackrubMoverCreator::keyname() const {
...
std::string
BackrubMoverCreator::mover_name() {
	return "Backrub";
}
...
MoverOP BackrubMover::clone() const { return new BackrubMover( *this ); }
MoverOP BackrubMover::fresh_instance() const { return new BackrubMover; }
...
)
{
	...
	for (core::Size i = 1; i <= option[ backrub::pivot_residues ].size(); ++i) {
		if (option[ backrub::pivot_residues ][i] >= 1) pivot_residues.push_back(option[ backrub::pivot_residues ][i]);
	}

  mover.set_pivot_residues(pivot_residues);
	mover.set_pivot_atoms(option[ backrub::pivot_atoms ]);
	mover.set_min_atoms(option[ backrub::min_atoms ]);
	mover.set_max_atoms(option[ backrub::max_atoms ]);
}
...
{
	for (std::map<BackrubSegment::BondAngleKey, core::Size>::iterator iter(bond_angle_map_.begin());
