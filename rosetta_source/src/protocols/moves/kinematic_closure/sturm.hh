...
#ifndef INCLUDED_protocols_moves_kinematic_closure_sturm_hh
#define INCLUDED_protocols_moves_kinematic_closure_sturm_hh
...
int numroots(int np, poly* sseq, int* atneg, int* atpos)
{
	...
	//TR.Debug << "sturm.cc::numroots lf: " << lf << std::endl;
    ...
    //TR.Debug << "sturm.cc::numroots at neg inf: " << atneginf << std::endl;
    //TR.Debug << "sturm.cc::numroots at pos inf: " << atposinf << std::endl;
...
int modrf(int ord, double *coef, double	a, double b, double *val)
{
	...
	*/
    ...
    //TR.Debug << "sturm.cc::modrf overflow " << a << " " << b << " " << fx << std::endl;
	return(0);
}
...
void sbisect(int np, poly* sseq, double min, double max, int atmin, int atmax, double* roots)
{
		...
		for (its = 0; its < MAXIT; its++) {
			...
			if (fabs(mid) > RELERROR) {
				if (fabs((max - min) / mid) < RELERROR) {
					...
					roots[0] = mid;
					//TR.Debug << "sturm.cc::sbisect mid, min, max: " << mid << ", " << min << ", " << max << std::endl;
					return;
				}
			}
	...
	for (its = 0; its < MAXIT; its++) {
		...
		if (n1 < 0) {
			...
			//}
			//TR << "atmin was less than atmid (" << atmin << " vs " << atmid << "). Exiting sturm prematurely." <<
			//TR << "atmin was less than atmid (" << atmin << " vs " << atmid << "). But not exiting prematurely." <<
			//	std::endl;
			//break;
		}
	...
	if (its == MAXIT) {
		//TR << "Maximum bisection iterations reached in sturm. Returning mid value for remaining solutions." << std::endl;
		for (n1 = atmax; n1 < atmin; n1++) {
			//roots[index+(n1 - atmax)] = static_cast<Real> ( mid );
			roots[n1 - atmax] = mid;
		}
...
void solve_sturm(const int& p_order, int& n_root, const utility::vector1<double>& poly_coeffs, utility::vector1<double>& roots)
{
	...
	/* // DJM: Debug
		if (basic::options::option[basic::options::OptionKeys::run::run_level] >= basic::options::verbose ) {
			//TR.Debug << "sturm.cc::solve_sturm Sturm sequence for: " << std::endl;
			for (int i = order; i >= 0; i--) {
				//TR.Debug << sseq[0].coef[i] << " ";
			}
			//TR.Debug << std::endl << std::endl;
			for (int i = 0; i <= np; i++) {
				for (int j = sseq[i].ord; j >= 0; j--) {
					//TR.Debug << sseq[i].coef[j] << " ";
				}
				//TR.Debug << std::endl;
			}
		}
	*/
    ...
    //TR.Debug << "sturm.cc::solve_sturm: Number of real roots: " << nroots << std::endl;
	...
	if (nchanges != atmin) {
		//TR.Debug << "sturm.cc::solve: unable to bracket all negative roots" << std::endl;
		atmin = nchanges;
	}
	...
	//TR.Debug << "sturm.cc::solve_sturm atmin: " << atmin << std::endl;
	//TR.Debug << "sturm.cc::solve_sturm atmax: " << atmax << std::endl;
	//TR.Debug << "sturm.cc::solve_sturm nroots: " << nroots << std::endl;
	...
	/* // DJM: debug
		if (basic::options::option[basic::options::OptionKeys::run::run_level] >= basic::options::verbose ) {
			if (nroots == 1) {
				//TR.Debug << std::endl << "sturm.cc::solve_sturm 1 distinct real root at x = " << roots[1] << std::endl;
			}
			else {
				//TR.Debug << nroots << " distinct real roots for x: " << std::endl;
				for (i = 1; i <= nroots; i++)	{
					//TR.Debug << roots[i] << std::endl;
				}
			}
		}
	*/
...
#endif //INCLUDED_protocols_moves_kinematic_closure_sturm_hh
