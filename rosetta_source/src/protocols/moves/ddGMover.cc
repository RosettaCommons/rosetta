...
)
{
	...
	while ( std::abs((*s)(pose)-(*s)(before)) > 1 ){
	  ...
	  before=pose;
		...
		if(basic::options::option[ddg::pack_until_converge]()){
...
)
{
	...
	if(basic::options::option[ddg::use_rotamer_constraints_to_native]()){
	...
	if(basic::options::option[ddg::min_cst]()){
		if(basic::options::option[ddg::harmonic_ca_tether].user()){
			setup_constraints( pose, min_cst_sfxn_, basic::options::option[ddg::harmonic_ca_tether](), false, min_cst_wt_types_);
			min_cst_set_wt_ = (new core::scoring::constraints::ConstraintSet(*(pose.constraint_set())));
			//
			setup_constraints( pose, min_cst_sfxn_, basic::options::option[ddg::harmonic_ca_tether](), true, min_cst_mut_types_);
	...
	if( option[ddg::opt_input_structure].user() && option[ddg::opt_input_structure]() ) {
	...
	if(!dmp_pdb_ && stat(mutant_traj.c_str(), &filesize) == 0){
		...
		for(int i =1; i <= num_iterations_; i++){
			...
			std::string file_to_check = ("mut_" + this->mutation_label(pose) + "_"
																	 + "round_" + q.str() + ".pdb");
			if(stat(file_to_check.c_str(), &filesize) == 0 && !basic::options::option[ddg::suppress_checkpointing]()){
		...
		if(!basic::options::option[ddg::suppress_checkpointing]()){
...
ddGMover::apply(core::pose::Pose & pose)
{
	...
	if(basic::options::option[ddg::output_silent].user()){
		output_silent = basic::options::option[ddg::output_silent]();
	}
	...
	if(any_pdb_empty || basic::options::option[ddg::suppress_checkpointing]()){
