...
/// @file src/protocols/moves/RepulsiveOnlyMover.cc
...
/// RepulsiveOnlyMover; based on the Mover basis class
RepulsiveOnlyMover::RepulsiveOnlyMover() : Mover() {}
...
RepulsiveOnlyMover::apply( core::pose::Pose & pose ) {
	...
	if ( basic::options::option[ basic::options::OptionKeys::in::replonly_residues ].user() ){
		...
		//TR << "RepulsiveOnly Mover has been called" << std::endl;
