...
/// @file /protocols/moves/InterfaceAnalyzerMover.cc
...
static basic::Tracer TR("protocols.moves.InterfaceAnalyzerMover");
static basic::Tracer TRinterface("protocols.moves.InterfaceAnalyzerMover.interface_selection");
static basic::Tracer TRhbonds("protocols.moves.InterfaceAnalyzerMover.missing_hbonds");
...
void InterfaceAnalyzerMover::apply( core::pose::Pose & pose )
{
	...
	compute_separated_sasa( complexed_pose, separated_pose );
	compute_interface_energy( complexed_pose, separated_pose );
	//mut_to_gly(complexed_pose, separated_pose );  this didn't help
	if(use_centroid_) calc_centroid_dG( complexed_pose, separated_pose );
	calc_hbond_sasaE( complexed_pose ); //get hbond E at interface

	if( compute_packstat_) {
		compute_interface_packstat( complexed_pose );
	}
...
core::pose::Pose InterfaceAnalyzerMover::make_separated_pose( core::pose::Pose & pose ){
	...
	protocols::moves::RigidBodyTransMoverOP translate( new protocols::moves::RigidBodyTransMover( separated_pose, interface_jump_ ) );
...
void InterfaceAnalyzerMover::compute_interface_packstat( core::pose::Pose & pose )
{
	...
	foreach(Size ii, interface_set_){
		interface_pack_score_sum += interface_pack_scores[ii];
		++interface_num_res;
	}
		...
		core::pose::Pose & separated_pose)
{
	...
	//protocols::moves::RigidBodyTransMoverOP translate( new protocols::moves::RigidBodyTransMover( separated_pose_, interface_jump_ ) );
	//translate->step_size( 1000.0 );
	...
	foreach(core::Size ii, interface_set_){
		...
		core::chemical::ResidueType const & res(complexed_pose.residue_type(ii));
		for( core::Size i=1 ; i <= res.natoms(); ++i ){
			core::id::AtomID const atid(i, ii);
...
void InterfaceAnalyzerMover::print_pymol_selection_of_interface_residues( core::pose::Pose const & pose, std::set< core::Size > const interface_set )
{
	...
	foreach(core::Size ii, interface_set){
		//sets the current values
		resnum = pose.pdb_info()->number(ii);
		chain_char = pose.pdb_info()->chain(ii);
...
void InterfaceAnalyzerMover::calc_per_residue_energy( core::pose::Pose pose ){
	...
	foreach(core::Size ii, interface_set_){
		total+=pose.energies().residue_total_energies( ii )[ ScoreType( total_score ) ];
	}
...
void InterfaceAnalyzerMover::mut_to_gly( core::pose::Pose complex_pose, core::pose::Pose separated_pose ) {
	...
	foreach(core::Size ii, interface_set_){
		task->nonconst_residue_task( ii ).restrict_absent_canonical_aas(allowed_aa);
		packable[ ii ] = true;
	}
