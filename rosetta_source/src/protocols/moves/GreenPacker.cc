...
/// @file   protocols/moves/GreenPacker.cc
...
basic::Tracer TR("protocols.moves.GreenPacker");
...
void
GreenPacker::set_group_discriminator( GroupDiscriminatorOP discriminator )
{
...
)
{
	...
	for ( Size ii = 1; ii <= pose.total_residue(); ++ii ) {
		...
		while ( orig_id <= ii_orig_nrots && curr_id <= ii_curr_nrots ) {
			MinimalRotamerOP orig_rot = original_rotamers_[ ii ][ orig_id ];
			MinimalRotamerOP curr_rot = current_rotamers_[ ii ][ curr_id ];
