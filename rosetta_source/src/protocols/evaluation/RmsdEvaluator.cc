...
RmsdEvaluator::apply( core::pose::Pose& pose, std::string tag, core::io::silent::SilentStruct &pss) const {
  static basic::Tracer tr("protocols.Evaluator.RMSD");
...
SelectRmsdEvaluator::SelectRmsdEvaluator( core::pose::PoseCOP pose, std::string tag, bool CAonly )
	: SingleValuePoseEvaluator< Real >( "rms"+tag ),
		...
		CAonly_( CAonly )
{
	if ( pose ) find_existing_residues( pose, tag, selection_ );
}
...
SelectRmsdEvaluator::apply( core::pose::Pose& pose ) const {
	...
	if ( !target_pose ) utility_exit_with_message(" no target pose for rmsd evaluation "+tag_ );
  core::Real rmsd;
...
LoopRmsdEvaluator::LoopRmsdEvaluator( core::pose::PoseCOP pose, protocols::loops::Loops loops, std::string tag, bool CAonly, bool superimpose )
  : SingleValuePoseEvaluator< Real >( "looprms"+tag ),
		rmsd_pose_( pose ),
		loops_( loops ),
		CAonly_( CAonly ),
...
LoopRmsdEvaluator::apply( core::pose::Pose& pose ) const {
	...
	if ( !target_pose ) utility_exit_with_message(" no target pose for rmsd evaluation "+name(0) );
  core::Real rmsd;
	if ( superimpose_ ) {
