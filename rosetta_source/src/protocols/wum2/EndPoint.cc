...
EndPoint::EndPoint( mpi::communicator world, uint64_t reserved_mem, uint64_t reserved_mem_multiplier, function< uint64_t () > role_available_mem  )
  : world_( world ),
...
void EndPoint::check_and_act_status_request( function< void ( StatusResponse & , int ) > functor ) {
...
void EndPoint::listen_wu_sendrecv( StatusResponse & r, int requesting_node ) {
...
void EndPoint::send_status_request( int rank ) {
	mpi::request req;
	StatusRequest m;
	m.rank = mpi_rank();
...
void EndPoint::act_on_status_response( function<bool ( StatusResponse & r )> f ){
...
bool EndPoint::initiate_wu_sendrecv( StatusResponse & r ) {
...
void EndPoint::receive_wus( int rank, uint64_t mem_size ) {
  if( mem_size != 0 ) {
...
void EndPoint::send_wus( int rank, uint64_t mem_size ) {
...
void EndPoint::check_and_act_clearcommand() {
