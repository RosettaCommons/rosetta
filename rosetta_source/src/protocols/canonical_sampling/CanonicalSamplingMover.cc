  ...
  for ( ; i_trial < ntrials_; i_trial++ ) {
	...
	if( use_hierarchical_clustering_ ) {
	  if( !MPI_synchronize_pools_ ) {
	    protocols::canonical_sampling::mc_convergence_checks::HierarchicalLevel* hpool_ptr = dynamic_cast<protocols::canonical_sampling::mc_convergence_checks::HierarchicalLevel * > (&(*pool_rms_));
...
#ifdef USEMPI
	    protocols::canonical_sampling::mc_convergence_checks::MPIHPool_RMSD* hpool_ptr = dynamic_cast<protocols::canonical_sampling::mc_convergence_checks::MPIHPool_RMSD * > (&(*pool_rms_));
