...
CanonicalSamplingMover::CanonicalSamplingMover(
  ...
  Mover("CanonicalSamplingMover"),
  mc_(new MonteCarlo( *sfxn, basic::options::option[ basic::options::OptionKeys::canonical_sampling::sampling::mc_kt ]() ) ),
  sfxn_(sfxn),
  randmove_(new RandomMover()),
  pool_rms_(ptr),
...
void CanonicalSamplingMover::set_defaults_from_cmdline() {
  ...
  ntrials( option[ canonical_sampling::sampling::ntrials ] );
  detailed_balance( !option[ canonical_sampling::sampling::no_detailed_balance ] );
  output_only_cluster_transitions( option[ canonical_sampling::sampling::output_only_cluster_transitions ] );
  set_interval_pose_dump( option[ canonical_sampling::sampling::interval_pose_dump ] );
  set_interval_data_dump( option[ canonical_sampling::sampling::interval_data_dump ] );
  transition_threshold_ = option[ canonical_sampling::sampling::transition_threshold ]();
  ramp_temperature_ = false;
  //ramp_temperature_ = option[ canonical_sampling::sampling::ramp_temperature ]();
  save_loops_only_ =  option[ canonical_sampling::sampling::save_loops_only ]();
  dump_loops_only_ =  option[ canonical_sampling::sampling::dump_loops_only ]();
...
CanonicalSamplingMover::apply(Pose & pose){
  ...
  core::Size nr_dirs( nr_jobs / option[ canonical_sampling::sampling::max_files_per_dir ]() + 1 );
