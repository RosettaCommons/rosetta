...
/// @file   protocols/jd2/EnzdesJobOutputter.cc
	...
	silent_job_outputter_(NULL)
{
	...
	if(  basic::options::option[ basic::options::OptionKeys::out::file::silent_struct_type ].user() ){
		silent_output_ = true;
		silent_job_outputter_ = new SilentFileJobOutputter;
...
void
EnzdesJobOutputter::final_pose( JobCOP job, core::pose::Pose const & pose )
{
	if( silent_output_){
...
bool
EnzdesJobOutputter::job_has_completed( JobCOP job )
{
...
)
{
  ...
  for( Job::StringRealPairs::const_iterator it(job->output_string_real_pairs_begin()), it_end(job->output_string_real_pairs_end());
       it != it_end; ++it){
