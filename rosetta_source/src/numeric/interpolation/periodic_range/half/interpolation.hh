...
)
{
	assert( w > X( 0.0 ) );
	assert( n > 0 );
	ssize_t const i( numeric::nearest_ssize( x / w ) ); // Non-periodic bin number
...
)
{
	...
	assert( n > 0 );
	X const r( x / w );
	ssize_t const i( numeric::nearest_ssize( r ) ); // Non-periodic bin number
	a = r - ( i - X( 0.5 ) );
...
)
{
	assert( n > 0 );
	X a; // Alpha fraction: ( x - x_l ) / ( x_u - x_l ) for bin [ x_l, x_u ]
	ssize_t const u( bin( x, w, n, a ) );
	assert( ( u >= 0 ) && ( u < n ) );
	ssize_t const l( numeric::modulo( u - 1 + n, n ) );
...
F
interpolated(
	ssize_t const u, // Bin number (== upper index)
...
)
{
	...
	ssize_t const u1( bin( x1, w, n, a1 ) );
	ssize_t const u2( bin( x2, w, n, a2 ) );
	assert( ( u1 >= 0 ) && ( u1 < n ) );
	assert( ( u2 >= 0 ) && ( u2 < n ) );
	X const b1( X( 1.0 ) - a1 ); // 1 - a1
	X const b2( X( 1.0 ) - a2 ); // 1 - a2
	ssize_t const l1( numeric::modulo( u1 - 1 + n, n ) );
	ssize_t const l2( numeric::modulo( u2 - 1 + n, n ) );
	return
	 ( b1 * b2 * f( l1, l2 ) ) +
	 ( a1 * b2 * f( u1, l2 ) ) +
	 ( b1 * a2 * f( l1, u2 ) ) +
...
F
bilinearly_interpolated(
	ssize_t const u1, // Axis 1 bin number (== upper index)
	ssize_t const u2, // Axis 2 bin number (== upper index)
