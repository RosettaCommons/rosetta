...
public:
	CustomMover( ) {
		...
		utility::vector1< utility::file::FileName > files= option[ fpd::fragments ]();
	...
	void superpose( core::pose::Pose &frag, int frag_insert, core::pose::Pose &pose, 
		...
		for (int i=1; i<=(int)len; ++i) {
			...
			numeric::xyzVector< core::Real > y_1 = pose.residue(i+frag_insert-1).atom(" C  ").xyz();
			numeric::xyzVector< core::Real > y_2 = pose.residue(i+frag_insert-1).atom(" O  ").xyz();
			numeric::xyzVector< core::Real > y_3 = pose.residue(i+frag_insert-1).atom(" CA ").xyz();
			numeric::xyzVector< core::Real > y_4 = pose.residue(i+frag_insert-1).atom(" N  ").xyz();
	...
	void apply( Pose & pose ) {
		...
		// remap insertion points
		pose.pdb_info()->set_chains( 'A' );
		core::pose::PDBPoseMap const pose_map( pose.pdb_info()->pdb2pose() );

		for (int i=1; i<= pose1_insert_points_.size(); ++i) {
			core::Size remapped = pose_map.find('A',pose1_insert_points_[i]);
			pose1_insert_points_[i] = remapped;
		}
		for (int i=1; i<= pose2_insert_points_.size(); ++i) {
			core::Size remapped = pose_map.find('A',pose2_insert_points_[i]);
			pose2_insert_points_[i] = remapped;
		}
		...
		for (int m=1; m<=4; m+=1) {
			core::Real bonded_weight = 0.001*max_cart;
			if (m==1) bonded_weight = 0;
			if (m==4) bonded_weight = max_cart;

			core::Real cst_weight = max_cst;
			if (m<=2)  cst_weight = 4*max_cst;
			...
			for (int n=1; n<=ncycles; ++n) {
				...
				// 2 - insert homologue frag or subfrag, don't superpose
				// 3 - insert homologue frag, superpose
				// 4 - insert homologue frag or subfrag, superpose
				...
				if (m==2) action = 3;
				if (m==3) action = 4;
				if (m==4 || action_picker < 0.2) action = 5;
				...
				if (action == 5) {
					...
					core::Size frag_set = numeric::random::random_range
						(1, (pose2s_.size()>0 && (action == 2 || action == 4)) ? 2 : 1 );
					...
					if (frag_set == 1)
						...
						action_string = action_string+"_s";

					if (action == 3 || action == 4)
						superpose( *frag, insert_pt, pose, R, preT, postT);

					// xyz copy fragment to pose
					for (int i=1; i<=frag->total_residue(); ++i)
					for (int j=1; j<=pose.residue(insert_pt+i-1).natoms(); ++j) {
						core::id::AtomID src(j,i), tgt(j,insert_pt+i-1);
						pose.set_xyz( tgt, postT + (R*(frag->xyz( src )-preT)) );
					}
				}

				core::Real score1 = (*min3_scorefxn_)(pose);

				minimizer.run( pose, mm, *min3_scorefxn_, options );

				core::Real score2 = (*min3_scorefxn_)(pose);
