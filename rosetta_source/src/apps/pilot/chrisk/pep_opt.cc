...
RunPepSpec()
{
	...
	Pose start_pose;
	core::import_pose::pose_from_pdb( start_pose, pdb_filenames[ 1 ] );
	...
	for( Size i_opt = 1; i_opt <= option[ pep_spec::n_opt_loop ]; ++i_opt ){
		...
		for( Size peptide_loop = 1; peptide_loop <= pdb_filenames.size(); ++peptide_loop ){
			...
			PackRotamersMoverOP dz_pack( new PackRotamersMover( scorefxn, dz_task, 1 ) );
			RotamerTrialsMoverOP dz_rottrial ( new RotamerTrialsMover( scorefxn, rottrial_task_factory ) );
