...
*/
	...
	for( Size peptide_loop = 1; peptide_loop <= n_peptides; ++peptide_loop ){
		...
		if( !option[ pepspec::no_prepack_prot ] ){
			...
			PackRotamersMoverOP prepack_mover( new PackRotamersMover( soft_scorefxn, prepack_task, 1 ) );
			...
			RotamerTrialsMoverOP prepack_rottrial ( new RotamerTrialsMover( soft_scorefxn, prepack_task_factory ) );
			prepack_rottrial->apply( pose );

			kinematics::MoveMapOP mm_prepack ( new kinematics::MoveMap );
			mm_prepack->set_chi( true );
			MinMoverOP prepack_min_mover = new MinMover( mm_prepack, scorefxn, "dfpmin", 0.001, true );
		...
		for( Size i_dock = 1; i_dock <= option[ pepspec::n_dock_loop]; ++i_dock ){
...

			{
				...
				MinMoverOP min_mover = new MinMover( mm_min, scorefxn, "dfpmin", 0.001, true );
			...
			protocols::moves::PackRotamersMoverOP packer( new PackRotamersMover( soft_scorefxn, task, 1 ) );
			packer->apply( pose );
			( *scorefxn )( pose );
