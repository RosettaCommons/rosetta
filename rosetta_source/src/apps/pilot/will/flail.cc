...
struct Link {
	...
	X const n_anchor(){ return X( P[ 1 ], P[ 2 ], P[3] ); }
	X const c_anchor(){ return X( P[N-2], P[N-1], P[N] ); }
	...
	bool clashes() const {
		...
		for(PTS::const_iterator i = P.begin(); i != P.end()-6; ++i) 
			for(PTS::const_iterator j = i+6; j != P.end(); ++j) 
				if( i->distance_squared(*j) < 12.0 ) 
					return true;
		return false;
	}
	...
	bool clashes(Rose const & rose) const {
		if( N > 16 ){
			...
			X xinv = ~rose.x;
			for(PTS::const_iterator i = P.begin()+8; i != P.end()-8; ++i){
...
struct Bola {
	...
	inline core::Real get_target_distance(){
		return get_target_xform().distance( target );
	}
	void random_move(){
		...
		link->set_phi_psi(ir,phi,psi);
		last_moved_link = link;
		++ntrials;
	}
	void random_nonclashing_move(){
		for(core::Size i = 0; i < 10000; ++i){
			random_move();
			if(!clashes()) return;
			++nclashes;
		...
		utility_exit_with_message("no non-clashing after 10000 tries!");
	}
	void undo_most_recent_move(){
	...
	void refold(){
		for(core::Size i = 1; i <= links.size(); ++i){
			links[i  ].align_n( roses[i].c_anchor() );
			roses[i+1].align_n( links[i].c_anchor() );
		}
	}
	...
	bool clashes(){
		for(vector1<Link>::const_iterator i=links.begin(); i!=links.end(); ++i) if(i->clashes()) return true;
		refold();
		for(vector1<Link>::const_iterator i=links.begin(); i!=links.end(); ++i){
			for(vector1<Link>::const_iterator j=i+1; j!=links.end(); ++j){
				if( j->clashes(*i) ) return true;
			}
		}
		for(vector1<Rose>::const_iterator i = roses.begin(); i != roses.end(); ++i){
...
int main(int argv, char **argc){
	...
	vector1<std::string> linker_seqs  = option[flail::linker_seqs]();
	...
	tag += string_of(numeric::random::uniform()).substr(2);
	...
	while(true){
		...
		cout << "trials:          " << bola.ntrials << endl;
		cout << "nclash trials:   " << bola.ntrials-bola.nclashes << endl;
		cout << "naccepts:         " << bola.naccepts << endl;
		cout << "clash/ntrial:    " << (core::Real)bola.nclashes/(core::Real)bola.ntrials << endl;
		cout << "naccepts/ntrials: " << (core::Real)bola.naccepts/(core::Real)bola.ntrials << endl;

		bola.dump_pdb("test1.pdb");
		utility_exit_with_message("bla");

		core::Size tot=0,tot5=0;
		core::Real frac5=0.0;
		...
		for(core::Size i = 1; i <= 10; ++i) frac5 += hist2[i];
		for(core::Size i = 1; i <= 10; ++i) tot5 += hist[i];
		cout << "frac5: " << frac5 << " count5: " << tot5 << " TOT: " << tot << endl;

		utility::io::ozstream out(option[out::file::o]()+"/hist_"+tag+".dat");
