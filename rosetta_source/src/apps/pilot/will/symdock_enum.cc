...
void run(  ) {
  ...
  ObjexxFCL::FArray3D<float> surfdis((Size)floor(72.0/ANGLE_INCR),(Size)floor(120.0/ANGLE_INCR),(Size)floor(360.0/ANGLE_INCR),0.0);
  {
    double mxd = 0;
  ...
  vector1<int> cbtmp;
  for(Size i = 0; i < cbcount_local.size(); ++i) {
  ...
  for(int ipnt = 1; ipnt <=  72/ANGLE_INCR; ++ipnt) {
    for(int itri = 1; itri <= 120/ANGLE_INCR; ++itri) {
      for(int iori = 1; iori <= 360/ANGLE_INCR; ++iori) {
        ...
        Pose p = pinit;
        Pose t = tinit;
        rot_pose(p,paxs,(Real)((ipnt-1)*ANGLE_INCR));
        rot_pose(t,taxs,(Real)((itri-1)*ANGLE_INCR));

        Vecf sicaxis = (rotation_matrix_degrees(-Vec(0,1,0),(Real)((iori-1)*ANGLE_INCR)) * Vec(0,0,1)).normalized();

        Real theta = ((iori-1)*ANGLE_INCR);
        Real gamma = theta-alpha;
        Real x = surfdis(ipnt,itri,iori) * sin(numeric::conversions::radians(gamma));
        Real y = surfdis(ipnt,itri,iori) * cos(numeric::conversions::radians(gamma));
        Real w = x / sin(numeric::conversions::radians(alpha));
        Real z = x / tan(numeric::conversions::radians(alpha));
