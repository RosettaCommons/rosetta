...
public:
	MoveThenFixHMin(core::pose::Pose const & pose, ScoreFunctionOP sf, protocols::moves::MoverOP mover) : mover_(mover) {
		...
		movemap->set_chi(true);
		movemap->set_jump(true);
		minmover_ = new protocols::moves::MinMover( movemap, sf, "dfpmin_armijo_nonmonotone", 1e-5, true, false, false );
		...
		// TR << *task << std::endl;	
		packmover_ = new protocols::moves::PackRotamersMover( sf, task );
	}
	void apply(core::pose::Pose & pose) {
		mover_->apply(pose);
		fixH(pose);
...
int main( int argc, char * argv [] ) {
	...
	// } else {
		...
		bbmove = new MoveThenFixHMin(pose,sffa,new protocols::moves::BBG8T3AMover());
		// AtomID a1( pose.residue(1).atom_index(   "N"), 1 ), a2( pose.residue(pose.n_residue()).atom_index("OVL1"), pose.n_residue() );
		// AtomID b1( pose.residue(1).atom_index(  "CA"), 1 ), b2( pose.residue(pose.n_residue()).atom_index("OVL2"), pose.n_residue() );
		// AtomID c1( pose.residue(1).atom_index("OVU1"), 1 ), c2( pose.residue(pose.n_residue()).atom_index(   "C"), pose.n_residue() );
		// pose.remove_constraints();
	...
	for(int ITER=1; ITER <= option[out::nstruct](); ITER++) {
		...
		if( ITER % option[cyclic::log_interval]() == 0) {
			TR << ObjexxFCL::fmt::I(10, ITER                ) << " ITERs " 
			   ...
			   << ObjexxFCL::fmt::F(7,3, core::scoring::CA_rmsd(pose,targets_in[1]) ) << " rms " 
			   // << ObjexxFCL::fmt::F(7,3, core::scoring::all_atom_rmsd(pose,start_pose) ) << " rms " 
			   << ObjexxFCL::fmt::F(7,3,  pose.energies().total_energies()[core::scoring::atom_pair_constraint]) << " cst " 
