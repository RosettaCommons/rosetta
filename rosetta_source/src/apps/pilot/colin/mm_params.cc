	...
	for (std::list< core::chemical::AA >::const_iterator aa_iter(residue_set->aas_defined_begin());
		...
		for (core::chemical::ResidueTypeCAPs::const_iterator residue_iter(aa_caps.begin());
			...
			for (core::Size i = 1; i <= residue_type.num_bondangles(); ++i) {
				...
				core::Real residue_type_theta0(numeric::angle_radians(residue_type.xyz(atom_set.key1()), residue_type.xyz(atom_set.key2()), residue_type.xyz(atom_set.key3())));
			...
			for (core::Size i = 1; i <= residue_type.n_residue_connections(); ++i) {
				...
				for (core::Size j = 1; j <= bonded_neighbors.size(); ++j) {
					...
					core::Real residue_type_theta0(numeric::angle_radians(residue_type.xyz(bonded_neighbors[j]), residue_type.xyz(connection_atomno), external_xyz));
