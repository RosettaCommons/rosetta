...
rebuild_test(){
	...
	protocols::swa::initialize_input_streams( input_streams );
  ...
  // move following into its own function?
	...
	if ( option[ in::file::frag_files ].user() ){
		...
		if ( input_streams.size() == 1 ){
			...
			StepWisePoseSampleGeneratorOP sample_generator_identity = new StepWiseIdentitySampleGenerator( input_streams[1],
...
enable_sampling_of_loop_takeoff( protocols::swa::StepWisePoseSampleGeneratorOP & sample_generator,
	...
	StepWisePoseSampleGeneratorOP sample_generator_for_takeoff_res = new StepWiseProteinMainChainSampleGenerator( stepwise_screener.which_torsions(),
																																																								stepwise_screener.main_chain_torsion_set_lists_real() );

	sample_generator = new StepWiseCombineSampleGenerator( sample_generator /*input pose sample generator from above*/, sample_generator_for_takeoff_res);
...
combine_loops_test(){
	...
	if ( option[ pack_weights ].user() ){
		...
		StepWisePoseSampleGeneratorOP sample_generator = new StepWiseDoNothingSampleGenerator();
		StepWiseProteinPacker stepwise_packer( moving_residues, sample_generator );
		stepwise_packer.set_scorefxn( pack_scorefxn );
...
my_main( void* )
{
	...
	if ( option[ cluster_test ] ){
		...
		protocols::swa::protein::generate_beta_database_test(); // in StepWiseBetaAntiParallelUtil.cc
