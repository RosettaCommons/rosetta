...
pdb_minimizer() {
	...
	if ( fix_all_P_ ) {
		...
		for ( Size i = 1; i <= nres;  ++i ) {
			if ( pose.residue ( i ).aa() != core::chemical::aa_vrt ) {
				...
				Size atm_index = rsd.atom_index ( "P" );
				cst_set -> add_constraint ( new CoordinateConstraint (
				                              AtomID ( atm_index, i ), AtomID ( 1, my_anchor ), rsd.xyz ( atm_index ),
	...
	if ( sample_res_list.size() == 0 ) {
		...
		for ( Size ii = 1; ii <= sample_res_list.size(); ++ii ) {
			core::Size sample_res = sample_res_list[ii];
			std::cout << sample_res << " ";
			allow_insert ( sample_res ) = true;
			mm.set_bb ( sample_res, true );
			mm.set_chi ( sample_res, true );
		}
	...
	for ( Size i = 1; i <= fixed_res_list.size(); ++i ) {
		...
		Size atm_indexP = rsd.atom_index ( "P" );
		Size atm_indexO3 = rsd.atom_index ( "O3*" );
		Size atm_indexC1 = rsd.atom_index ( "C1*" );
		...
		cst_set -> add_constraint ( new CoordinateConstraint ( AtomID ( atm_indexC1, fixed_res_num ), AtomID ( 1, my_anchor ), rsd.xyz ( atm_indexC1 ), new HarmonicFunc ( 0.0, coord_sdev ) ) );
		pose.constraint_set ( cst_set );
		scorefxn->set_weight ( coordinate_constraint, 10 );
	...
	//Setup new fold tree and for Sample res mode
	if ( sample_res_list.size() != 0 ) {
		setup_fold_tree_sample_res ( pose, sample_res_list );
		kinematics::FoldTree fold_tree ( pose.fold_tree() );
		...
		for ( Size i = 1; i <= fold_tree.num_jump(); ++i ) {
			...
			if ( is_elem_in_list ( k, sample_res_list ) || is_elem_in_list ( m, sample_res_list ) ) {
				mm.set_jump ( i, true );
			} else {
				mm.set_jump ( i, false );
			}
		...
		protocols::swa::rna::Output_fold_tree_info ( pose.fold_tree(), "rna_pdb_minimizing new tree" );
	}
...
main ( int argc, char * argv [] ) {
	...
	NEW_OPT ( sample_res, "optional: residues to be sampled in minimizer", blank_size_vector );
