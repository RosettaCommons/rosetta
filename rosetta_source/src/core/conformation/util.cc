...
// @brief determine torsion bins for given phi/psi/omega combination, grid based on http://www.ncbi.nlm.nih.gov/pubmed/19646450
...
char get_torsion_bin (core::Real phi, core::Real  psi, core::Real omega) // omega defaults to 180 unless specified 
{
	if (phi > 180)
	...
	} else { // [rare] cis --> lowercase letters
		if (phi <= 0) {
			if (psi < -130 or psi > 50) { 
				pos_bin = 'b';
			} else { // -130 <= psi <= 50
