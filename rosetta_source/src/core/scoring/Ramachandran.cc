...
Ramachandran::init_rama_sampling_table(
									   char const torsion_bin) //AS: torsion_bin defaults to 'X' unless specified
{
	...
	for (int aa=1; aa<=n_aa_; aa++) { // loop over all residue types
		...
		utility::vector1< utility::vector1< Real> > res_torsions( max_allowed ); // double vector of allowed torsions or this residue
		...
		for (int i=0; i<n_phi_; i++) {
			for (int j=0; j<n_psi_; j++) {
				...
				if ( res_prob > rama_sampling_thold_ ) {
					...
					char cur_tb;
					if (torsion_bin != 'X') 
						cur_tb = core::conformation::get_torsion_bin(cur_phi * binw_, cur_psi * binw_); 
...
) const
{
	...
	if ( rama_sampling_table_.size() == 0 ) {
		...
		const_cast< Ramachandran * > (this)->init_rama_sampling_table('X'); // AS: this is the complete rama table, unrestricted
	}
...
//AS: just to avoid code duplication
void
