...
void OrbitalsStatistics::sc_H_orbital(
		core::pose::Pose & pdb
){
		...
		if(resid1.has_sc_orbitals()){
			...
			for(core::Size res_num2 = 1; res_num2 <= pdb.n_residue(); ++res_num2){
				...
				if(resid1.seqpos() != resid2.seqpos()){
					if(resid2.is_polar() || resid2.is_aromatic()){
						core::Real container = 11;
						core::Real low_D = 11;
							...
							for(
								...
								if(
										...
										for(
												...
												hpol_index = resid2.Hpos_polar_sc().begin(),
												hpol_end = resid2.Hpos_polar_sc().end(); hpol_index != hpol_end; ++hpol_index
											)
										{
											res2_H_xyz = resid2.atom( *hpol_index ).xyz();
											container = orb_xyz.distance( res2_H_xyz );
											if(container <= low_D){
