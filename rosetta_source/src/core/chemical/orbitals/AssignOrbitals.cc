	...
	if(restype_->aa() == aa_tyr || restype_->aa() == aa_phe || restype_->aa() == aa_trp){
		if(restype_->actcoord_atoms().size() != 0){
			...
			if(!atmtype.is_virtual()){
				...
				AOdist_ =0.7;
				AOhybridization_=2;
				numeric::xyzVector<core::Real> acct_coord = ((restype_->ideal_xyz(Aindex_) - restype_->ideal_xyz(atm_index2))/2);
				...
				{
					new_action += restype_->ideal_xyz(restype_->actcoord_atoms()[ii]);
				}
				...
				numeric::xyzVector<core::Real> vector_d( new_action - restype_->ideal_xyz(atm_index2));
				numeric::xyzVector<core::Real> vector_f( new_action - restype_->ideal_xyz(atm_index3));
				...
				pi_orbital_xyz_vector.push_back((xyz_left.normalized() *  AOdist_) + restype_->ideal_xyz(atm_index3));
				pi_orbital_xyz_vector.push_back((xyz_right.normalized() *  AOdist_) + restype_->ideal_xyz(atm_index3));
				...
				for(core::Size vector_index = 1; vector_index <= pi_orbital_xyz_vector.size(); ++vector_index){
					...
					Vector const stub1_xyz = restype_->ideal_xyz(Aindex_);
					Vector const stub2_xyz = restype_->ideal_xyz(atm_index2);
					Vector const stub3_xyz = restype_->ideal_xyz(atm_index3);
...
*/
				...
				numeric::xyzVector<core::Real> vector_a(restype_->ideal_xyz(Aindex_) - restype_->ideal_xyz(atm_index2)   );
				numeric::xyzVector<core::Real> vector_b( restype_->ideal_xyz(Aindex_) - restype_->ideal_xyz(atm_index3) );
				...
				orbital_xyz_vectors.push_back((vector_ab_norm.normalized()*AOdist_)+restype_->ideal_xyz(Aindex_));
...
)
{
		...
		numeric::xyzVector<core::Real> vector_d( restype_->ideal_xyz(atm_index1) - restype_->ideal_xyz(atm_index2));
		numeric::xyzVector<core::Real> vector_f( restype_->ideal_xyz(atm_index1) - restype_->ideal_xyz(atm_index3));
		...
		pi_orbital_xyz_vector.push_back((xyz_right.normalized() * dist) + restype_->ideal_xyz(atm_index1));
		pi_orbital_xyz_vector.push_back((xyz_left.normalized() *  dist) + restype_->ideal_xyz(atm_index1));
...
{
	Vector const stub1_xyz = restype_->ideal_xyz(atm_index1);
	Vector const stub2_xyz = restype_->ideal_xyz(atm_index2);
	Vector const stub3_xyz = restype_->ideal_xyz(atm_index3);
...
utility::vector1< numeric::xyzVector<core::Real> >  AssignOrbitals::Coordinates_TriganolPlanar_bondedto1atom_helper(
	...
	numeric::xyzVector<core::Real> vector_a = restype_->ideal_xyz(atm_index1);
	numeric::xyzVector<core::Real> vector_b = restype_->ideal_xyz(atm_index2);
	numeric::xyzVector<core::Real> vector_c = restype_->ideal_xyz(atm_index3);
...
utility::vector1< numeric::xyzVector<core::Real> >  AssignOrbitals::Coordinates_Tetrahedral_bondedto3atoms_helper(
	 ...
	 numeric::xyzVector<core::Real> vector_b = restype_->ideal_xyz(atm_index2);
	 numeric::xyzVector<core::Real> vector_c = restype_->ideal_xyz(atm_index3);
	 numeric::xyzVector<core::Real> vector_d = restype_->ideal_xyz(atm_index4);
