					...
					bool const copy_dofs_for_junction_residues /*= true*/ )
{
	...
	// The use of the "MiniPose" takes a little extra time; however,
	// elsewhere in the code (e.g., RNAChunkLibrary) I store fragments
	// in these mini_poses...
	copy_dofs( pose, MiniPose( scratch_pose ), res_map, copy_dofs_for_junction_residues );
					...
					bool const copy_dofs_for_junction_residues /*= true*/ )
{
	...
	for ( ResMap::const_iterator
		...
		if ( !copy_dofs_for_junction_residues ) {
			//Disallow changes in residue torsions for any residue that might be adjoining
			// to the outside world?
			if ( scratch_pose.fold_tree().is_cutpoint(i_scratch_pose) &&
					 ( ! pose.fold_tree().is_cutpoint( i ) ) &&
					 i < pose.total_residue() ) continue;

			if ( (i_scratch_pose == 1 || scratch_pose.fold_tree().is_cutpoint(i_scratch_pose-1)) &&
					 i > 1 &&
					 ( ! pose.fold_tree().is_cutpoint( i - 1) )	 ) continue;
		}
		...
		for ( Size j = 1; j <= rsd_type.natoms(); j++ ) {
			...
			if ( !get_scratch_atom_id( current_atom_scratch_atom_id, atom_id_map, current_atom ) ) {
					std::cout << "No current atom id? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
				continue;
			}
			...
			if ( current_atom->is_jump() ) {
				...
				if ( !current_atom->parent() ) {
					std::cout << "No parent " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
					continue;
				}
				//					// completely copy over this root residue -- the rest of the function doesn't do the right thing
				//					// for the root residue!!!
				//					completely_copy_over_root_residue( pose, scratch_pose, atom_id_map, i );
				//					break;
				//				}
				...
				if ( !get_scratch_atom_id( input_stub_atom1_scratch_atom_id, atom_id_map, input_stub_atom1 ) ) {
					std::cout << "No JUMP input_stub_atom1? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
					continue;
				}
				if ( !get_scratch_atom_id( input_stub_atom2_scratch_atom_id, atom_id_map, input_stub_atom2 ) ) {
					std::cout << "No JUMP input_stub_atom2? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
					continue;
				}
				if ( !get_scratch_atom_id( input_stub_atom3_scratch_atom_id, atom_id_map, input_stub_atom3 ) ) {
					std::cout << "No JUMP input_stub_atom3? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
					continue;
				}
				...
				if ( !get_scratch_atom_id( stub_atom1_scratch_atom_id, atom_id_map, stub_atom1 ) ) {
					std::cout << "No JUMP stub_atom1? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
					continue;
				}
				if ( !get_scratch_atom_id( stub_atom2_scratch_atom_id, atom_id_map, stub_atom2 ) ) {
					std::cout << "No JUMP stub_atom2? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
					continue;
				}
				if ( !get_scratch_atom_id( stub_atom3_scratch_atom_id, atom_id_map, stub_atom3 ) ) {
					std::cout << "No JUMP stub_atom3? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
					continue;
				}
...
//				std::cout << "OLD " << pose.jump( AtomID( j, i ) ) << std::endl;
				...
				//				std::cout << "NEW " << pose.jump( AtomID( j, i ) ) << std::endl;
			...
			core::kinematics::tree::Atom const * input_stub_atom1( current_atom->input_stub_atom1() );
			if ( !get_scratch_atom_id( input_stub_atom1_scratch_atom_id, atom_id_map, input_stub_atom1 ) ) {
					std::cout << "No D input_stub_atom1? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
				continue;
			}
			...
			if (  input_stub_atom1->is_jump() && input_stub_atom1->parent() /* root is special case */ ) continue;
			/////////////////////////////////////////////////////////////////
			...
			for ( Size n = 0; n < input_stub_atom1->n_children(); n++ ) {
				...
				if (!get_scratch_atom_id( dummy_atom_id, atom_id_map, child_atom ) ) {
					std::cout << "No THETA ... Sister atom outside region of interest? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
			...
			core::kinematics::tree::Atom const * input_stub_atom2( current_atom->input_stub_atom2() );
			if ( !get_scratch_atom_id( input_stub_atom2_scratch_atom_id, atom_id_map, input_stub_atom2 ) ) {
				std::cout << "No THETA input_stub_atom2? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
				continue;
			}
			...
			/////////////////////////////////////////////////////////////////
			// WAIT DO WE NEED THIS?
			if ( input_stub_atom2->is_jump() && input_stub_atom2->parent() /* root is special case*/ ) continue;
			/////////////////////////////////////////////////////////////////

			core::kinematics::tree::Atom const * input_stub_atom3( current_atom->input_stub_atom3() );
			if ( !get_scratch_atom_id( input_stub_atom3_scratch_atom_id, atom_id_map, input_stub_atom3 ) ) {
				std::cout << "No PHI input_stub_atom3? " << " skipping " <<  pose.residue( i ).name1() << i << " " << pose.residue( i ).atom_name( j ) << std::endl;
				continue;
			}
