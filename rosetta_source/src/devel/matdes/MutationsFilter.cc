...
#include <devel/matdes/MutationsFilter.hh>
...
static basic::Tracer TR( "devel.matdes.MutationsFilter" );
...
void
MutationsFilter::write_to_pdb( std::map< core::Size, std::string > const & res_names1, std::map< core::Size, std::string > const & res_names2 ) const {
  ...
  std::string user_name = this->get_user_defined_name();
	...
	while( it_name1 != res_names1.end() ) {
		std::string output_string = "MutationsFilter " + user_name + ": " + it_name2->second + ObjexxFCL::string_of( it_name1->first ) + it_name1->second;
...
{
	out<<"MutationsFilter returns "<<compute( pose, false )<<std::endl;
}

void
MutationsFilter::parse_my_tag( utility::tag::TagPtr const tag,
