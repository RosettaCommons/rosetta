...
public:
    ...
    sfxn->set_energy_method_options( *emopts );
		...
		{ // scope
			SymmLinearMemNode * node1 = static_cast< SymmLinearMemNode * > ( symlinmem_ig->get_node( 1 ) );
		...
		//// Now let's make sure that the edge adjacency matrix is accurate
		{
			...
			for ( int ii = 11; ii <= 13; ++ii ) {
				...
				SymmLinearMemNode * iinode = static_cast< SymmLinearMemNode * > ( symlinmem_ig->get_node( iimoltenres ) );
