...
public: //
	...
	void test_limit_aroma_chi2()
	{
		...
		Size resid( 10 ); 
		Residue const & trial_res = pose.residue( resid  );
		...
		// create default packer task for comparison and rotamerset
		PackerTaskOP dtask( TaskFactory::create_packer_task( pose ) );
		GraphOP d_packer_neighbor_graph = core::pack::create_packer_graph( pose, *scfxn, dtask );		
					
	 	RotamerSetOP drotset = rsf.create_rotamer_set( trial_res );
		drotset->set_resid( resid );
		drotset->build_rotamers( pose, *scfxn, *dtask, d_packer_neighbor_graph );
		
		// create packer task that contain LimitAromaChi2Operation
		TaskFactory taskf;
		...
		GraphOP packer_neighbor_graph = core::pack::create_packer_graph( pose, *scfxn, ptask );
																 
		RotamerSetOP rotset = rsf.create_rotamer_set( trial_res );
		rotset->set_resid( resid );
		rotset->build_rotamers( pose, *scfxn, *ptask, packer_neighbor_graph );
