...
p <- ggplot(data=dens) + theme_bw() +
	geom_line(aes(x=180-acos(x)*180/pi, y=log(y + 1), colour=sample_source)) +
	geom_indicator(aes(colour=sample_source, indicator=counts)) +
	facet_grid(don_chem_type ~ acc_chem_type) +
	opts(title = "Hydrogen Bonds AHD Angle by Chemical Type\n(normalized for equal volume per unit distance)") +
	scale_y_continuous("log(FeatureDensity + 1)", limits=c(0,3.4), breaks=c(0,2)) +
