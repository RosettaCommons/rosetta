...
WHERE
  ...
  (don_site.HBChemType = 'hbdon_HXL' OR don_site.HBChemType = 'hbacc_AHXL');"
...
plot_id = "cosBAH_chem_type_res_q"
ggplot(data=dens) + theme_bw() +
	geom_line(aes(x=acos(x)*180/pi, y=log(y+1), colour=resolution_q)) +
	geom_indicator(aes(colour=resolution_q, indicator=counts)) +
#	facet_grid(don_chem_type ~ acc_chem_type) +
	opts(title = "OH Donor Hydrogen Bonds BAH Angle by Resolution\n(normalized for equal volume per unit distance)") +
