...
p <- ggplot(data=dens) + theme_bw() +
	...
	geom_indicator(aes(indicator=counts, colour=alpha_type)) +
	facet_wrap( ~ sample_source) +
	opts(title = "Backbone-Backbone H-Bonds A-H Distance by Alpha Type\nB-Factor < 30 normalized for equal weight per unit distance") +
...
p <- ggplot(data=dens) + theme_bw() +
	...
	geom_indicator(aes(indicator=counts, colour=alpha_type)) +
	facet_wrap( ~ sample_source) +
	opts(title = "Backbone-Backbone H-Bonds AHD Angle by Alpha Type\nB-Factor < 30 normalized for equal weight per unit distance") +
...
p <- ggplot(data=dens) + theme_bw() +
	...
	geom_indicator(aes(indicator=counts, colour=alpha_type)) +
	facet_wrap( ~ sample_source) +
	opts(title = "Backbone-Backbone H-Bonds BAH Angle by Alpha Type\nB-Factor < 30 normalized for equal weight per unit distance") +
...
p <- ggplot(data=dens) + theme_bw() +
	...
	geom_indicator(aes(indicator=counts, colour=alpha_type)) +
	facet_wrap( ~ sample_source) +
	opts(title = "Backbone-Backbone H-Bonds CHI Angle by Alpha Type\nB-Factor < 30 normalized for equal weight per unit distance") +
...
d_ply(f, .(sample_source), function(sub_f){
	...
	p <- ggplot(data=sub_f) + theme_bw() +
		...
		opts(title = paste("Backbone-Backbone H-Bonds CHI Angle by Alpha Type\nB-Factor < 30 equal coordinate projection Sample Source: ", ss_id, sep="")) +
