...
plot_parts <- list(
	...
	coord_equal(ratio=1),
	scale_x_continuous(expression(paste("phi Angle (Degrees)", sep="")), limits=c(-180, 180), parse=T),
	scale_y_continuous(expression(paste("psi Angle (Degrees)", sep="")), limits=c(-180, 180), parse=T),
...
d_ply(f, .(sample_source, res_type, dssp), function(sub_f){
	...
	sub_plot_id <- paste("ramachandran_res-", res_type, "_dssp-", dssp, sep="")
